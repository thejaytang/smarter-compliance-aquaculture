...
2026-09-17 handoff: source authority revision 13 (88 sources, 137 operations, 240 history rows, 73 source-version links) and human assessment holds survived Materials cleanup unchanged. All 73 referenced originals pass SHA-256 checks. The initial seed is a separate release asset; active originals, local config and generated workbook are no longer Git-updated. [Evidence](../project-support/shared-workspace-20260917/RESULTS.md).

2026-09-17 retention audit: the user retains genuine source-management review while excluding development Materials from first delivery. Current authority is revision 13; comparison with the saved revision-8 checkpoint found changes only to PE001 and its associated operations, with prior history unchanged and all four named assessment holds identical. The checkpoint is an authority restore, not a Collaboration ZIP; source-only colleague initialization remains to be prepared. No live record was changed. [Evidence and limits](../project-support/windows-colleague-handoff-20260917/REVIEW.md#3-source-preservation-evidence).

2026-09-16 selected human-record checkpoint: revision 8 is packaged with its immutable migration workbook and 4 named human assessment holds. Isolated restore matches all live owning tables and passes SQLite integrity, foreign keys and package hashes. All 71 original version references are covered by Git plus the included example original. [Archive and restore scope](saved-records/RESTORE.md). System1 dependencies now have a transitive hash lock consumed by both existing platform setup scripts and the shared rebuild entry in [ENVIRONMENT.md](../ENVIRONMENT.md). No live business tables were changed.
