...
def selection_review_trigger(
    ...
    return (
        ...
        since,
        "; ".join(reasons) + ".",
        "Review the source assessment in the browser workbench: System1 > Pending review.",
        fingerprint,
...
def review_trigger(record: dict[str, Any]) -> tuple[str, date | None, str, str] | None:
    ...
    if "official_url" in record and not str(record.get("official_url") or "").strip():
        return (
            ...
            "Confirm the official source identity in the browser workbench: System1 > Pending review.",
        )
...
def consolidate_review_queue(items: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for source_id, group in grouped.items():
        ...
        consolidated.append({
            ...
            **({"human_reported_issue": next(item["human_reported_issue"] for item in group if item.get("human_reported_issue"))}
               if any(item.get("human_reported_issue") for item in group) else {}),
        })
...
def run_leader(
    ...
    review_queue.extend(human_operations.pending_human_reported_issues(config_path))
    ...
    if provider_extensions["status"] == "DEGRADED":
        ...
        user_message = f"{desktop_state['open_count']} source(s) require attention in the browser workbench: System1 > Pending review."
    else:
