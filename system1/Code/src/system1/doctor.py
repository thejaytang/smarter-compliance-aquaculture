...
def _governance_check(path: Path) -> Check:
    ...
    try:
        with connect_sqlite(path.as_uri()+'?mode=ro', uri=True) as db:
            ...
            states = db.execute('SELECT schema,revision,import_sha256,archive FROM state').fetchall()
            if len(states) != 1 or states[0][0] != SCHEMA:
                raise ValueError('Unsupported governance database state')
            _, revision, expected, archive = states[0]
            if not isinstance(archive,str) or Path(archive).name != archive:
                raise ValueError('Invalid immutable companion path')
            if sha256((path.parent/archive).read_bytes()).hexdigest() != expected:
                raise ValueError('Immutable migration companion hash mismatch')
        return Check('governance_database','PASS',f'Revision {revision}; database integrity and immutable companion verified')
    except (OSError, ValueError, sqlite3.Error) as exc:
        return Check('governance_database','FAIL',str(exc))
...
def run_doctor(config_path: Path, schedule_path: Path | None = None) -> dict:
    ...
    database_mode = bool(raw.get('governance_db'))
    for key in ("workbook", "source_root", "backup_root", "log_root") + (('governance_db',) if database_mode else ()):
    ...
    if database_mode:
        checks.append(_governance_check((base/raw['governance_db']).resolve()))
    checks.append(Check("workbook", "PASS" if workbook.is_file() else "WARN" if database_mode else "FAIL",
                        str(workbook) if workbook.is_file() else 'Derived Excel is missing; synchronize from the saved database' if database_mode else str(workbook)))
    excel_open = excel_appears_open(workbook)
    checks.append(Check("excel_lock", ("WARN" if database_mode else "FAIL") if excel_open else "PASS",
                        'Excel synchronization waits for close; database decisions remain available' if excel_open and database_mode else "Workbook is open" if excel_open else "Workbook is available"))
