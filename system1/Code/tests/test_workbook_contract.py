...
class FinalWorkbookContractTest(unittest.TestCase):
    def test_dashboard_management_reporting_contract(self):
        ...
        operation_rows = [
            ...
            if operations.cell(row, operation_headers["operation_id"]).value
        ]
        # Compare the dashboard with its live inputs; business counts change after reviews.
        source_ids = [register.cell(r, register_headers["source_id"]).value for r in register_rows]
        self.assertEqual(len(source_ids), len(set(source_ids)))
        source_sheet = workbook["Source Register"]
        sources = [updater.record_from_row(source_sheet, r, register_headers) for r in register_rows]
        expected = {
            "A5": len(register_rows),
            "C5": sum(updater.selection_from_scores(r) == "INCLUDE" for r in sources),
            "E5": sum(r.get("snapshot_status") == "STORED" for r in sources),
            "H5": sum(operations.cell(r, operation_headers["program_status"]).value
                      in {"PENDING", "WAITING_FOR_HUMAN", "NEEDS_REPLAN", "QUEUED"}
                      for r in operation_rows),
            "L5": sum(r.get("download_status") in {"FAIL", "PAYWALL_BLOCKED"} for r in sources),
        }
        for cell, expected_value in expected.items():
            self.assertEqual(dashboard[cell].data_type, "f")
            cached = values_workbook["Dashboard"][cell].value
            # openpyxl intentionally clears Excel caches. Check caches when Excel saved them.
            if cached is not None:
                self.assertEqual(cached, expected_value, cell)
        self.assertEqual(len({operations.cell(r, operation_headers["source_id"]).value or operations.cell(r, operation_headers["operation_id"]).value
                              for r in operation_rows
                              if operations.cell(r, operation_headers["program_status"]).value
                              in {"PENDING", "WAITING_FOR_HUMAN", "NEEDS_REPLAN", "QUEUED"}}),
                         expected["H5"])
    ...
    def test_updater_preserves_english_contract_dashboard_and_excel_features(self):
        ...
        with tempfile.TemporaryDirectory() as temp_name:
            ...
            try:
                ...
                self.assertEqual(after.sheetnames, ["Instructions", "Categories", "Source Register", "Dashboard", "Human Operation Desktop", "Machine confidence"])
                self.assertEqual(
                    (
