...
**Exception remains an independent relationship.** New Exception and Subrequirement relationships must select another complete Requirement entry (Rx). Its own Scope/Conditions/Demand belong to that referenced entry. It is not automatically negated or merged into the main Scope. Use the two visible Exception and Subrequirement link selectors. Unlink removes only the relationship, never the referenced entry. Local fragments and internal child IDs are not selectable link targets; prior inline content and its exact quantity stay under History & structured result → Earlier inline relationships. Each active relationship uses one heading and one dropdown; linked Rx entries appear as compact buttons with Unlink. Legacy fragments are not repeated in the daily editor. No compatibility notice appears in the daily editor. The contextual field toolbar excludes both relationship types. A source-defined exception may be incomplete; do not invent a missing demand or deadline. Prior saved records remain readable and unmodified.
