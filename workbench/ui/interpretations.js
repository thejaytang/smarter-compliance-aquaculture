...
export class InterpretationEditor{
 ...
 render(){
  ...
  host.innerHTML=`<div class="ip-heading"><p class="ip-caption">Interpretation · revision ${d.revision||0}</p><p class="ip-state">${d.dirty?'Unsaved draft':d.reviewed?'Interpretation reviewed':d.revision?'Saved / not reviewed':'Not saved'}${d.stale||d.sourceChanged?' · needs source review':''}</p></div><p class="ip-notice" role="status">${esc(this.notice)}</p>${other.length?`<details><summary>${other.length} unsaved interpretation(s)</summary>${other.map(([k,x])=>`<button data-ip="switch" data-unit="${esc(x.unit_id)}">${esc(x.context?.requirement?.text?.slice(0,80)||x.unit_id)}</button>`).join('')}</details>`:''}
   <blockquote class="ip-original">${esc(d.context?.requirement?.text||d.latest?.context?.requirement?.text||'Current source unavailable')}</blockquote><div class="ip-source-actions"><button data-ip="source">Locate source</button><button data-ip="trace" ${d.revision?'':'disabled'}>Source trail</button></div>
   ...
   ${designMarkup(d.check_design,disabled)}
   ...
   <div class="ip-save"><button data-ip="save" class="primary" ${disabled||d.context_error?'disabled':''}>Save interpretation</button><button data-ip="review" ${disabled||d.dirty||!d.revision||d.stale||d.sourceChanged||interpretationKeys.some(k=>!d.fields[k].value.trim()||d.fields[k].basis==='unresolved'||d.fields[k].gaps.length)?'disabled':''}>Mark interpretation reviewed</button></div>
   <details><summary>History &amp; recovery</summary><button data-ip="reload" ${disabled}>Reload saved version</button><p>Restore creates a new revision. Splitting and interpretations are not included in collaboration ZIPs.</p>${(d.history||[]).map(h=>`<p>Revision ${h.revision} · ${esc(h.action)} · ${esc(h.at)} <button data-ip="restore" data-revision="${h.revision}" ${disabled}>Restore</button></p>`).join('')}</details>`;
  ...
  bindDesign(host,d.check_design,()=>{this.changed();host.querySelector('.ip-state').textContent='Unsaved draft';host.querySelector('[data-ip="review"]').disabled=true;},()=>this.render());
 ...
 async save(action,node){
  ...
  const request=action==='retry'?d.retry:{request_id:crypto.randomUUID(),unit_id:d.unit_id,expected_revision:d.revision,context_fingerprint:d.context?.fingerprint,linked_material_ids:d.linked_material_ids,fields:copy(d.fields),check_design:copy(d.check_design||emptyDesign()),action:action==='save'?'save':action,history_revision:Number(node.dataset.revision)};
