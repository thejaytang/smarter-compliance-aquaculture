...
export class StructureEditor{
 ...
 nodeMarkup(node,u,tree,labels,disabled,root=false,suppressRemove=false){
  ...
  if(node.kind==='reference'&&['exceptions','subrequirement'].includes(node.role)){
   ...
   const linked=this.e.completeRequirements(u.id).find(v=>v.id===node.target_id);
   return `<div class="rq-linked-requirement" data-structure-node="${id}" data-owner="${u.id}"><button type="button" data-rq="select-unit" data-id="${esc(node.target_id)}" title="${esc(linked?.text||this.e.unitText(node.target_id))}">${esc(this.e.label(node.target_id))}</button>${!this.e.locked?control('remove','Unlink','title="Remove this relationship only; keep the referenced Requirement"'):''}</div>`;
  }
  ...
  if(node.kind==='group'&&['exceptions','subrequirement'].includes(node.role)){
   const legacy=this.hasInlineRelation(node),children=node.children.filter(child=>this.hasExternalRelation(child));
   return `<section class="rq-reference-picker semantic ${semanticClass(node.role)}" data-structure-node="${id}" data-owner="${u.id}"><strong>${node.role==='exceptions'?'Exception':'Subrequirement'}</strong>${remove}${!legacy&&children.length>1?this.qcMarkup(node,disabled):''}<div class="rq-linked-list">${children.map(child=>this.nodeMarkup(child,u,tree,labels,disabled)).join('')}</div>${this.linkMarkup(u,node,disabled,true)}</section>`;
  }
