...
export const transparentGroup=n=>n.kind==='group'&&n.role==='requirements'&&!n.span&&!n.relationship&&!n.negated&&(n.quantity===1||JSON.stringify(n.quantity)==='[1,1]')&&n.children.length===1&&n.children[0].kind==='clause';
...
export class StructureEditor{
 ...
 nodeMarkup(node,u,tree,labels,disabled,root=false,suppressRemove=false){
  ...
  if(node.kind==='fragment')return `<div class="rq-tree-leaf semantic ${semanticClass(node.role)}" data-structure-node="${id}" data-owner="${u.id}"><span class="rq-tree-id">${label}</span><span>${esc(node.text)}</span>${!disabled?control('decompose','Decompose',` ${node.span?'':'disabled title="Source position is unresolved"'}`) :''}${remove}</div>`;
  ...
  if(node.kind==='group'&&['exceptions','subrequirement'].includes(node.role)){
   ...
   return `<section class="rq-reference-picker rq-relation-group semantic ${semanticClass(node.role)}" data-structure-node="${id}" data-owner="${u.id}"><strong>${node.role==='exceptions'?'Exception':'Subrequirement'}${nested?' · '+esc(labels[node.id]):''}</strong>${remove}<div class="rq-tree-body">${!legacy&&children.length?this.qcMarkup(node,locked):''}<ol class="rq-tree-children rq-linked-list">${rows}</ol>${!locked&&!legacy&&(children.length>1||nested)?`<div class="rq-tree-actions">${children.length>1?control('group','Group selected'):''}${nested?control('ungroup','Degroup','title="Remove this grouping and keep its links"'):''}</div>`:''}${this.linkMarkup(u,node,locked||legacy,true)}</div></section>`;
  }
 ...
 bind(host){
  ...
  host.querySelectorAll?.('[data-structure-source]').forEach(source=>{
   ...
   const capture=event=>{
    ...
    const bar=container.querySelector('[data-selection-tools]');if(bar){const owner=containers[0]||tree,relation=bar.querySelector('[data-straction="relationship"]');if(relation){relation.disabled=owner===tree||!owner.span||['exceptions','subrequirement'].includes(owner.role);relation.title=relation.disabled?'Create a Group around both parts first':'Use this source wording as the Group relationship; AND / OR use quantity';}const rect=range.getBoundingClientRect();bar.style.left=Math.max(8,Math.min(event?.clientX??rect.left,window.innerWidth-bar.offsetWidth-8))+'px';bar.style.top=Math.max(8,Math.min((event?.clientY??rect.bottom)+10,window.innerHeight-bar.offsetHeight-8))+'px';}
