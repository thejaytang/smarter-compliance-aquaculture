...
export class StructureEditor{
 ...
 toolsMarkup(allowed=roles){return `<div class="rq-selection-tools" data-selection-tools hidden role="toolbar" aria-label="Assign selected source text"><span class="rq-tool-section">${control('add-group','Group')}</span><span class="rq-tool-divider" aria-hidden="true"></span><span class="rq-tool-section">${allowed.filter(role=>!['exceptions','subrequirement','requirements'].includes(role)).map(role=>control('add',esc(role),`data-field="${role}" class="semantic ${semanticClass(role)}"`)).join('')}</span></div>`;}
