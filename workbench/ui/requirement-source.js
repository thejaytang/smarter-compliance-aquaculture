import {treeNodes} from './requirement-structure.js';
...
export function sourcePreview(doc){
 ...
 for(const tree of Object.values(doc.structures||{})){let g=0,f=0;for(const n of treeNodes(tree))labels[n.id]=n.kind==='fragment'?`F${++f}`:n.kind==='reference'?(doc.labels?.[n.target_id]||n.target_id):`G${++g}`;}
 for(const [id,u] of Object.entries(doc.units||{})){
  ...
  if(doc.structures?.[id]){
   for(const n of treeNodes(doc.structures[id])){
    const field=n.origin_role||n.role;if(!field||field==='requirements')continue;
    if(n.span)spans.push({field,start:offset+n.span[0],end:offset+n.span[1],id:n.id,owner:id});
    else if(n.kind==='reference'&&doc.spans?.[n.target_id]){const [start,end]=doc.spans[n.target_id];spans.push({field,start,end,id:n.target_id,owner:id});}
   }continue;
  }
 ...
 const groups=spans.filter(s=>s.owner).map(s=>`<span class="annotation-relation semantic ${semanticClass(s.field)}">${esc(s.field)} · ${esc(labels[s.id]||s.id)} in ${esc(doc.labels?.[s.owner]||s.owner)}</span>`).join('');
