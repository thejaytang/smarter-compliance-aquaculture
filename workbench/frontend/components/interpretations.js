...
import {emptyDesign,setDesign,setHandoff,domainMarkup,ruleMarkup,setDefinitionMarkup,logicText,logicFields,pruneConcepts,conceptIssues,bindDesign,revealDesign} from './check-design.js';
...
export function logicMarkup(){return '<p class="ip-caption">Required set relation</p><p class="ip-set-formula">B ⊆ A &nbsp;·&nbsp; B ⊆ C</p>';}
...
export class InterpretationEditor{
 ...
 hasQuotation(d){return Object.values(d?.citationDrafts||{}).some(c=>!!c.quote);}
 hasUnsaved(){return [...this.drafts.entries()].some(([key,d])=>key.startsWith(this.owner()+':')&&(d.dirty||d.retry||d.generationRequest||this.hasQuotation(d)))||this.pending;}
 ...
 async open(uid){
  this.restoreJournal();const key=this.key(uid),owner=this.owner();if(this.active!==key&&this.draft?.designUI)this.draft.designUI.undo=null;this.active=key;this.loading=true;this.notice='Loading interpretation…';this.render();
  try{const doc=await this.m.api('/api/interpretations?'+new URLSearchParams({unit_id:uid}));if(this.owner()!==owner||this.active!==key)return;
   ...
   const old=this.drafts.get(key);if(remote&&old?.dirty&&remote.revision!==old.draftRevision)old.workingConflict=true;if(old?.dirty||old?.retry||old?.generationRequest||this.hasQuotation(old)){old.latest=doc;old.provider=doc.provider;old.conflict=old.conflict||old.revision!==doc.revision||JSON.stringify(old.catalog)!==JSON.stringify(doc.catalog);old.sourceChanged=old.sourceChanged||old.context?.fingerprint!==doc.context?.fingerprint||old.revision!==doc.revision;old.impact=doc.impact;if(old.sourceChanged&&old.designUI)old.designUI.undo=null;}
   else this.drafts.set(key,{...reviewDraft(doc),dirty:false,citationDrafts:old?.citationDrafts,disclosures:old?.disclosures,candidate:(doc.runs||[]).find(r=>r.status==='ready'&&r.context_fingerprint===doc.context?.fingerprint)});
   ...
   const running=doc.runs?.find(r=>r.status==='running');if(running){this.drafts.get(key).generation=running;void this.watch(running.id,key);}
  }catch(e){this.notice=e.message;}finally{if(this.active===key){this.loading=false;this.render();}}
 }
 sourceChanged(){for(const [key,d] of this.drafts)if(key.startsWith(this.owner()+':')){d.sourceChanged=true;if(d.designUI)d.designUI.undo=null;}this.render();}
 ...
 render(){
  ...
  if(this.renderedKey&&this.drafts.has(this.renderedKey))this.drafts.get(this.renderedKey).disclosures=[...(host.querySelectorAll?.('details[open][data-disclosure]')||[])].map(el=>el.dataset.disclosure);
  ...
  const inspectTerm=e=>{const term=e.target.closest('[data-concept-nav]');if(!term)return false;e.preventDefault();e.stopPropagation();revealDesign(host,term.dataset.ruleId,JSON.parse(term.dataset.conceptNav));return true;};
  host.onclick=e=>{if(inspectTerm(e))return;const b=e.target.closest('[data-ip]');if(b&&!b.disabled)void this.action(b.dataset.ip,b).catch(error=>{this.notice=error.message;this.render();});};
  ...
  if(valid&&d.check_design){d.designUI||={};bindDesign(host,d.check_design,()=>this.changed(),destination=>{this.render();if(destination)revealDesign(this.host,destination.ruleId,destination.conceptIds);},d.catalog,d.fields,d.designUI);}
 }
 fieldMarkup(d,key,label,readOnly=false){
  const f=d.fields[key],staged=d.citationDrafts?.[key],disabled=this.m.collaboration.readonly?'disabled':'';
  return `<div class="ip-field" data-ip-field="${key}">${readOnly?`<details data-disclosure="earlier-${key}"><summary>Earlier explanation</summary><p>${esc(f.value||'No earlier explanation.')}</p></details>`:`<label>${esc(label)}<textarea data-logic-value="${key}" aria-label="${esc(label)}" rows="3" ${disabled}>${esc(f.value)}</textarea></label>`}<details class="ip-evidence" data-disclosure="evidence-${key}"><summary>Quotations &amp; questions (${f.references?.length||0})</summary>${f.state==='not_stated'?`<p>${esc(f.absence_reason)}</p>`:''}${(f.references||[]).map((ref,i)=>`<blockquote>${esc(ref.quote)}</blockquote><button data-ip="remove-ref" data-ref="${i}" ${disabled}>Remove citation</button>`).join('')}<label>Saved context passage<select data-ip-citation ${disabled}>${staged?.id&&!(d.context?.citations||[]).some(ref=>ref.id===staged.id)?`<option value="${esc(staged.id)}" selected disabled>Unavailable source · ${esc(staged.id)}</option>`:''}${(d.context?.citations||[]).map(ref=>`<option value="${esc(ref.id)}" ${staged?.id===ref.id?'selected':''}>${esc(ref.text.slice(0,130))}</option>`).join('')}</select></label><label>Exact supporting quotation<textarea data-ip-quote rows="2" ${disabled}>${esc(staged?.quote||'')}</textarea></label><button data-ip="add-ref" ${disabled}>Add citation</button><label>Open questions · one per line<textarea data-field-gaps="${key}" rows="2" ${disabled}>${esc((f.gaps||[]).join('\n'))}</textarea></label></details></div>`;
 }
 ...
 cardCandidate(d,key,index,disabled){
  ...
  const design=setDesign(c.check_design||emptyDesign());
  return `<div class="ip-candidate"><details data-disclosure="candidate-${key}"><summary><span class="ip-caption">AI candidate · approval required</span><br>${setDefinitionMarkup(design,key,c.suggestions[key],false)}</summary><div class="ip-review-notes">${[key,logicKeys[index]].map(k=>`<p>${esc(c.suggestions[k]?.value||'')}</p>${(c.suggestions[k]?.references||[]).map(ref=>`<blockquote>${esc(ref.quote)}</blockquote>`).join('')}${(c.suggestions[k]?.gaps||[]).map(g=>`<p class="ip-warning">${esc(g)}</p>`).join('')}`).join('')}</div></details><button data-ip="accept-card" data-field="${key}" ${disabled}>Approve ${key} candidate</button><button data-ip="dismiss-card" data-field="${key}" ${disabled}>Dismiss candidate</button></div>`;
 }
 editorMarkup(d){
  ...
  return `<div class="ip-heading"><p class="ip-state">${esc(r.label(d.unit_id))} · ${d.dirty?'Unsaved changes':d.revision?'Saved':'Not saved'} · ${d.reviewed&&!d.stale&&!d.sourceChanged?'Confirmed':'Review pending'}</p></div><p class="ip-notice" role="status">${esc(notice)}</p>
   ...
   ${d.conflict?'<p class="ip-warning">The saved interpretation or its dependencies changed. Compare before saving again.</p><button data-ip="compare-conflict">Compare saved and local</button>':d.stale||d.sourceChanged?'<p class="ip-warning">Source changed. Refresh before saving or generating.</p><button data-ip="context">Refresh source</button>':''}
   <div class="ip-generate"><button data-ip="generate" ${disabled||!d.provider?.available||d.generation||d.generationRequest?'disabled':''}>${d.generation?'Generating candidates…':'AI Gen'}</button>${d.generationRequest&&!d.generation?`<button data-ip="generation-retry" ${d.generationStarting?'disabled':''}>Retry AI Gen request</button>`:''}${d.generationStatusError?'<button data-ip="generation-status">Retry status</button>':''}${!d.provider?.available?'<span class="ip-caption">Not connected</span>':''}</div>
   ${cardKeys.map((key,i)=>`<section class="ip-group" data-card="${key}"><h4>${['Scope · A','Condition · B','Demand · C'][i]}</h4><p class="ip-caption">Combined extract</p><blockquote class="ip-source-wording" tabindex="0" aria-label="${['Scope','Condition','Demand'][i]} program-combined extract">${esc(derived[key].value||'No wording assigned in the third pane.')}</blockquote>
    <details class="ip-set-editor" data-disclosure="set-${key}"><summary title="Edit set ${'ABC'[i]}">${setDefinitionMarkup(design,key,d.fields[key])}</summary>
     ${design.groups[key]?ruleMarkup(design,key,editDisabled,d.catalog,d.fields,d.designUI):`${this.fieldMarkup(d,key,'Set '+ 'ABC'[i]+' definition')}${ruleMarkup(design,key,editDisabled,d.catalog,d.fields,d.designUI)}`}
     <details class="ip-review-notes" data-disclosure="review-${key}"><summary>Review notes</summary>${design.groups[key]?this.fieldMarkup(d,key,'',true):''}${this.fieldMarkup(d,logicKeys[i],interpretationLabels[interpretationKeys.indexOf(logicKeys[i])])}${i===0?domainMarkup(design,editDisabled):''}</details>
    </details>
 ...
 refreshOutput(){const d=this.draft;for(const key of cardKeys){const summary=this.host?.querySelector?.(`[data-disclosure="set-${key}"] > summary`);if(summary)summary.innerHTML=setDefinitionMarkup(d.check_design,key,d.fields[key]);}const review=this.host?.querySelector?.('[data-ip="review"]');if(review){review.disabled=true;review.hidden=true;}}

 changed(){const d=this.draft;if(d.designUI)d.designUI.undo=null;this.host?.querySelector?.('[data-rd="undo-remove"]')?.parentElement?.remove();d.dirty=true;d.reviewed=false;d.workingStored=false;d.check_design=setDesign(d.check_design||emptyDesign());d.check_design.based_on=null;d.changeNumber=(d.changeNumber||0)+1;const state=this.host?.querySelector?.('.ip-state');if(state)state.textContent='Unsaved changes';
 ...
 approveCard(key,runId=null){
  ...
  this.notice=key[0].toUpperCase()+key.slice(1)+' approved in this draft. Save to record your approval.';this.render();this.focusCard(key);
 }
 focusCard(key){const status=this.host?.querySelector?.(`[data-card-status="${key}"]`);status?.focus?.({preventScroll:true});status?.scrollIntoView?.({block:'nearest'});}
 ...
 async action(action,node){
  ...
  if(action==='compare-conflict')return this.compareConflict();
  if(action==='generation-retry')return this.startGeneration(this.active);
  if(action==='generation-status'){if(d.generation)return this.watch(d.generation.id,this.active);return;}
  if(action==='approve-card')return this.approveCard(node.dataset.field);
  if(action==='dismiss-card'){d.candidate.dismissed=[...(d.candidate.dismissed||[]),node.dataset.field];this.notice=node.dataset.field[0].toUpperCase()+node.dataset.field.slice(1)+' candidate dismissed. Current card content is retained.';this.render();this.focusCard(node.dataset.field);return;}
  ...
  if(action==='context'){
   ...
   if(d.context?.fingerprint!==ctx.fingerprint)for(const term of d.check_design?.concepts||[])term.status='proposed';d.context=ctx;d.linked_material_ids=ids;d.context_error=null;d.stale=false;d.sourceChanged=!!d.conflict;this.changed();this.notice='Context refreshed. Check each card against these source versions.';return this.render();
  }
  if(action==='related'){
  ...
  if(action==='add-ref'){const el=node.closest('[data-ip-field]'),id=el.querySelector('[data-ip-citation]').value,quote=el.querySelector('[data-ip-quote]').value,c=d.context?.citations.find(c=>c.id===id);if(!quote||!c?.text.includes(quote))throw Error('Use an exact nonempty quotation from the selected saved block.');if(!field.references.some(r=>r.id===id&&r.quote===quote))field.references.push({id,quote});delete d.citationDrafts?.[k];this.changed();return this.render();}
  ...
  if(action==='generate'||action==='generate-one'){
   ...
   if(this.m.collaboration.readonly||this.pending||d.generation||d.generationRequest)throw Error('Wait for the current operation before generating.');
   ...
   this.m.q('[data-ip-confirm]').onclick=async()=>{if(this.m.q('[data-ip-confirm]').disabled)return;if(this.active!==key||this.draft!==d||d.generationRequest||d.generation)throw Error('The active request changed. Reopen AI Gen.');this.m.q('[data-ip-confirm]').disabled=true;d.generationRequest=copy(request);this.m.q('#mw-dialog').close();await this.startGeneration(key);};return;
  }
  if(action==='reload'){
   if(d.dirty||d.retry||d.generationRequest||this.hasQuotation(d)){this.m.dialog('<h2>Reload saved interpretation?</h2><p>Your current unsaved draft will be replaced by the server version.</p><button data-ip-confirm>Reload saved version</button>');this.m.q('[data-ip-confirm]').onclick=async()=>{try{await this.clearWorking(d);this.m.q('#mw-dialog').close();this.drafts.delete(this.active);this.journal();void this.open(d.unit_id);}catch(e){this.m.message(e.message,'warning');}};return;}
 ...
 async save(action,node){
  ...
  try{const result=await this.m.api('/api/interpretations/save',request);d.retry=null;d.dirty=false;d.draftRevision=result.draft_revision??d.draftRevision;this.journal();const saved=result.document||await this.m.api('/api/interpretations?'+new URLSearchParams({unit_id:d.unit_id}));if((d.changeNumber||0)!==changeAtStart){d.revision=saved.revision;d.context=saved.context;d.card_reviews=saved.card_reviews;for(const k of Object.keys(request.approve_cards||{}))if(cardSnapshot(d,k)===cardSnapshot(saved,k))delete d.pendingApprovals?.[k];d.dirty=true;this.notice='Saved. Newer edits remain unsaved.';}else{this.drafts.set(key,{...reviewDraft(saved),draftRevision:d.draftRevision,dirty:false,candidate:d.candidate,generation:d.generation,generationRequest:d.generationRequest,generationStatusError:d.generationStatusError,generationStarting:d.generationStarting,generationPolling:d.generationPolling,citationDrafts:d.citationDrafts,disclosures:d.disclosures,designUI:{values:d.designUI?.values}});this.notice=action==='review'?'Interpretation confirmed.':'Draft and individual approvals saved.';}if(this.hasQuotation(this.drafts.get(key)))this.notice+=' An unadded quotation remains unsaved. Use Add citation to include it.';}
  catch(e){if(e.definitive){d.retry=null;if(e.status===409){d.sourceChanged=true;d.conflict=true;}}this.notice=e.message+(d.retry?' Save outcome uncertain. Retry the same save before leaving.':'');this.journal();}
  finally{this.pending=false;this.render();}
 }
 async compareConflict(){
  const key=this.active,d=this.draft;if(!d||this.pending||d.retry)return;const view=Symbol();this.conflictView=view;
  const latest=await this.m.api('/api/interpretations?'+new URLSearchParams({unit_id:d.unit_id}));
  if(this.active!==key||this.draft!==d||this.conflictView!==view)return;
  const localContext=await this.m.api('/api/interpretations/context',{unit_id:d.unit_id,linked_material_ids:d.linked_material_ids||[]});
  if(this.active!==key||this.draft!==d||this.conflictView!==view)return;
  const signature=()=>JSON.stringify([d.fields,d.check_design,d.revision,d.context?.fingerprint,d.linked_material_ids]),before=signature();
  const stamp=doc=>JSON.stringify([doc.revision,doc.context?.fingerprint,doc.catalog,doc.context_error]);
  const content=doc=>cardKeys.map((k,i)=>`<section><h4>${esc(k[0].toUpperCase()+k.slice(1))}</h4>${[k,logicKeys[i]].map(field=>`<p>${esc(doc.fields[field]?.value||doc.fields[field]?.absence_reason||'Not filled')}</p>${(doc.fields[field]?.references||[]).map(r=>`<blockquote>${esc(r.quote)}</blockquote><p>${esc(r.id)}</p>`).join('')}`).join('')}</section>`).join('')+`<details><summary>Complete definitions, concepts and evidence</summary><pre>${esc(JSON.stringify({fields:doc.fields,check_design:doc.check_design},null,2))}</pre></details>`;
  const contextChanged=d.context?.fingerprint!==localContext.fingerprint,catalogChanged=JSON.stringify(d.catalog)!==JSON.stringify(latest.catalog);
  this.m.dialog(`<h2>Compare saved and local interpretation</h2><p>Local starting revision ${d.revision} · latest saved revision ${latest.revision}. ${contextChanged?'Source context changed. Review the local definitions against the refreshed evidence below.':'Local source context is unchanged.'} ${catalogChanged?'Site Model catalog changed; review mappings.':''}</p><div class="collab-comparison"><section><h3>Your protected local draft</h3>${content(d)}</section><section><h3>Latest saved interpretation</h3>${content(latest)}</section></div>${contextChanged?`<details open><summary>Current source context</summary>${(localContext.citations||[]).map(c=>`<blockquote>${esc(c.text)}</blockquote><p>${esc(c.id)}</p>`).join('')}</details>`:''}<p>No choice saves or approves an interpretation. Closing keeps your local draft.</p><button data-conflict-use="saved">Reload latest saved</button><button data-conflict-use="local">Continue with reviewed local draft</button><p id="ip-conflict-error" role="status"></p>`,dialog=>{
   let busy=false,cancelled=false;const feedback=dialog.querySelector('#ip-conflict-error'),visible=()=>!cancelled&&dialog.open!==false&&this.conflictView===view&&dialog.querySelector('#ip-conflict-error')===feedback;dialog.addEventListener?.('cancel',()=>cancelled=true,{once:true});dialog.addEventListener?.('close',()=>cancelled=true,{once:true});dialog.querySelectorAll('[data-conflict-use]').forEach(button=>button.onclick=async()=>{
    if(busy||!visible())return;busy=true;dialog.querySelectorAll('[data-conflict-use]').forEach(b=>b.disabled=true);
    try{
     if(this.active!==key||this.draft!==d||signature()!==before||this.pending||d.retry)throw Error('The local draft changed. Reopen the comparison to review it.');
     const verified=await this.m.api('/api/interpretations?'+new URLSearchParams({unit_id:d.unit_id}));
     if(!visible())return;const ctx=await this.m.api('/api/interpretations/context',{unit_id:d.unit_id,linked_material_ids:d.linked_material_ids||[]});
     if(!visible())return;
     if(stamp(verified)!==stamp(latest)||ctx.fingerprint!==localContext.fingerprint)throw Error('A version changed again. Reopen the comparison for the latest values.');
     if(this.active!==key||this.draft!==d||signature()!==before)throw Error('The local draft changed. Reopen the comparison to review it.');
     if(button.dataset.conflictUse==='saved')this.drafts.set(key,{...reviewDraft(verified),dirty:false,citationDrafts:d.citationDrafts,disclosures:d.disclosures,generationRequest:d.generationRequest,generation:d.generation,generationStatusError:d.generationStatusError});
     else {
      d.revision=verified.revision;d.context=ctx;d.catalog=verified.catalog;d.provider=verified.provider;d.card_reviews=verified.card_reviews;d.history=verified.history;d.adopted_candidates=verified.adopted_candidates;d.approvalSnapshots={};d.pendingApprovals={};d.reviewed=false;d.conflict=false;d.sourceChanged=false;d.stale=false;d.context_error=null;if(contextChanged||catalogChanged)for(const term of d.check_design?.concepts||[])term.status='proposed';this.changed();
     }
     this.notice=button.dataset.conflictUse==='saved'?'Latest saved interpretation loaded.':'Reviewed local draft retained against the latest revision. Save and approve each card explicitly.';dialog.close();this.render();
    }catch(e){if(visible())feedback.textContent=e.message;}finally{busy=false;if(visible())dialog.querySelectorAll('[data-conflict-use]').forEach(b=>b.disabled=false);}
   });
  });
 }
 async startGeneration(key){
  const d=this.drafts.get(key);if(!d?.generationRequest||d.generationStarting)return;
  const request=d.generationRequest;d.generationStarting=true;if(this.active===key)this.render();
  try{
   const run=await this.m.api('/api/interpretations/generate',request),current=this.drafts.get(key);if(!current||current.generationRequest!==request)return;
   current.generationRequest=null;current.generation=run;current.generationStatusError=false;this.journal();if(this.active===key){this.notice='Generation in progress. Your formal fields remain editable.';this.render();}void this.watch(run.id,key);
  }catch(e){const current=this.drafts.get(key);if(current?.generationRequest===request){if(e.definitive)current.generationRequest=null;if(this.active===key){this.notice=e.message+(e.definitive?'':' The outcome is uncertain. Retry the same AI Gen request.');this.render();}}}
  finally{d.generationStarting=false;const current=this.drafts.get(key);if(current)current.generationStarting=false;if(this.active===key)this.render();}
 }
 async watch(id,key){
  const original=this.drafts.get(key);if(!original||original.generationPolling)return;original.generationPolling=true;original.generationStatusError=false;
  try{const run=await this.m.api('/api/interpretations/run?'+new URLSearchParams({id}));const d=this.drafts.get(key);if(!d||d.generation?.id&&d.generation.id!==id)return;
   if(run.unit_id&&run.unit_id!==d.unit_id)return;
   if(run.status==='running'){d.generation=run;setTimeout(()=>this.watch(id,key),1500);return;}
   d.generation=null;d.generationStatusError=false;
   if(run.status==='ready'&&run.context_fingerprint===d.context?.fingerprint){d.candidate=run;if(this.active===key)this.notice='Three candidates ready. Approve each separately to fill your draft.';}
   else if(this.active===key)this.notice=run.error||'Candidate is stale. Refresh source context before generating again.';
   if(this.active===key)this.render();
  }catch(e){const d=this.drafts.get(key);if(d&&(!d.generation?.id||d.generation.id===id)){d.generationStatusError=true;if(this.active===key){this.notice='Generation status unavailable. Retry status for the same request.';this.render();}}}
  finally{original.generationPolling=false;const d=this.drafts.get(key);if(d)d.generationPolling=false;}
 }
}
