...
export const modulePages = Object.freeze({
  addSources: [['addSources','Official website','url'],['sourceUpload','Upload file','manual'],['sourceDiscovery','Source discovery','search']],
});
export function moduleFor(view) {
  return Object.keys(modulePages).find(key=>modulePages[key].some(([id])=>id===view)) || view;
}
const storageKey = 'workbench-navigation-v2';
export function workspaceFor(view) {
  return Object.keys(workspaceModules).find(key=>workspaceModules[key].some(([id])=>id===moduleFor(view))) || null;
}
