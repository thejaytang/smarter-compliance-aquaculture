...
const pdfUrl = '/demo-assets/review-sample.pdf';
const sourceImage = page => '/demo-assets/sample-page-' + (page + 1) + '.webp';
...
export class System2Review {
  ...
  render() {
    ...
    const banner = `<div class="s2-demo-bar"><div><span class="s2-badge">Review demo</span> <span>Sample data · Live Requirement Extraction System is not connected</span></div><small>Practice records stay in this browser, under your reviewer name.</small></div>`;
  ...
  renderSource(item) {
    ...
    if (item.format === 'PDF') {
      source.innerHTML = `<div class="s2-evidence-links"><a href="${pdfSource.officialUrl}" target="_blank" rel="noopener noreferrer">Synthetic sample ↗</a><a href="/demo-assets/review-sample.pdf">Download PDF ↓</a></div><div class="s2-view-controls"><button id="s2-region" aria-pressed="${!this.full}">Highlighted region</button><button id="s2-page" aria-pressed="${!!this.full}">Full page</button><select id="s2-zoom" aria-label="Source zoom">${[['fit','Fit width'],['150','150%'],['200','200%']].map(([value,label])=>`<option value="${value}" ${(this.zoom||'fit')===value?'selected':''}>${label}</option>`).join('')}</select></div><div class="s2-image-scroll" id="s2-image" data-zoom="${this.zoom||'fit'}" tabindex="0" aria-label="Source image; zoom to enlarge, scroll to move across the page">${picture(item.page,item.boxes,!!this.full)}</div><p class="s2-source-caption">Page ${item.page+1} of ${pdfSource.sourcePages} · Red outline marks the review area. Zoom to enlarge.${item.kind==='table'?' Region located manually for this demo.':''}</p>${item.footnote?`<details><summary>Footnote 6 · View source</summary><div class="s2-image-scroll">${picture(item.page,item.footnote.boxes,false,true)}</div><p class="muted">${esc(item.footnote.text)}</p></details>`:''}<button id="s2-open-pdf" class="s2-reader-toggle">Screenshot does not match? Open full PDF</button><div id="s2-reader-area" hidden></div><details class="s2-provenance"><summary>Source details</summary><p>${esc(pdfSource.title)}<br>Synthetic sample · English</p><p>Example: ${pdfSource.historicalRun}. This document is demonstration evidence, not a live Requirement Extraction System task.</p><small>Source SHA-256: ${pdfSource.sourceHash}</small></details>`;
  ...
  async openPDF(item) {
    ...
    try {
      ...
      if(!response.ok)throw Error('Synthetic sample unavailable.');
      const buffer=await response.arrayBuffer();
      const hash=Array.from(new Uint8Array(await crypto.subtle.digest('SHA-256',buffer)),b=>b.toString(16).padStart(2,'0')).join('');
      if(hash!==pdfSource.sourceHash)throw Error('Synthetic sample identity differs.');
      display(pdfUrl,'Synthetic sample identity verified.');
