...
export async function settings(container,api,{run=fn=>fn(),readonly=false}={}) {
  ...
  container.innerHTML=`<section class="panel"><h2>Confidence thresholds</h2><p>Scores guide review routing; they are not document accuracy. Missing or uncalibrated scores and evidence conflicts always require review.</p><form id="policy-form">${Object.entries(labels).map(([key,label])=>`<label>${label}<input type="number" name="${key}" min="0.01" max="100" step="0.01" required value="${(data.policy[key]*100).toFixed(2)}"></label>`).join('')}<p>Raising thresholds reopens affected machine decisions. Lowering thresholds rechecks untouched, low-score-only tasks. Human decisions and drafts remain protected.</p><button>Save and re-evaluate</button><div role="status" id="policy-status"></div></form><details><summary>Policy history</summary><table><thead><tr><th>Revision</th><th>Operator</th><th>Changed</th><th>Source Management System</th><th>Content</th><th>Requirements</th></tr></thead><tbody>${data.history.map(h=>`<tr><td>${h.policy.revision}</td><td>${esc(h.actor)}</td><td>${esc(new Date(h.created*1000).toLocaleString())}</td><td>${percent(h.policy.system1)}</td><td>${percent(h.policy.content)}</td><td>${percent(h.policy.requirement)}</td></tr>`).join('')}</tbody></table></details></section>`;
  const form=container.querySelector('#policy-form'),values=()=>Object.fromEntries([...form.querySelectorAll('input[name]')].map(e=>[e.name,Number(e.value)/100]));
  if(readonly)form.querySelectorAll('input,button').forEach(node=>node.disabled=true);
  form.onsubmit=event=>{event.preventDefault();if(readonly||settingsPending(form))return;const payload={revision:data.policy.revision,values:{...values(),system3:data.policy.system3}};
   return settingsOperation(form,run,async()=>{try{await api('/api/settings',payload);await settings(container,api,{run,readonly});container.querySelector('#policy-status').textContent='Confidence thresholds saved and routing re-evaluated. Human decisions remain separate.';}catch(error){settingsFailure(container.querySelector('#policy-status'),error,{read:()=>api('/api/settings'),local:values,saved:x=>Object.fromEntries(Object.keys(labels).map(k=>[k,x.policy[k]])),stamp:()=>JSON.stringify(values()),useLocal:latest=>Object.assign(data,latest),useSaved:latest=>{Object.assign(data,latest);for(const key of Object.keys(labels))form.elements[key].value=(latest.policy[key]*100).toFixed(2);},run});}});
  };
}
