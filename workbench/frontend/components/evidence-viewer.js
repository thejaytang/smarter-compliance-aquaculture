...
export async function showEvidence(container, sourceId, api, {page = 1, sheet = '', row = 1, column = 1, locator='', references=[], full=false, documentId='', unitId='', interactivePDF=false} = {}) {
  const options={page,sheet,row,column,locator,references,full,documentId,unitId,interactivePDF};
  ...
  try {
    ...
    if(result.image){
      const img=document.createElement('img');img.alt=result.image_label||result.label;img.style.width='100%';
      const enlarge=document.createElement('button');enlarge.type='button';enlarge.textContent='Enlarge original region';enlarge.disabled=true;
      ...
      const failure=document.createElement('p');failure.setAttribute('role','alert');failure.hidden=true;
      const retry=document.createElement('button');retry.type='button';retry.textContent='Retry original region';retry.hidden=true;
      const current=()=>container.isConnected&&container.evidenceRequest===request;
      img.onload=()=>{if(!current())return;enlarge.disabled=false;failure.hidden=true;retry.hidden=true;};
      img.onerror=()=>{if(!current())return;enlarge.disabled=true;failure.textContent='The original region image could not be displayed. Retry this region.';failure.hidden=false;retry.hidden=false;};
      retry.onclick=()=>{if(!current())return;if(cacheKey)evidenceCache.delete(cacheKey);return showEvidence(container,sourceId,api,options);};
      enlarge.onclick=()=>{if(current()&&!enlarge.disabled){fullImage.src=result.image;dialog.showModal();}};
      container.append(enlarge,img,failure,retry,dialog);img.src=result.image;
    }
    if(result.kind==='html_region'||result.kind==='html'){
      const frame=document.createElement('iframe');frame.setAttribute('sandbox','');frame.setAttribute('referrerpolicy','no-referrer');frame.className='original-html-reader';frame.title=result.kind==='html'?'Simplified HTML reading view':'Isolated bound original region';if(result.kind==='html'){
        if(!/^[0-9a-f]{64}$/.test(result.source_sha256||''))throw Error('The original HTML version is missing. Reload the source.');
        frame.src='/api/preview/'+encodeURIComponent(sourceId)+'?'+new URLSearchParams({view:'html',expected_hash:result.source_sha256});
      }else frame.srcdoc=result.html;
      container.append(frame);return result;
    }
    ...
    if (result.kind === 'pdf') {
      ...
      select.onchange=()=>showEvidence(container,sourceId,api,{...options,sheet:select.value,row:1,column:1,locator:''});controls.append(select);
      for(const [name,r,c] of [['Previous rows',Math.max(1,result.row-40),result.column],['Next rows',result.row+40,result.column],['Previous columns',result.row,Math.max(1,result.column-12)],['Next columns',result.row,result.column+12]]) {
        const button=document.createElement('button');button.textContent=name;button.disabled=r>result.rows||c>result.columns||(r===result.row&&c===result.column);
        button.onclick=()=>{if(!button.disabled)return showEvidence(container,sourceId,api,{...options,sheet:result.sheet,row:r,column:c,locator:''});};controls.append(button);
      }
