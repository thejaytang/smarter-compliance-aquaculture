...
def read_config(config_path: Path) -> dict[str, Any]:
    ...
    for key in ("workbook", "source_root", "backup_root", "log_root", "manual_intake_root", "manual_intake_archive_root", "governance_db", "assessment_db"):
...
def open_registry(config, data_only=False):
    if config.get('governance_db'):
        from system1.governance_store import GovernanceStore
        return GovernanceStore(config['governance_db']).business_workbook(config, data_only=data_only)
    return load_workbook(config['workbook'], data_only=data_only)


def registry_revision(config):
    if config.get('governance_db'):
        from system1.governance_store import GovernanceStore
        return GovernanceStore(config['governance_db']).revision()
    return workbook_mtime(config['workbook'])


def save_registry(wb, config, expected_revision):
    if config.get('governance_db'):
        from system1.governance_store import GovernanceStore
        return GovernanceStore(config['governance_db']).save(wb, config, expected_revision, getattr(wb, '_governance_actor', 'system1.local'))
    return save_workbook_atomic(wb, config['workbook'], expected_revision)


def backup_registry(config, now):
    if config.get('governance_db'):
        from system1.governance_store import GovernanceStore
        import uuid
        return GovernanceStore(config['governance_db']).backup(config['backup_root']/f'governance_{now:%Y%m%d_%H%M%S}_{uuid.uuid4().hex[:8]}.sqlite')
    return create_backup(config['workbook'], config['backup_root'], now)


@contextmanager
def registry_lock(config, stale_hours=12):
    if config.get('governance_db'):
        from system1.workbook_guard import exclusive_process_lock
        with exclusive_process_lock(config['governance_db'].with_suffix('.write.lock'), stale_hours):
            yield
    else:
        with updater_lock(config['workbook'], stale_hours):
            yield
...
def save_workbook_atomic(wb, workbook_path: Path, expected_mtime_ns: int) -> int:
    ...
    try:
        ...
        from system1.source_assessment import sync_confidence_sheet
        sync_confidence_sheet(wb, workbook_path.parent / "Code/runtime/logs/source-assessments.sqlite")
        from system1.formula_cache import WorkbookCalculator, write_formula_caches
        calculator = WorkbookCalculator(wb)
        ...
        write_formula_caches(temporary_path, wb, caches, calculator)
        ...
        if excel_appears_open(workbook_path):
            raise UpdaterError("STORAGE", "Excel opened during formula calculation; the saved copy was not promoted.")
        try:
...
def update_one_source(
    ...
    if (
        ...
        mtime_ns = save_registry(wb, config, mtime_ns)
    ...
    try:
        with tempfile.TemporaryDirectory(prefix=f"source-{source_id}-") as temp_name:
            ...
            mtime_ns = save_registry(wb, config, mtime_ns)
            if has_current and current_path != new_path and not config.get('governance_db'):
...
def run_updates(
    ...
    workbook_path: Path = config["workbook"]
    if not config.get("governance_db") and not workbook_path.is_file():
        raise UpdaterError("CONFIG", f"Excel workbook not found: {workbook_path}")
    with registry_lock(config, float(config["lock_stale_hours"])):
        wb = open_registry(config)
        if config["sheet_name"] not in wb.sheetnames:
        ...
        if not candidates:
            ...
            return 0
        backup = backup_registry(config, local_now(config))
        append_log(config, {"event": "RUN_START", "sources": len(candidates), "backup": str(backup)})
        mtime_ns = registry_revision(config)
