...
GUIDE_MARKER = "system1-browser-guide-v2"
...
INSTRUCTION_ROWS = {
    5: ("1. Open", "Project-root launcher", "Save and close Excel, then double-click Open Workbench (Windows).cmd on Windows or Open Workbench (macOS).command on macOS in the project root."),
    ...
    8: ("4. Record", "Submit decision", "Use Submit decision after choosing the requested ratings. Other tasks have their own save button. Do not run a program after each review."),
    ...
    12: ("Instructions", "Read-only guidance", "This guide points to the browser workflow. Full instructions: USER_GUIDE.md in the project root."),
    ...
    25: ("Source assessment", "Rate five dimensions", "Click H / M / L for each requested dimension. Light fill is the previous score; dark fill is your current choice. Use Submit decision to apply the selected ratings."),
    ...
    46: ("Project documentation", "One entry point", "Start with the project-root README.md. Agent rules are in the root AGENTS.md; daily source review is explained in USER_GUIDE.md."),
}
...
def prepare_browser_workbook(workbook):
    ...
    if getattr(workbook, '_governance_revision', None) is not None:
        # A migrated guide must not retain obsolete rating-click submission text.
        for row, values in INSTRUCTION_ROWS.items():
            for col, value in enumerate(values,1):workbook['Instructions'].cell(row,col).value=value
        changes = {
            5: ('1. Open','Project-root launcher', 'Double-click Open Workbench (Windows).cmd on Windows or Open Workbench (macOS).command on macOS to review sources. Excel does not need to be closed to save a decision.'),
            14: ('Source Register','Generated source view','The System1 database owns source identity, versions, selection and decisions. This Excel snapshot is for viewing and delivery only; edits are never imported.'),
            16: ('Human Operation Desktop','Generated audit history; hidden','The database retains authoritative operations and history. This hidden worksheet is a generated historical view, not a submission surface.'),
            20: ('Saved / Excel pending','Separate states','A saved database decision can be awaiting Excel synchronization. Pending, synchronizing, synchronized and failed states are shown in the workbench.'),
            38: ('System1 weekly QA','Service-owned schedule','When enabled, every Monday System1 samples 5 source governance records. Other stages show their own implemented sampling scope in the workbench.'),
            44: ('Excel is open','Close it for synchronization','Decisions remain saved in the database. Close and reopen Excel after the background retry succeeds to view the latest snapshot.'),
        }
        for row, values in changes.items():
            for col, value in enumerate(values,1): workbook['Instructions'].cell(row,col).value=value
        workbook['Instructions']['C5'].value += f' Snapshot revision {workbook._governance_revision}.'
