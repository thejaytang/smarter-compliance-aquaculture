...
import uuid
from local_workbench.collaboration import source_review, validate_source_review, fingerprint, encoded, now, named
from backend.shared.collaboration_exchange import segment
from local_workbench.snapshot_graph import ancestors
...
def material_ids(adapter):
 ...
 if not path.exists() and not (path.parent/'.storage.json').is_file():return []
...
class SnapshotWorkspace:
 ...
 def mark_observed(self,key,head,actor):
  if key.startswith('requirements:'):
   from backend.shared.identities import REVIEWERS
   from backend.system3.requirement_delivery import key_for
