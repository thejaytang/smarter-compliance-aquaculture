...
from local_workbench.collaboration import COORDINATOR, named
...
class MaterialInspectionSchedule:
    ...
    def run_due(self, settings=None, clock=None):
        ...
        if set(settings) - {'enabled', 'timezone', 'sample_size', 'assignee', 'interval_days', 'anchor_at', 'next_run_at'}:
        ...
        if 'interval_days' in settings:
            ...
            if type(days) is not int or not 1 <= days <= 365: raise ValueError('Invalid inspection interval.')
            due = datetime.fromisoformat(settings['next_run_at'])
            anchor = datetime.fromisoformat(settings['anchor_at'])
            if due.tzinfo is None or anchor.tzinfo is None: raise ValueError('Schedule timestamps need a time zone.')
            if instant < due: return {'status':'scheduled','enabled':True,'next_run_at':due.isoformat()}
            period = int((instant-anchor).total_seconds() // (days*86400))
            batch_id = 'material-interval-' + sha256((anchor.isoformat()+':'+str(days)).encode()).hexdigest()[:16] + '-' + str(period)
