...
def staged(app,actor,uid):
    uid=str(uuid.UUID(uid));root=(getattr(app,'uploads',app.runtime/'uploads')).resolve();meta=json.loads((root/(uid+'.json')).read_text())
...
def inspect_source(app,actor,request):
    ...
    if uid:
        ...
        uid=str(uuid.uuid4());root=getattr(app,'uploads',app.runtime/'uploads');root.mkdir(exist_ok=True);path=root/(uid+suffix);path.write_bytes(raw)
