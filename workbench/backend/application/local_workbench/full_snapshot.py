...
import uuid
from local_workbench.collaboration import encoded, fingerprint, named, now
from backend.shared.collaboration_exchange import pack, unpack
from local_workbench.snapshot_graph import node, tips, preview, validate_graph
...
class FullSnapshot:
 ...
 def export(self,actor,request):
  ...
  with self.c.lock:
   ...
   if getattr(self.c,'layout',None):
    from local_workbench.workspace_package import capture
    data,manifest=capture(self.c,actor,rid)
    self.c._archive(rid,data)
    result={'id':rid,'filename':'workbench-collaboration-'+rid[:8]+'.zip','bytes':len(data),
      'records':len(manifest['bindings']),'originals':len(manifest['originals']),'versions':len(manifest['bindings']),
      'history_count':sum(x['tables'].get('history',0) for x in manifest['databases'].values()),'actor':actor,'at':manifest['at']}
 ...
 def download(self,actor,identity):
  ...
  return (self.c.packages/(identity+'.zip')).read_bytes(),info['filename']
 def load(self,raw):
  from local_workbench.workspace_package import is_bundle, validate
  business_raw=raw if is_bundle(raw) else None
  if business_raw:raw,_=validate(raw)
  ...
  if business_raw:package['business_archive']=business_raw
  return package
 ...
 def prepare(self,actor,request):
  ...
  with self.c.lock:
   ...
   package=self.load((self.c.packages/(identity+'.zip')).read_bytes());m=package['metadata']
 ...
 def apply(self,actor,request):
  ...
  with self.c.lock:
   ...
   if package.get('business_archive'):
    from local_workbench.workspace_package import retain_history
    retain_history(self.c,package['business_archive'])
