...
from local_workbench.collaboration import named, fingerprint, now, validate_source_review, COORDINATOR
...
class SourceWorkflow:
  ...
  with self.c.lock:
   ...
   if not apply:
    ...
    if request['action']=='manual':
     uid=str(uuid.UUID(request['upload_id']));root=getattr(self.c.app,'uploads',self.c.app.runtime/'uploads');metadata=json.loads((root/(uid+'.json')).read_text())
 ...
 def intake(self,actor,request):
  self.c.coordinator(actor)
  from local_workbench.source_intake import INSPECTION_FIELDS, verified_inspection
  ...
  with self.c.lock:
   ...
   if not payload:
    ...
    uid=inspected['upload_id'];root=getattr(self.c.app,'uploads',self.c.app.runtime/'uploads');metadata=json.loads((root/(uid+'.json')).read_text())
    payload.update(upload_path=metadata['path'],upload_hash=metadata['hash'],upload_root=str(root))
