...
import sqlite3
from backend.shared.sqlite_support import connect as connect_sqlite, operational_root
import time
...
def read_status(runtime, worker=None):
    ...
    try:
        marker = operational_root(runtime) / 'workbook.json'
        if marker.is_file():
        ...
        source=json.loads((operational_root(runtime)/'source-version.json').read_text())
