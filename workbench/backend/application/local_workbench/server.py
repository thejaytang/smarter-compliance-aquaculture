...
from backend.shared.sqlite_support import operational_root
from local_workbench.runtime_identity import describe as runtime_evidence
from local_workbench.original_stream import stream_original
from local_workbench.collaboration import COORDINATOR
...
class Handler(BaseHTTPRequestHandler):
    ...
    def saved_collection_download(self, actor, value):
        ...
        data = (self.app.collaboration.packages / (identity + '.zip')).read_bytes()
    ...
    def do_GET(self):
        ...
        try:
            ...
            if parsed.path=='/api/delivery/download':
                from local_workbench.workspace_package import DeliveryPackage
                raw,filename=DeliveryPackage(self.app.collaboration).download(self.current_session()['name'],parse_qs(parsed.query).get('id',[''])[0])
            ...
            if parsed.path == '/api/sources/check-updates':
                session=self.current_session(create=False)
                from local_workbench.source_checks import get_check
            ...
            if parsed.path == '/api/settings/site-catalog':
                from backend.system3.site_catalog import Catalog
            ...
            if parsed.path == '/api/settings/ai':
                from backend.shared.ai_settings import AISettings
                self.current_session()
            ...
            if parsed.path in {'/api/interpretations', '/api/interpretations/run', '/api/interpretations/trace', '/api/interpretations/impacts', '/api/requirement-annotations'}:
                from backend.system3.interpretations import Interpretations, annotations
            ...
            if parsed.path=="/api/state":
                ...
                from backend.shared.qa import with_sampling
            ...
            if parsed.path == '/api/system2/workbook' and parse_qs(parsed.query).get('cached')==['true']:
                marker=operational_root(self.app.system2.runtime)/'workbook.json'
            ...
            if parsed.path in {'/pdf-reader.html', '/pdf-reader.js', '/pdf-reader.css'}:
                asset = self.app.static_path(parsed.path.lstrip('/'))
            ...
            static={"/":"index.html","/app.js":"app.js","/style.css":"style.css", "/package-download.js":"package-download.js",
                    ...
                    "/demo-assets/review-sample.pdf":"demo-assets/review-sample.pdf",
                    "/demo-assets/sample-page-1.webp":"demo-assets/sample-page-1.webp",
                    "/demo-assets/sample-page-2.webp":"demo-assets/sample-page-2.webp"}
            if parsed.path.startswith(('/components/','/assets/')):
                parsed=parsed._replace(path='/' + parsed.path.split('/',2)[2])
    ...
    def do_POST(self):
        ...
        with getattr(self.app,'operation_lock',nullcontext()):return self._do_POST()
    ...
    def _do_POST(self):
        ...
        try:
            ...
            limit = 512 * 1024 * 1024 if self.path in {'/api/collaboration/import','/api/sync/import'} else 55_000_000
            ...
            if self.path=="/api/upload":
                ...
                root=self.app.uploads;root.mkdir(exist_ok=True)
            ...
            if self.path == '/api/requirements/step':
                ...
                return self.send(409 if result.get('status') == 'conflict' else 200, result)
            if self.path=='/api/delivery/export':
                from local_workbench.workspace_package import DeliveryPackage
                return self.send(200,DeliveryPackage(self.app.collaboration).export(session['name'],body))
            ...
            if self.path == '/api/sources/check-updates':
                from local_workbench.source_checks import start_check
                return self.send(200,start_check(self.app,session['name'],body))
            if self.path == '/api/sources/inspect':
                from local_workbench.source_intake import inspect_source
