...
DEFAULT_DATABASE = Path(__file__).resolve().parents[5] / "runtime" / "state" / "system2-jobs.sqlite3"


class _DeferredJobStore:
    """Keep the ASGI import read-only; initialize only on a store operation."""
...
def create_app(database: str | Path = DEFAULT_DATABASE, ui_dir: str | Path | None = None, *, defer_store: bool = False) -> FastAPI:
    store = _DeferredJobStore(database) if defer_store else JobStore(database)
...
app = create_app(defer_store=True)
