...
def apply_repeating_marginals(
    ...
    for annotation_index, annotation in enumerate(annotations):
        ...
        # Repeated table text (for example Site name) is still cell content.
        # Taking it out as a running header shrinks the compound to remaining
        # glyphs, destroys its border and gives those words a second owner.
        if any(region.label == "table"
               and region.bbox.x0 <= box.x0 < box.x1 <= region.bbox.x1
               and region.bbox.y0 <= box.y0 < box.y1 <= region.bbox.y1
