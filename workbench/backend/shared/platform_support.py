...
def python_path(*source_roots):
    """Explicit import roots for this checkout and its component subprocesses."""
    workbench = Path(__file__).resolve().parents[2]
    return os.pathsep.join(map(str, (*source_roots, workbench, workbench / 'backend/application')))


def system1_code(root):
    """Current source layout, with an explicit old-installation fallback."""
    root = Path(root).resolve()
    return root if (root / 'src/system1').is_dir() else root / 'Code'
