...
const {newSession,restoreSession,reviewCounts,nextPending,recordReview}=await importFile('../frontend/components/system2-review-state.js');
const {demoCases,pdfSource}=await importFile('../frontend/components/system2-demo-data.js');
...
test('five-case walkthrough leaves the missing table open and never manufactures live QA',async()=>{
 let s=newSession();
 ...
 assert.equal(pdfSource.synthetic,true);
 assert.equal(pdfSource.sourceHash,(await import('node:crypto')).createHash('sha256').update(await readFile(new URL('../frontend/assets/demo-assets/review-sample.pdf',import.meta.url))).digest('hex'));
 assert.equal(text.page+1,1);assert.equal(join.page+1,2);
});
