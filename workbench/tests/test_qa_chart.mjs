...
import {weeklyChart} from '../frontend/components/qa-chart.js';
...
test('inactive reserved stage is omitted, while actual legacy observations remain visible',()=>{
 const data=fixture();data.series[2].points=data.series[2].points.map(p=>({...p,accuracy:null,reviewed:0,sampled:0,status:'not_connected'}));
 const html=weeklyChart(data);assert.equal((html.match(/<path d=/g)||[]).length,2);assert.doesNotMatch(html,/system3|System3|Legacy reserved-stage records/);
 const historical=weeklyChart(fixture());assert.match(historical,/Legacy reserved-stage records/);assert.equal((historical.match(/<path d=/g)||[]).length,3);
});
