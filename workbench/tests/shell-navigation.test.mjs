...
import {createNavigation, workspaceFor, workspaceModules, guardedNavigation} from '../frontend/components/shell-navigation.js';
...
test('source subpages belong to Sources and retain their selected page across workspace changes',()=>{
 const store=memory(),nav=createNavigation(store);nav.commit('sourceUpload');
 assert.equal(workspaceFor('sourceUpload'),'sources');assert.equal(workspaceFor('sourceDiscovery'),'sources');
 nav.commit('system2');assert.equal(nav.forWorkspace('sources'),'sourceUpload');nav.commit('sourceDiscovery');assert.equal(createNavigation(store).view,'sourceDiscovery');
});
