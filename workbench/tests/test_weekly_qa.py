...
from backend.shared.qa import weekly_dashboard, with_sampling
from backend.shared.identities import canonical_name, REVIEWERS
...
class WeeklyDashboardTests(unittest.TestCase):
    ...
    def test_five_weeks_calendar_gaps_and_smaller_sample(self):
        ...
        self.assertIsNone(chart['series'][0]['points'][-1]['accuracy'])
        self.assertEqual(chart['series'][0]['points'][-1]['shortfall'],2)
    ...
    def test_reported_source_problem_keeps_batch_incomplete(self):
        snapshot=self.snapshot(['CORRECT']*4+['INCORRECT'])
        snapshot['tasks']=[{'operation_id':'followup','payload_json':json.dumps({'human_reported_issue':{'evidence_id':'4'}})}]
        point=self.chart(snapshot)['series'][0]['points'][-1]
        self.assertIsNone(point['accuracy']);self.assertEqual(point['open_findings'],1)
    def test_four_stage_targets_and_missing_b_stratum(self):
        sampling={'schedule':{'enabled':True},'batches':[{'week':'2026-09-07','stage':'B','sampled':1,
            'shortfall':4,'complete':False,'agreement':None,'missing_strata':['non_requirement'],
            'items':[{'status':'complete','verdict':'INCORRECT'}]}]}
        chart=with_sampling(self.chart(self.snapshot([])),sampling)
        self.assertEqual(chart['targets'],{'system1':5,'system2a':20,'system2b':5,'system3':0})
        p=chart['series'][2]['points'][-1]
        self.assertIsNone(p['accuracy']);self.assertEqual(p['shortfall'],4)
