...
class WorkbenchBridgeTests(OperatorJourneyTests):
    ...
    def test_defer_keeps_unresolved_source_in_pending_across_routine_and_replay(self):
        task = self.task()
        request = self.request(task, action='defer', note='Authorisation remains unresolved.')
        self.assertEqual(bridge.apply(self.config_path, request)['status'], 'applied')
        for _ in range(2):
            snapshot = bridge.read(self.config_path)
            pending = next(t for t in snapshot['tasks'] if t['source_id']=='PA001')
            self.assertEqual(pending['program_status'], 'WAITING_FOR_HUMAN')
            self.assertEqual(pending['source']['effective_selection'], 'PENDING')
            self.assertIn(request['request_id'], h.parse_payload(pending['payload_json'])['workbench_receipts'])
            self.assertEqual(bridge.apply(self.config_path, request)['status'], 'applied')
            with contextlib.redirect_stdout(io.StringIO()): run_cycle(self.config_path, False)
        self.assertEqual(bridge.apply(self.config_path, self.request(pending))['status'], 'applied')
        self.assertFalse(any(t['source_id']=='PA001' for t in bridge.read(self.config_path)['tasks']))

    def test_full_ratings_cannot_close_missing_authorised_original(self):
        self.task()
        cfg = u.read_config(self.config_path);wb=u.open_registry(cfg);ws=wb[cfg['sheet_name']];hs=u.workbook_headers(ws,2)
        for key,value in {'snapshot_status':'NOT_COLLECTED','download_status':'PAYWALL_BLOCKED',
                          'failure_reason':'An authorised original is required.'}.items():ws.cell(3,hs[key]).value=value
        u.save_registry(wb,cfg,u.registry_revision(cfg));wb.close()
        with contextlib.redirect_stdout(io.StringIO()):run_cycle(self.config_path,False)
        task=next(t for t in bridge.read(self.config_path)['tasks'] if t['source_id']=='PA001')
        self.assertEqual(bridge.apply(self.config_path,self.request(task))['status'],'applied')
        for _ in range(2):
            snapshot=bridge.read(self.config_path)
            self.assertTrue(any(t['source_id']=='PA001' for t in snapshot['tasks']))
            self.assertEqual(next(s for s in snapshot['sources'] if s['source_id']=='PA001')['effective_selection'],'PENDING')
            with contextlib.redirect_stdout(io.StringIO()):run_cycle(self.config_path,False)
