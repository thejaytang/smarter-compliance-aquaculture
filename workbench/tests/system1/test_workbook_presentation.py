...
class BrowserWorkbookPresentationTest(unittest.TestCase):
    def test_controlled_save_preserves_records_and_retires_legacy_surfaces(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            book.properties.keywords = "system1-browser-workbench;system1-browser-guide-v1"
            ...
            for attempt in range(2):
                ...
                self.assertEqual([s.title for s in book if s.sheet_state == "visible"],
                                 ["Instructions", "Categories", "Source Register", "Machine confidence"])
                ...
                self.assertIn("USER_GUIDE.md in the project root", book["Instructions"]["C12"].value)
                self.assertIn("Open Workbench (Windows).cmd on Windows", book["Instructions"]["C5"].value)
