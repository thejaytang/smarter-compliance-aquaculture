...
def initial_seed(root):
    ...
    for name in ('config', 'schedule'):
        shutil.copy2(ROOT / 'workbench/config/system1' / (name + '.example.json'), config / (name + '.example.json'))
    for name in ('config','schedule'):
        value=json.loads((config/(name+'.example.json')).read_text())
        if name=='config':value['governance_db']='../runtime/governance.sqlite';value['random_qa']['enabled']=False
        else:value['enabled']=False
        (config/(name+'.json')).write_text(json.dumps(value))
    book = source / 'system1/Requirement_Source_Registry.xlsx'
    shutil.copy2(ROOT / 'workbench/tests/system1/fixtures/source_registry.xlsx', book)
...
'''
    run([python('workbench/backend/system1'), '-c', script, source, sha256(original.read_bytes()).hexdigest()],
        env={**os.environ, 'PYTHONPATH': os.pathsep.join((str(ROOT / 'workbench/backend/system1/src'),str(ROOT / 'workbench'))), 'PYTHONUTF8': '1'})
...
class Peer:
    def __init__(self, root, actor):
        ...
        self.state_file = root / 'workbench/runtime/state/server.json'
        self.csrf = self.cookie = ''
        self.port = None

    def launch(self):
        args = ['start','--no-browser','--root',str(self.root/'workbench')]
        if os.name=='nt':command=['cmd.exe','/d','/c','call',str(ROOT/'Open Workbench (Windows).cmd'),*args]
        else:command=[python('workbench'),str(ROOT/'deployment.py'),*args]
        run(command)
...
def exercise(root):
    ...
    for rel in ('workbench/config/system1/config.example.json','workbench/config/system1/schedule.example.json'):
    ...
    git(producer, 'add', '--', '.gitignore','workbench/config/system1','application-version.txt'); git(producer, 'commit', '-m', 'Initial application'); git(producer, 'push', 'origin', 'main')
    ...
    try:
        ...
        git(producer, 'add', '--', '.gitignore','workbench/config/system1','application-version.txt'); git(producer, 'commit', '-m', 'Application update'); git(producer, 'push', 'origin', 'main')
