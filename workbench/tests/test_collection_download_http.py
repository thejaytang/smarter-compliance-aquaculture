...
class CollectionDownloadHTTPTests(unittest.TestCase):
    ...
    def test_real_receipt_export_returns_canonical_recovery_url_and_reuses_saved_package(self):
        ...
        self.c.put('adoption_receipt', receipt, {'id': receipt, 'status': 'adopted',
                   'material': {'revision': 4, 'source': {'source_id': 'TS001'}, 'content_status': 'draft',
                                'history': [{'revision': 3, 'content_status': 'confirmed'}]}})
        receipts_before = self.c.all('adoption_receipt')
        summary = 'Checked § 13a and table 2.\nRemaining: page 9 needs original review. 保留说明。'
        ...
        self.assertEqual(unpack(raw)['metadata']['summary'], summary)
        self.assertEqual(self.c.get('sent_collection', identity)['summary'], summary)
        self.assertEqual(self.c.all('adoption_receipt'), receipts_before)
        before = self.snapshot()
    ...
    def test_receipt_summary_default_and_existing_length_convention(self):
        receipt = str(uuid.uuid4())
        self.c.put('adoption_receipt', receipt, {'id': receipt, 'status': 'adopted'})
        for fields, expected in (({}, 'Explicit per-item adoption receipts'),
                                 ({'summary': ''}, ''), ({'summary': 'x' * 4001}, 'x' * 4000)):
            with self.subTest(fields=fields.keys(), expected_length=len(expected)):
                body = {'request_id': str(uuid.uuid4()), 'kind': 'receipt',
                        'items': [{'type': 'receipt', 'key': receipt}], **fields}
                status, raw, _ = self.request('/api/collaboration/collection-export', 'POST', body)
                self.assertEqual(status, 200, raw)
                self.assertEqual(unpack(raw)['metadata']['summary'], expected)

    def test_legacy_frozen_receipt_replay_never_rewrites_its_lost_summary_or_history(self):
        identity = str(uuid.uuid4()); receipt = str(uuid.uuid4())
        body = {'request_id': identity, 'kind': 'receipt', 'summary': 'Previously entered, not retained',
                'items': [{'type': 'receipt', 'key': receipt}]}
        # Reproduce a pre-fix artifact and its exact original request binding.
        legacy = {'id': identity, 'actor': 'Weijie Tang', 'direction': 'receipt',
                  'collection_version': 2, 'summary': 'Explicit per-item adoption receipts', 'items': []}
        raw = pack('collection', legacy, {})
        self.c.claim_request('receipt-export', identity, fingerprint(body), 'Weijie Tang')
        self.c._archive(identity, raw); self.c.put('sent_collection', identity, legacy)
        before = self.snapshot()
        status, replayed, _ = self.request('/api/collaboration/collection-export', 'POST', body)
        self.assertEqual((status, replayed), (200, raw))
        self.assertEqual(unpack(replayed)['metadata']['summary'], 'Explicit per-item adoption receipts')
        self.assertEqual(self.snapshot(), before)
        changed = dict(body, summary='Changing the note must not replace a frozen package')
        self.assertEqual(self.request('/api/collaboration/collection-export', 'POST', changed)[0], 400)
        self.assertEqual(self.snapshot(), before)
