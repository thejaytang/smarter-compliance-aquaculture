...
def test_import_and_health_do_not_create_a_database(tmp_path):
    from system2.support.paths import PROJECT_ROOT
    ...
    result=subprocess.run([sys.executable,'-c',code],cwd=tmp_path,capture_output=True,text=True,
                          env=__import__('os').environ|{'PYTHONPATH':__import__('os').pathsep.join((str(PROJECT_ROOT/'src'),str(PROJECT_ROOT.parents[1]))),'PYTHONDONTWRITEBYTECODE':'1'},timeout=30)
...
def test_default_belongs_to_system2_and_explicit_database_stays_authoritative(tmp_path,monkeypatch):
    ...
    from system2.support.paths import PROJECT_ROOT
    assert module.DEFAULT_DATABASE==PROJECT_ROOT.parents[1]/'runtime/state/system2-jobs.sqlite3'
