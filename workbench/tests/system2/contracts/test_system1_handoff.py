...
from system2.contracts.test_source_intake import source, row, store
...
def test_database_handoff_ignores_excel_and_guards_logical_versions(tmp_path,monkeypatch):
    root,registry,reply,_=fixture(tmp_path,monkeypatch)
    config=root/'Code/config/config.json'
    ...
    assert handoff.evidence['registry_kind']=='sqlite_snapshot'
    assert handoff.records[0]['registry_kind']=='sqlite_snapshot'
    assert len(build_manifest(handoff.records,handoff.registry_sha256,handoff.source_root,{'PA001'})['items'])==1
    registry.unlink()
    handoff.assert_current()
    version['revision']=2
    with pytest.raises(IntakeError,match='changed_during_handoff'):handoff.assert_current()


def test_open_source_issues_include_new_material_reports_without_legacy_id(tmp_path,monkeypatch):
    root,registry,reply,_=fixture(tmp_path,monkeypatch)
    before=registry.read_bytes()
    reply['data']['tasks']=[
        {'operation_id':'native','source_id':'PA001','is_open':True,'human_issue':{'reason':'Original appendix missing','related_reports':[
            {'reason':'New material image truncated','material_id':'new-material'},
            {'reason':'Old legacy report','system2_document_id':'old-document'},
            {'reason':'Resolved related report','resolved':True}]}},
        {'operation_id':'closed','source_id':'PA001','is_open':False,'human_issue':{'reason':'Already closed'}},
        {'operation_id':'empty','source_id':'PA001','is_open':True,'human_issue':{'related_reports':[]}},
    ]
    handoff=system1.read_system1(root)
    assert [i['reason'] for i in handoff.evidence['source_open_issues']]==[
        'Original appendix missing','New material image truncated','Old legacy report']
    assert all(i['source_id']=='PA001' and i['task_id']=='native' for i in handoff.evidence['source_open_issues'])
    assert [i['system2_document_id'] for i in handoff.evidence['original_issues']]==['old-document']
