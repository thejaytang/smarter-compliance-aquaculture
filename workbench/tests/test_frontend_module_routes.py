...
class FrontendModuleRouteTests(unittest.TestCase):
    def test_app_import_graph_is_served_as_javascript_with_exact_source_bytes(self):
        ui = Path(__file__).resolve().parents[1] / 'frontend'
        server = ThreadingHTTPServer(('127.0.0.1', 0), Handler)
        server.app = SimpleNamespace(ui_root=ui, reviewer=False, read_only_restored=True)
        server.app.static_path=lambda name:Application.static_path(server.app,name)
        ...
        try:
            while pending:
                ...
                try:
                    ...
                    self.assertEqual(data, server.app.static_path(path.removeprefix('/assets/').lstrip('/')).read_bytes(), path)
                finally:
                    connection.close()
                visited.add(path)
                pending.extend(urljoin('http://localhost'+path, target).removeprefix('http://localhost') for target in imports.findall(data.decode('utf-8')))
            ...
            self.assertIn('/export-status.js', visited)
            self.assertNotIn('/package-download.js', visited,
                             'Startup must retain the pre-update parent static routes; the helper lives in export-status.js.')
        finally:
