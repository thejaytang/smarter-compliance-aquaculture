...
from backend.shared import platform_support as platform
...
'''
            env = dict(os.environ, PYTHONPATH=platform.python_path())
            def attempt():
    ...
    def test_launcher_uses_actual_source_for_an_isolated_reviewer(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(popen.call_args.kwargs['env']['PYTHONPATH'], os.pathsep.join((str(Path(launcher.__file__).resolve().parents[3]),str(Path(launcher.__file__).resolve().parents[1]))))
    ...
    def test_launcher_does_not_swallow_service_io_failure_or_reuse_wrong_mode(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary); (root/'runtime/state').mkdir(parents=True)
            (root/'runtime/state/server.json').write_text(json.dumps({'port':1,'instance':'fixture'}))
