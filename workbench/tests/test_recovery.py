...
class ModernRecoveryBoundaryTests(unittest.TestCase):
    def test_backup_uses_current_worker_locks_and_never_recreates_old_root(self):
        from unittest.mock import patch
        from backend.shared.workspace import Workspace, DATABASES
        from local_workbench.recovery import layout, quiescent
        with tempfile.TemporaryDirectory() as folder:
            root=Path(folder);w=Workspace(root/'workbench')
            (w.runtime/'state/layout.json').write_text('{"schema":"workbench-layout/1"}')
            w.source_config.parent.mkdir(parents=True)
            w.source_config.write_text(json.dumps({'workbook':'../../cache/source.xlsx','log_root':'../../state/system1'}))
            for path in [w.journal,*[w.database(name) for name in DATABASES]]:
                with closing(sqlite3.connect(path)) as db:db.execute('CREATE TABLE saved(id INTEGER)')
            personal=w.workspace/'sources/collaboration/personal/reviewer'
            personal.mkdir(parents=True)
            # A stale marker must not replace the state-directory Path with a dict.
            (w.runtime/'state/server.json').write_text('{"pid":123456789}')
            info=layout(root)
            with patch('local_workbench.recovery.process_alive',return_value=False):
                for worker in [w.runtime/'state/system1/.system1-run.lock',w.materials/'.worker.lock',personal/'.material-worker.lock']:
                    with exclusive_lock(worker):
                        with self.assertRaisesRegex(ValueError,'writer is active'):
                            with quiescent(root,info):self.fail('An active worker was ignored')
                with quiescent(root,info):
                    self.assertFalse((root/'system2').exists())
            self.assertFalse((root/'system2').exists())
