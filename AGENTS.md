...
The subsequent 2026-09-16 unified-group revision supersedes terminal grammatical fields and separate exception editing. Preserve source-bound clause containers, same-role recursive field groups, exact/range QC and independent group NOT. A clause binds fields; never count mixed Subject/verb/Object fields as alternatives. The subsequent user clarification keeps Exception as an independent complete-clause/reference relationship, not an automatic negated Condition. New NOT applies only to Conditions. Display one Rx per source entry and local children as Groups; only its arrow toggles disclosure, and only one entry opens at a time. Canonical trees must flow through relational links, source projections, interpretation fingerprints and versioned delivery without flattening. [Current group contract](workbench/docs/manual-requirement-splitting.md#unified-source-bound-groups-current).
