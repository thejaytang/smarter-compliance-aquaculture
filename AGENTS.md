...
This is the single Agent entry point, including for local VS Code Copilot. Read `README.md`, `USER_GUIDE.md`, `ENVIRONMENT.md` and the relevant contract under `workbench/contracts/`. `PROJECT_STATE.md`, `project-support/` and tests are versioned on Jay’s development branch; they are not required by the main product checkout. Maintained project documentation is English; preserve original source language and identifiers. The bilingual public introduction under `.github/` links to these authoritative guides and does not replace them.
...
- `main` carries application code/resources, dependency declarations/locks, templates, guides and only the permanent PE002 Example with its saved work. `developing` additionally carries complete authorized business snapshots, development plans, designs, tests and validation evidence. Active databases are shared through consistent snapshots. Private authentication configuration, sessions, environments and cache stay local; `workspace/` and `runtime/` carry only README files in Git.
- The repository retains only `main` and `developing`. Both branches are public, not access-restricted to Jay. Agent guides retained inside project-support are historical evidence; this root remains the current entry point. Transfer selected product changes to `main`; never merge development-only documents, data policies or records wholesale into `main`.
- On 2026-09-22 the user explicitly chose this same public repository's development branch for complete content. This supersedes the earlier permission to include the full seed on `main`: dated immutable packages under `workbench/initial-data/` now belong only to development. They contain the four business stores, originals and human history, including confidential business content. Credentials and runtime state stay local. Never restore a seed over existing work or during application updates. Cleaning development Materials was a previous one-time user action, never a startup/migration policy.
...
- The current Example is the full original Norwegian law, PE002-001, with saved content, Requirements and interpretations pending human review. The 2026-09-22 Example-only seed and scoped Collaboration package under `workbench/resources/examples/` are the only business-data exception for `main`. Keep their exact audited names and checksums in the boundary checker. The earlier PE001 excerpt package remains development/history only. Original HTML bytes are immutable; an update never automatically replaces local sources or saved work. Fresh seed restoration and existing-workspace Collaboration import remain explicit and preserve unrelated work.
