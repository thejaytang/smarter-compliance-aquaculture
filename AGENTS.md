...
- The user authorised distributing the three-excerpt English training HTML on 2026-09-18. `workbench/resources/examples/` is an authored demonstration resource, not an active workspace snapshot. Updating it never automatically replaces local sources or saved work.
- On 2026-09-18 the user also authorised publishing the saved PE001 example work to `main`: the dated ZIP and checksum under `workbench/resources/examples/` are a scoped Collaboration snapshot. Import remains explicit and must preserve unrelated local work; do not extend this exception to other workspace data.
