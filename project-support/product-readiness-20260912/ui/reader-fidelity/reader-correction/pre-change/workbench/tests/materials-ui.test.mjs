...
test('PDF reader presents a bound page image without requiring a native PDF plugin',()=>{
  const x=instance(),html=x.pdfMarkup({page:2,pages:6,image:'data:image/png;base64,fixture',native_text:'Original <text>',page_warning:'Native text is unverified.'});assert.match(html,/Saved original PDF page 2 of 6/);assert.match(html,/src="data:image\/png;base64,fixture"/);assert.match(html,/separate from the page image/);assert.match(html,/Original &lt;text&gt;/);assert.doesNotMatch(html,/<iframe/);assert.match(html,/Optional browser PDF viewer/);
});
test('scan-only PDF reader does not claim selectable original text or OCR completion',()=>{
  const x=instance(),html=x.pdfMarkup({page:1,pages:1,image:'data:image/png;base64,scan',native_text:''});assert.match(html,/No native text on this page/);assert.match(html,/manually transcribe/);assert.match(html,/No OCR has run/);assert.doesNotMatch(html,/<pre/);
});
...
test('personal confirmation and newer master draft are separately labelled without changing either version',()=>{
 ...
 x.history=true;x.renderList();html=x.q('#mw-material-list').innerHTML;assert.match(html,/Accepted revision 2/);assert.doesNotMatch(html,/Master revision 3/);
});
