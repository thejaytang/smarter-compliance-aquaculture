...
from .material_queue import MaterialQueue, acceptance_key
...
class Collection:
 ...
 def catalogue(self,actor):
  ...
  return {'items':items,'sent':[p for p in self.c.all('sent_collection') if p['actor']==actor],
   'inbox':self.c.all('collection_item'),'receipts':self.c.all('collection_adoption'),'received_receipts':self.c.all('received_adoption'),'assignment_changes':self.c.all('inspection_assignment_pending')}
 ...
 def receipt_catalogue(self,actor):
  ...
  return {'items':[{'type':'receipt','key':p.get('id') or p['request_id'],'title':(p.get('submission_id') or p.get('item_id') or p.get('merge_id'))+' · '+p['status']} for p in values]}
 def receipt_export(self,actor,request):
  ...
  with self.c.lock:
   ...
   if old:return (self.c.root/'packages'/(rid+'.zip')).read_bytes(),'adoption-receipts-'+rid[:8]+'.zip'
   entries=[];files={}
   ...
   metadata={'id':rid,'actor':actor,'at':now(),'collection_version':2,'direction':'receipt','items':entries,'summary':'Explicit per-item adoption receipts'}
 ...
 def export(self,actor,request):
  ...
  with self.c.lock:
   ...
   if prior:return (self.c.root/'packages'/(rid+'.zip')).read_bytes(),'review-'+kind+'-'+rid[:8]+'.zip'
 ...
 def import_collection(self,actor,blob,package):
  ...
  with self.c.lock:
   ...
   for p,raw in decoded:
    path=self.c.root/'packages'/(p['metadata']['id']+'.zip')
