...
class MaterialQueue:
    ...
    def listing(self, actor, bucket, offset=0, limit=50, query=''):
        from .collaboration import named, fingerprint
        actor = named(actor)
        if bucket not in ('pending', 'archive') or type(offset) is not int or offset < 0 or type(limit) is not int or not 1 <= limit <= 50 or not isinstance(query, str) or len(query) > 300:
        ...
        with read_index(self.c.app.system2.runtime) as db:
            ...
            candidates = {}
            for row in db.execute("SELECT material_id,data FROM material_candidate_index WHERE json_extract(data,'$.status') IN ('running','ready','partial')"):
        ...
        for master in current:
            ...
            if workspace and bucket == 'pending':
                with read_index(self.c.workspace_runtime(workspace)) as db:
                    ...
                    personal_candidates = [json.loads(r[0]) for r in db.execute("SELECT data FROM material_candidate_index WHERE material_id=? AND json_extract(data,'$.status') IN ('running','ready','partial')", (identity,))]
            ...
            qualified = bool(archived and master.get('content_status') == 'content_review_complete' and master.get('confirmation') and not master.get('source_stale') and not candidates.get(identity) and master['source']['source_id'] not in issue_sources)
            if bucket == 'archive' and not archived:
                continue
            if bucket == 'pending' and qualified and not (open_tasks or personal_changed or personal_candidates or identity in incoming):
                continue
            ...
            row['queue'] = {'bucket': bucket, 'archive_revision': archived.get('revision') if archived else None,
                ...
                'inspection_only': qualified and bool(open_tasks) and not personal_changed and not personal_candidates and identity not in incoming,
