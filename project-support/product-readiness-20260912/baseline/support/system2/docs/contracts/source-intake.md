...
Calls must specify source IDs explicitly. Process only unique registered records satisfying `selection_status=INCLUDE`, `snapshot_status=STORED`, `source_status=CURRENT` and `download_status=SUCCESS`. In normal intake, `selection_status` is the compatibility name for System1 `effective_selection`. These gates verify agreement between upstream eligibility and the actual snapshot. Named human INCLUDE remains valid. Machine admission instead binds `selection_origin=machine`, an assessment receipt and System1 effective INCLUDE; it never fabricates an operator decision. The assessment store also participates in handoff-version checking.
...
The [two-stage workflow](two-stage-review.md) implements actor-bound conversion tasks and immutable parsing copies. Snapshot adds optional `parsing_copy_of` lineage while preserving the authoritative original. The standalone format parser still reports unsupported originals; the workflow owns conversion registration.
...
The legacy PDF review service remains compatible. The [two-stage workflow](two-stage-review.md) adds Requirement classification, omissions, boundary corrections, field overlays, versioned decisions and receipts in the shared workbench. This parser index itself remains read-only: result → Canonical → locator.
