...
root=Path('/Users/tang/Desktop/smarter-compliance-aquaculture/05_Working area of requirements side');s=root/'system2';out=root/'project-support/product-readiness-20260912/quality/regression-parser6/targeted-recovery-01';out.mkdir(exist_ok=False)
...
paths=[str(s/'tests/integration/test_scheme2.py')+'::test_links_notes_and_real_img2table_route',str(s/'tests/integration/test_scheme3.py')+'::test_rest_api_and_release_gate']
...
result={'command':cmd,'cwd':str(out/'cwd'),'environment_overrides':overrides,'relevant_code_before':before,'scope':'Only two unchanged failed tests after exact installed-RECORD byte restoration of two dependency files. Full suite remains FAIL.'}
