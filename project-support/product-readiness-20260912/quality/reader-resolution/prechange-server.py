...
class Handler(BaseHTTPRequestHandler):
        ...
        try:
            ...
            if parsed.path in {'/api/material-queue', '/api/material-inspection'}:
                ...
                return self.send(200, service.listing(actor, query.get('bucket', ['pending'])[0],
                    offset=int(query.get('offset', ['0'])[0]), limit=int(query.get('limit', ['50'])[0]), query=query.get('query', [''])[0]))
            ...
            if parsed.path in {'/api/material', '/api/material/history', '/api/material/reader', '/api/material/original', '/api/material/html', '/api/material/candidate', '/api/material/conflict'}:
                ...
                if parsed.path == '/api/material/reader':
                    ...
                    return self.send(200, adapter.call('material_reader', material_id=identity, options=options))
            ...
            if parsed.path=="/api/state":
                ...
                return self.send(200,{**public,"actor":{"id":session["id"],"name":session["name"]},
                    ...
                    "collaboration":{"mode":"inspection" if inspection else self.app.collaboration.mode,"coordinator":COORDINATOR,
            ...
            static={"/":"index.html","/app.js":"app.js","/style.css":"style.css",
                    ...
                    "/collaboration.js":"collaboration.js", "/collaboration-relationships.js":"collaboration-relationships.js", "/collaboration.css":"collaboration.css",
                    "/material-editing.js":"material-editing.js", "/source-workspace.js":"source-workspace.js", "/source-workspace.css":"source-workspace.css", "/submission-drawer.js":"submission-drawer.js", "/material-inspection.js":"material-inspection.js", "/material-navigation.js":"material-navigation.js", "/runtime-status.js":"runtime-status.js",
    ...
    def do_POST(self):
        ...
        try:
            ...
            limit = 256 * 1024 * 1024 if self.path == '/api/collaboration/import' else 55_000_000
            ...
            if self.path.startswith('/api/collaboration/'):
                ...
                if action.startswith('collection-'):
                    ...
                    if action in ('collection-export','collection-download'):
                        if action=='collection-export':result,filename=service.export(session['name'],body)
                        else:
                            identity=str(uuid.UUID(body['id']));record=self.app.collaboration.get('sent_collection',identity)
                            if not record or record['actor']!=session['name']:raise ValueError('This saved package belongs to another reviewer.')
                            result=(self.app.collaboration.root/'packages'/(identity+'.zip')).read_bytes();filename='review-'+identity+'.zip'
                        return self.send(200,result,'application/zip',{'Content-Disposition':'attachment; filename="'+filename+'"'})
            ...
            if self.path.startswith('/api/system2/') or self.path in {'/api/system1/assess', '/api/settings'}:
