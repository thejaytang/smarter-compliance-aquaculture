...
VERSION = 'material-structural-parser/4+dev-grid-consistency'
POLICY = {'adapter': VERSION, 'semantic_processing': False, 'external_models': False, 'ocr': False, 'pdf_window_pages': 3, 'pdf_backend': 'pdfium', 'table_grid_guard': 'native-line-grid-consistency/experiment-1', 'table_grid_tolerance_points': 1.0, 'table_grid_reject_ratio': 0.25, 'pdf_marker_order': 'same-row-before-body/1', 'pdf_paragraph_grouping': 'same-page-source-preserving/1', 'human_adoption_required': True}
...
def _pdf_tables(page, original_page, source, reference, output):
    ...
    from grid_guard import inspect_grid
    for number, box in enumerate(detect_table_boxes(image_path, TableSettings(backend='native', recognition_pipeline_enabled=False, img2table_enabled=False, gmft_enabled=False))):
        ...
        consistency = inspect_grid(page, box, xs, ys, specs, bbox)
        inspected.append({'candidate_id': block['id'], **consistency})
    ...
    _write(output / f"pdf-table-grid-review-{reference['page']:04d}.json", {'method':'native-line-grid-consistency/experiment-1','source_sha256':source.content_hash,'page_reference':reference,'rejected':rejected,'inspected':inspected})
        ...
        for offset in range(0, len(selected), 3):
            ...
            for local, page in enumerate(native.pages):
                ...
                with closing(original[index]) as original_page:
                    ...
                    rejected_grids = json.loads((output / grid_evidence).read_text())['rejected']
                    for rejected in rejected_grids:
                        message = f"Page {index+1}: table layout could not be established ({rejected['crossing_line_numerator']}/{rejected['captured_line_denominator']} native lines cross proposed cells). Original positioned text is retained; compare the marked range with the source and reconstruct the table if needed."
                        unresolved.append({'scope_id':scope_id,'code':'table_grid_inconsistent','message':message,'source_refs':rejected['source_refs'],'rejected_candidate_id':rejected['candidate']['id'],'evidence_ref':grid_evidence})
