...
**Current objective, explicitly revised 2026-09-13: finish local delivery of the existing human-review workbench.** This user instruction supersedes conflicting earlier accuracy thresholds, expansion scope and open-ended optimization requirements. The fixed closeout checklist is maintained in the [existing execution entry](../product-readiness-20260912/execution.md#fixed-local-delivery-checklist); [PROJECT_STATE.md](../../PROJECT_STATE.md) owns current delivery/loading status. Earlier sections below retain product contracts and historical context only where compatible with this closeout.

A colleague must be able to find a material, read the original, manually check/correct it, save/resume, compare and explicitly adopt changes, separately confirm review, and exchange work through the interface. Unchecked content and unusable/unreviewed results must remain explicit. Extraction and Requirement recognition are optional assistance. Stop accuracy optimization, parser research, additional evaluation samples, model experiments and training. Retain existing quality defects and UNMEASURED results. Formal third-pane schema, ontology, semantic processing and Site Model remain deferred.

### Fixed local delivery scope

1. Explicit archive-inspection pass, finding resolution and task-result return.
2. Known high-risk branches: delayed repeated operations and returned table conflicts. Finish the observed inspection conflict recovery blocker.
3. Key-task usability with browser zoom, keyboard and reduced motion/transparency, reusing applicable existing viewport/visual evidence.
4. One identified integrated candidate with relevant regression, mixed-operation stability and recovery checks.
5. Runnable version, verified loading state, restoration instructions and concise handoff.

Only admit changes backed by concrete evidence of lost/overwritten input, history or originals; wrong-version application or confused/false save/adopt/confirm states; blocked core UI tasks/recovery; or reading, location, clipping and accessibility failures that materially impede manual checking. Record evidence and impact against the existing checklist before adding a blocker. Other feature, efficiency and decorative improvements stay in existing deferred work. No new dependencies, framework or extra service. Reuse reliable current evidence; do not reopen completed work merely to improve it.

### Freeze, completion and stopping

Preserve existing work/recovery points and freeze one identified candidate promptly. A necessary blocker fix increments the candidate identity; rerun affected checks and necessary integration only. All write, confirmation, conflict, exchange and recovery tests stay isolated. Never perform genuine business review or switch the formal business master/environment, upload, send, publish, push or add automation. Stop repeating a problem after three attempts without new evidence/improvement; record diagnosis and the minimum next step.

Local completion requires every fixed local engineering item to pass, no unresolved human-work-safety or core-flow blocker, and complete handoff. Real colleague usability, Windows physical-machine round trips and formal-environment activation are independent later acceptance, not reasons to keep this local goal running. Do not claim those external gates passed. When the local gate passes, deliver and end this goal without seeking more optimization. If an essential item cannot be completed, state **Local trial gate not reached**, deliver the recoverable candidate and exact blocker/evidence/required decision; recording a gap is not passing it.

The final handoff contains only version/location/opening/loading, each fixed item's PASS/FAIL/UNMEASURED/BLOCKED and key evidence, one complete task example, remaining limits and three colleague checks, and recovery/next entry. Test counts or elapsed time do not substitute for product completion.
