...
This is the single visual agreement for the active workbench, refining the existing light glass shell for the user's material-processing task. It applies UI UX Pro Max accessibility/layout guidance and Impeccable Operate guidance. Product truth is in [PRODUCT.md](PRODUCT.md); acceptance status is in [PROJECT_STATE.md](PROJECT_STATE.md).
...
- Daily comparison and master-view actions remain visible; package exchange and inbox actions use an inline disclosure rather than consuming a permanent toolbar row for every operation.
...
- Narrow views use the existing pane tabs; expanded mode preserves source/content comparison and adjustable widths. Native keyboard alternatives for resizing remain available.
