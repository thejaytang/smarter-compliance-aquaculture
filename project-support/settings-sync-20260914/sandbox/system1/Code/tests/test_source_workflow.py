...
class SourceWorkflowTests(CollaborationReviewTests):
 ...
 def test_rereview_is_new_task_and_only_selected_task_closes(self):
  ...
  req=self.request(selection='INCLUDE',note='The selected source check was completed.',task_checks={selected['operation_id']:selected['revision']})
  self.assertEqual(self.apply(req)['status'],'saved_partial')
 ...
 def test_returned_include_blocks_handoff_until_explicit_task_confirmation(self):
  self.activate();source=self.source();req={'actor':'Weijie Tang','source_id':'PA001','expected_source_revision':source['source_revision'],'request_id':str(uuid.uuid4()),'note':'Recheck after a possible version change'}
  result=reopen(self.config_path,req);data=bridge.read(self.config_path);source=next(s for s in data['sources'] if s['source_id']=='PA001')
  self.assertEqual(source['decision_display'],'Pending-Include');self.assertEqual(source['effective_selection'],'PENDING')
  h.reconcile_resolved_reviews(self.config_path);h.sync_review_queue(self.config_path,[])
  data=bridge.read(self.config_path);task=next(t for t in data['tasks'] if t['operation_id']==result['operation_id'])
  receipt=self.apply(self.request(selection='INCLUDE',note='Current source verified; restore Include.',task_checks={task['operation_id']:task['revision']}))
  self.assertEqual(receipt['effective_selection'],'INCLUDE');self.assertEqual(self.source()['effective_selection'],'INCLUDE')

 def test_basic_inspection_fields_and_unknown_ratings_survive_pending_ledger(self):
  from system1.source_workflow import intake
  import hashlib
  self.activate();root=self.runtime/'uploads';root.mkdir();file=root/'source.html';file.write_text('<h1>Aquaculture law</h1>');identity=hashlib.sha256(file.read_bytes()).hexdigest()
  req={'actor':'Weijie Tang','request_id':str(uuid.uuid4()),'source_title':'Reviewed title','upload_path':str(file),'upload_root':str(root),'upload_hash':identity}
  first=intake(self.config_path,req)
  req.update(request_id=str(uuid.uuid4()),source_title='Generated replacement title',issuer='Generated publisher',jurisdiction='Norway',document_type='Regulation',scope_relevance='HIGH',access_permission='',basic_inspection={'schema':'source-basic-inspection/1','content_hash':identity,'dimensions':{'access_permission':{'rating':'UNKNOWN'}}})
  result=intake(self.config_path,req);self.assertEqual(result['operation_id'],first['operation_id'])
  task=next(t for t in bridge.read(self.config_path)['tasks'] if t['operation_id']==first['operation_id'])
  self.assertEqual(task['candidate_fields']['source_title'],'Reviewed title');self.assertEqual(task['candidate_fields']['issuer'],'Generated publisher');self.assertEqual(task['candidate_fields']['scope_relevance'],'HIGH');self.assertEqual(task['candidate_fields']['access_permission'],'');self.assertEqual(task['basic_inspection']['content_hash'],identity)
