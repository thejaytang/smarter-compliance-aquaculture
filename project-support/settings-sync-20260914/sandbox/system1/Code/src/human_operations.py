...
Humans decide in the browser workbench. ``Human Operation Desktop`` is the
program-owned staging and audit store used by its compatibility adapter. This
module validates staged decisions, updates ``Source Register``, records program
timestamps, and hides completed rows without deleting item-level history.
"""
...
def prepare_human_workbook(wb, ws, headers: dict[str, int]) -> None:
    """Maintain the program-owned audit sheet and legacy reporting caches."""
    if wb.properties.keywords and "system1-browser-workbench" in wb.properties.keywords:
        ws.sheet_state = "veryHidden"
        if wb.active == ws:
            wb.active = wb.sheetnames.index("Instructions") if "Instructions" in wb else 0
    visible = {
    ...
    if "Dashboard" in wb.sheetnames:
        ...
        for letter in ("R", "S", "T", "U", "V"):
            dashboard.column_dimensions[letter].hidden = True
...
def completed_review_history(config_path: Path) -> dict[tuple[str, str], dict[str, Any]]:
    ...
    wb = updater.open_registry(config)
...
def add_candidate_to_source_register(ws, headers: dict[str, int], payload: dict[str, Any], config: dict[str, Any], now: datetime, operator: str, note: str) -> str:
    ...
    if not title or not folder_code or not file_format or (not official_url and not payload.get("intake_file")):
        raise HumanOperationError("Accepted candidates require a title, classification, format and either an official URL or an original file.")
    ...
    for row in range(3, ws.max_row + 1):
        if official_url and str(ws.cell(row, headers["official_url"]).value or "").strip() == official_url:
    ...
    defaults = {
        ...
        "authority_quality": (payload.get("authority_quality") or "") if payload.get("basic_inspection") else (payload.get("authority_quality") or "MEDIUM"),
        "scope_relevance": (payload.get("scope_relevance") or "") if payload.get("basic_inspection") else (payload.get("scope_relevance") or "MEDIUM"),
        "version_currency": (payload.get("version_currency") or "") if payload.get("basic_inspection") else (payload.get("version_currency") or "MEDIUM"),
        "traceability": (payload.get("traceability") or "") if payload.get("basic_inspection") else (payload.get("traceability") or "MEDIUM"),
        "access_permission": (payload.get("access_permission") or "") if payload.get("basic_inspection") else (payload.get("access_permission") or "HIGH"),
    ...
    if not official_url and not manual_state:
        raise HumanOperationError("A source without an official URL requires a validated original file.")
    if manual_state:
...
def review_operation_values(item: dict[str, Any], source: dict[str, Any], now: datetime) -> dict[str, Any]:
    ...
    if item.get("human_reported_issue"):
        payload["human_reported_issue"] = item["human_reported_issue"]
    values = {
...
def pending_human_reported_issues(config_path: Path) -> list[dict[str, Any]]:
    """Keep unresolved human findings in the queue until an explicit decision."""
    config = updater.read_config(config_path)
    wb = updater.open_registry(config)
    ops = wb[human_sheet_name(config)]
    headers = operation_headers(ops)
    items = []
    for row in range(HUMAN_DATA_START_ROW, last_operation_row(ops, headers) + 1):
        record = operation_record(ops, row, headers)
        payload = parse_payload(record.get("payload_json"))
        issue = payload.get("human_reported_issue")
        if record.get("program_status") not in OPEN_STATUSES or not issue:
            continue
        items.append({
            "source_id": record["source_id"], "source_title": record["source_title"],
            "trigger": "HUMAN_REPORTED_ISSUE", "issue_codes": ["HUMAN_REPORTED_ISSUE"],
            "review_since": None, "days_open": None, "severity": "OPEN",
            "reason": issue["reason"], "recommended_action": issue["action"],
            "review_fingerprint": issue["evidence_id"], "human_reported_issue": issue,
        })
    wb.close()
    return items
...
def sync_review_queue(config_path: Path, review_queue: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    added: list[str] = []
    superseded: list[str] = []
    with updater.registry_lock(config, float(config["lock_stale_hours"])):
        expected_mtime = updater.registry_revision(config)
        wb = updater.open_registry(config)
        ...
        for row in range(HUMAN_DATA_START_ROW, last_operation_row(ops, op_headers) + 1):
            ...
            if operation_type not in {"SOURCE_REVIEW", "SELECTION_REVIEW"} or status not in OPEN_STATUSES:
                continue
            replacement=any(str(item.get('source_id') or '')==str(record.get('source_id') or '') for item in review_queue)
            if sources.get(str(record.get('source_id') or ''),{}).get('operator_selection_decision')=='INCLUDE' and (not replacement or not operation_id.startswith('REV-')):
                continue  # Retain explicit holds; a consolidated replacement task may supersede an older machine task.
        ...
        if added or superseded:
            ...
            updater.backup_registry(config, now)
            updater.save_registry(wb, config, expected_mtime)
        wb.close()
...
def generate_random_qa(
    ...
    weekly = qa.get("schedule") == "MONDAY_WITH_CATCH_UP"
    if weekly:
        target_date = today - timedelta(days=today.weekday())
        batch_id = target_date.strftime("%G-W%V")
        batch_month = None
        scheduled_for = target_date
        if not force and not qa.get("enabled", True):
            return {"status": "NOT_DUE", "generated_count": 0, "operation_ids": []}
    else:
        ...
        target_date = today if today.day == scheduled_day or not catch_up else today.replace(day=1) - timedelta(days=1)
        if not force and (not qa.get("enabled", True) or (today.day != scheduled_day and not catch_up)):
            return {"status": "NOT_DUE", "scheduled_for": date(today.year, today.month, scheduled_day).isoformat(),
                    "generated_count": 0, "operation_ids": []}
        batch_month = target_date.strftime("%Y-%m")
        batch_id = target_date.strftime("%Y%m")
        scheduled_for = target_date.replace(day=calendar.monthrange(target_date.year, target_date.month)[1])
    try:
        sample_size = int(qa.get("weekly_sample_size" if weekly else "monthly_sample_size", 5))
    except (TypeError, ValueError):
        sample_size = 0
    if not 1 <= sample_size <= int(qa.get("max_sample_size", 20)):
        raise HumanOperationError("Random QA sample size must be within the configured range.")
    batch_fields = {"batch_week": target_date.isoformat()} if weekly else {"batch_month": batch_month}
    generated: list[str] = []
    with updater.registry_lock(config, float(config["lock_stale_hours"])):
        expected_mtime = updater.registry_revision(config)
        wb = updater.open_registry(config)
        ...
        open_random_ids = {
            ...
            for row in range(HUMAN_DATA_START_ROW, last_operation_row(ops, op_headers) + 1)
            if (weekly or str(ops.cell(row, op_headers["operation_type"]).value or "").strip() == "RANDOM_QA_CHECK")
        ...
        for row in range(HUMAN_DATA_START_ROW, last_operation_row(ops, op_headers) + 1):
            ...
            if (payload.get("batch_week") == target_date.isoformat() if weekly else
                    str(payload.get("batch_month") or legacy_month).strip() == batch_month):
                existing_batch = True
                break
        if existing_batch:
            wb.close()
            return {"status": "ALREADY_CREATED", **batch_fields, "generated_count": 0, "operation_ids": []}
        source_ws = wb[config["sheet_name"]]
        source_headers = updater.workbook_headers(source_ws, int(config["header_row"]))
        ...
        for record in selected:
            ...
            values = {
                ...
                "source_id": source_id,
                "source_title": record.get("source_title"),
                "trigger": "WEEKLY_RANDOM_SAMPLE" if weekly else "MONTH_END_RANDOM_SAMPLE",
                ...
                "payload_json": json.dumps({
                    ...
                    **batch_fields,
                    ...
                    "source_fingerprint": source_record_fingerprint(record),
        ...
        if weekly:
            append_operation(ops, op_headers, {
                "operation_id": f"QA-{batch_id}-SUMMARY", "operation_type": "RANDOM_QA_BATCH_SUMMARY",
                "trigger": "WEEKLY_RANDOM_SAMPLE", "program_status": "APPLIED", "created_at": now,
                "program_note": f"Weekly batch: {len(selected)} of {sample_size} requested items; {len(eligible)} eligible.",
                "payload_json": json.dumps({**batch_fields, "batch_id": batch_id,
                    "scheduled_for": scheduled_for.isoformat(), "requested_count": sample_size,
                    "sampled_count": len(selected), "eligible_count": len(eligible)}, sort_keys=True),
            })
        if generated or weekly:
            prepare_human_workbook(wb, ops, op_headers)
            updater.backup_registry(config, now)
            updater.save_registry(wb, config, expected_mtime)
        wb.close()
...
def process_decisions(config_path: Path, operation_ids: set[str] | None = None) -> dict[str, Any]:
    config = updater.read_config(config_path)
    now = updater.local_now(config)
    ...
    with updater.registry_lock(config, float(config["lock_stale_hours"])):
        ...
        for row in range(HUMAN_DATA_START_ROW, final_row + 1):
            ...
            if not operation_id or (operation_ids is not None and operation_id not in operation_ids):
                continue
            ...
            try:
                ...
                if operation_type == "NEW_SOURCE_CANDIDATE":
                    ...
                    if decision == "INCORRECT":
                        ...
                        followup = {
                            ...
                            "file_format": record.get("file_format"),
                            "created_at": now,
                            "payload_json": json.dumps({"action": "APPLY_FIELD_UPDATES", "source_id": record.get("source_id"), "updates": {},
                                "human_reported_issue": {"reason": note,
                                    "action": "Correct the reported error and explicitly verify the original.",
                                    "evidence_id": operation_id, "reported_by": operator,
                                    "reported_at": now.isoformat()}}, sort_keys=True),
                        }
                ...
                if payload.get("workbench_request") and operation_type in {"SOURCE_REVIEW", "SELECTION_REVIEW"}:
                    source_row = find_source_row(source_ws, source_headers, str(record.get("source_id") or ""))
                    if source_row is not None:
                        current_source = updater.record_from_row(source_ws, source_row, source_headers)
                        # Recording a decision does not resolve an absent original or pending selection.
                        # Reuse the owning deterministic source checks, without a network action.
                        from leader_orchestrator import review_trigger
                        unresolved_source = (updater.selection_from_scores(current_source) == "PENDING"
                                             or review_trigger(current_source) is not None)
                unresolved_human_issue = (payload.get("human_reported_issue")
                                          and not payload.get("human_reported_issue_resolved"))
                if payload.get("workbench_request") and (unresolved_source or unresolved_human_issue):
                    # A partial browser decision keeps the task open and retains its receipt.
                    source_row = find_source_row(source_ws, source_headers, str(record.get("source_id") or ""))
                    if source_row is not None:
                        payload["source_fingerprint"] = source_record_fingerprint(
                            updater.record_from_row(source_ws, source_row, source_headers))
                    payload["workbench_applied"] = payload["workbench_request"]
                    payload["workbench_receipts"] = list(dict.fromkeys(payload.get("workbench_receipts", []) + [payload["workbench_request"]]))
                    set_operation_fields(ops, row, op_headers, {
                        "checked_at": checked_at, "program_status": "WAITING_FOR_HUMAN",
                        "program_operated_at": now, "decision": "PENDING",
                        "operation_type": "SOURCE_REVIEW",
                        "payload_json": json.dumps(payload, sort_keys=True),
                        "program_note": ("Fields applied. The separately reported human issue still requires verification."
                                         if unresolved_human_issue else
                                         "Decision saved. Source selection or original acquisition remains unresolved."),
                    })
                    processed.append(operation_id)
                    changed = True
                    continue
                if payload.get("workbench_request"):
                    payload["workbench_applied"] = payload["workbench_request"]
                    payload["workbench_receipts"] = list(dict.fromkeys(payload.get("workbench_receipts", []) + [payload["workbench_request"]]))
                    set_operation_fields(ops, row, op_headers, {"payload_json": json.dumps(payload, sort_keys=True)})
...
def reconcile_resolved_reviews(config_path: Path) -> dict[str, Any]:
    ...
    with updater.registry_lock(config, float(config["lock_stale_hours"])):
        ...
        for row in range(HUMAN_DATA_START_ROW, last_operation_row(ops, op_headers) + 1):
            ...
            if source and source.get('operator_selection_decision')=='INCLUDE':
                continue  # Keep the re-review gate until a person resolves this task.
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Process staged browser-workbench decisions and compatibility requests.")
