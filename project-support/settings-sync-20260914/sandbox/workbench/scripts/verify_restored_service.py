...
def main():
    ...
    config = json.loads((ROOT/'system1/Code/config/config.json').read_text()); config['random_qa']={'enabled': False}
    path=source/'system1/Code/config/config.json'; path.parent.mkdir(parents=True); path.write_text(json.dumps(config))
    for old, new in [(prior/'Data',source/'system1/Data'),(prior/'governance',source/'system1/Code/runtime'),
    ...
    for relative in ('system1/Code/runtime/logs/source-assessments.sqlite','system1/Code/runtime/logs/leader_state.sqlite','system2/runtime/jobs.sqlite3'):
    ...
    for rel in ('system1/Code/src', 'system2/src', 'workbench/src', 'workbench/ui'):
    ...
    for relative in ('system1/Code/.venv', 'system2/.venv', 'workbench/.venv'):
    ...
    with (area/'server.log').open('w') as log:
        process=subprocess.Popen(command,stdout=log,stderr=log,env=dict(os.environ,PYTHONPATH=str(code/'workbench/src')))
