...
## Compact QC follow-up

Each group now has a QC title and one horizontal control row: live scalar K or inclusive [min, max], All, Any, Only, Not All, and numeric MIN-MAX inputs. Formula tooltips explain each shortcut. For N direct children, the shortcuts store N, [1,N], 1 and [1,N-1] respectively. The user-defined Not All excludes zero and is disabled when N < 2. It is not a general Boolean negation operator. Nested groups still count as one direct child.

Digit-only bounds clamp to 0..N. Empty or reversed bounds remain visible with an error and cannot be submitted. Enter or leaving the row applies an unsaved preview; Save splitting is still required for durable history. Pending bounds survive re-render, participate in leave warnings, and flush before manual save. Escape restores the previous quantity without discarding earlier splitting work. Narrow columns keep one line with horizontal scrolling.

Validation: 319 frontend and 18 Requirement backend tests passed, including representation fidelity, nested ownership, bounds validation, transient-input retention, failed-preview retention and save ordering. In the normal example browser, R1 conditions changed from 4 to [1,3] with Not All, then previewed/applied [0,2]. Entering 99 clamped to 4; [4,2] showed an error and was blocked. The explicit leave warning included pending invalid input. All test edits were discarded, leaving example personal revision 1 saved. No real AI request, schema migration or Windows native acceptance was performed.
