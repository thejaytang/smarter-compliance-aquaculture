...
import {emptyDesign,setDesign,setHandoff,designMarkup,domainMarkup,ruleMarkup,logicText,logicFields,pruneConcepts,conceptIssues,bindDesign} from './check-design.js';
...
export class InterpretationEditor{
 ...
 render(){
  ...
  const list=entries.map(s=>{const ids=r.rootIds?r.rootIds(s):Object.keys(s.units||{}).filter(id=>s.roles?.[id]!=='condition'),id=ids[0];if(!id)return '';const active=valid&&s.units?.[d.unit_id];
   return `<details class="ip-requirement" ${active?'open':''}><summary data-ip-select="${active?d.unit_id:id}"><strong>${esc(r.entryLabel?r.entryLabel(s):r.label(id))}</strong><span class="ip-entry-preview">${esc((s.text||s.units[id].text).slice(0,100))}</span></summary>${active&&ids.length>1?`<div class="ip-clause-choices">${ids.map((uid,i)=>`<button data-ip-select="${uid}" aria-pressed="${d.unit_id===uid}">${esc(r.internalLabel?.(uid)||`G${i+1}`)} · ${esc(s.units[uid].text.slice(0,65))}</button>`).join('')}</div>`:''}${active?this.editorMarkup(d):''}</details>`;}).join('');
 ...
 fieldMarkup(d,key,label,readOnly=false){
  const f=d.fields[key],disabled=this.m.collaboration.readonly?'disabled':'';
  return `<div class="ip-field" data-ip-field="${key}">${readOnly?`<p class="ip-caption">Previous explanation · retained</p><p>${esc(f.value||'No earlier explanation.')}</p>`:`<label>${esc(label)}<textarea data-logic-value="${key}" aria-label="${esc(label)}" rows="3" ${disabled}>${esc(f.value)}</textarea></label>`}<details class="ip-evidence" data-disclosure="evidence-${key}"><summary>Evidence &amp; questions (${f.references?.length||0} citations)</summary>${f.state==='not_stated'?`<p>${esc(f.absence_reason)}</p>`:''}${(f.references||[]).map((ref,i)=>`<blockquote>${esc(ref.quote)}</blockquote><button data-ip="remove-ref" data-ref="${i}" ${disabled}>Remove citation</button>`).join('')}<label>Saved context passage<select data-ip-citation ${disabled}>${(d.context?.citations||[]).map(ref=>`<option value="${esc(ref.id)}">${esc(ref.text.slice(0,130))}</option>`).join('')}</select></label><label>Exact supporting quotation<textarea data-ip-quote rows="2" ${disabled}></textarea></label><button data-ip="add-ref" ${disabled}>Add citation</button><label>Open questions · one per line<textarea data-field-gaps="${key}" rows="2" ${disabled}>${esc((f.gaps||[]).join('\n'))}</textarea></label></details></div>`;
 }
 cardCandidate(d,key,index,disabled){
  const c=d.candidate;if(!c?.suggestions?.[key]||c.accepted?.includes(key)||c.dismissed?.includes(key))return '';
  return `<details class="ip-candidate" data-disclosure="candidate-${key}" open><summary>Candidate · compare before accepting</summary><p class="ip-caption">Current logic</p><p>${esc(logicText(d.check_design?.groups[key])||d.fields[key].value)}</p><p class="ip-caption">Proposed logic</p><p>${esc(logicText(c.check_design?.groups[key])||c.suggestions[key].value)}</p><p>${esc(c.suggestions[logicKeys[index]]?.value||'')}</p>${c.check_design?`<p>Concepts: ${esc((c.check_design.concepts||[]).filter(t=>JSON.stringify(c.check_design.groups[key]).includes('"'+t.id+'"')).map(t=>t.label).join(', '))}</p>`:''}<button data-ip="accept-card" data-field="${key}" ${disabled}>Accept ${key} candidate</button><button data-ip="dismiss-card" data-field="${key}" ${disabled}>Dismiss candidate</button></details>`;
 }
 editorMarkup(d){
  ...
  return `<div class="ip-heading"><p class="ip-state">${d.dirty?'Unsaved changes':d.revision?'Saved draft':'Not saved'} · Interpretation ${d.reviewed?'confirmed':'pending'} · ${esc(r.label(d.unit_id))}</p></div><p class="ip-notice" role="status">${esc(this.notice)}</p>
   ...
   ${domainMarkup(design,editDisabled)}<div class="ip-generate"><button data-ip="generate" ${disabled||!d.provider?.available||d.generation?'disabled':''}>${d.generation?'Generating interpretation…':'Generate interpretation draft'}</button>${!d.provider?.available?'<span class="ip-caption">AI: Not connected</span>':''}</div>
   ${['scope','condition','demand'].map((key,i)=>`<section class="ip-group" data-card="${key}"><h4>${['Scope · Set A','Condition · Set B','Demand · Set C'][i]}</h4><p class="ip-caption">${['Which objects are in scope?','Which objects in A does this apply to?','What must those objects satisfy?'][i]}</p>${this.cardCandidate(d,key,i,disabled)}${design.groups[key]?ruleMarkup(design,key,editDisabled,d.catalog):`${this.fieldMarkup(d,key,['Scope Logic','Condition Logic','Demand Logic'][i])}${ruleMarkup(design,key,editDisabled,d.catalog)}`}<details data-disclosure="support-${key}"><summary>Source &amp; supporting information</summary><p class="ip-caption">Mapped from the third pane</p><blockquote class="ip-source-wording" tabindex="0" aria-label="${['Scope','Conditions','Demands'][i]} mapped wording">${esc(derived[key].value||'No wording assigned in the third pane.')}</blockquote>${!design.groups[key]?`<button data-ip="use-mapped" data-field="${key}" ${disabled||!derived[key].value?'disabled':''}>Use as starting text</button>`:this.fieldMarkup(d,key,'',true)}${this.fieldMarkup(d,logicKeys[i],interpretationLabels[interpretationKeys.indexOf(logicKeys[i])])}</details></section>`).join('')}
 ...
 changed(){const d=this.draft;d.dirty=true;d.reviewed=false;d.workingStored=false;d.check_design=setDesign(d.check_design||emptyDesign());d.check_design.based_on=null;d.changeNumber=(d.changeNumber||0)+1;const state=this.host?.querySelector?.('.ip-state');if(state)state.textContent='Unsaved changes';this.refreshOutput();}
 ...
 async action(action,node){
  ...
  if(action==='accept-card'){
   ...
   c.accepted=[...(c.accepted||[]),key];this.changed();this.notice='Candidate accepted into this card. Confirm its concepts and save the draft when ready.';return this.render();
  }
  if(action==='use-mapped'){
   const key=node.dataset.field,value=sourceSections(d.context)[key]?.value;if(!value)return;
   const apply=()=>{d.fields[key]={value,basis:'interpretation',state:'specified',references:[],gaps:[]};this.changed();this.render();};
 ...
 async watch(id,key){
  try{const run=await this.m.api('/api/interpretations/run?'+new URLSearchParams({id}));const d=this.drafts.get(key);if(!d)return;
   ...
   d.generation=null;if(run.status==='ready'){d.candidate={...run,suggestions:{...(d.candidate?.context_fingerprint===run.context_fingerprint?d.candidate.suggestions:{}),...run.suggestions}};this.notice='Draft ready. Compare and accept each card separately; your current edits are unchanged.';}
