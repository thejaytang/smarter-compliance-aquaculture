...
def merge_documents(base, current, incoming, decisions=None, *, version=1):
    ...
    Version 1 preserves historical whole-list reorder decisions. Version 2 can
    separate order from content only with exact three-side block identities.
    Unknown split/merge patterns still need whole-list review. Structural table
    changes need whole-table review, even on one side.
    """
    if type(version) is not int or version not in (1, 2):
        raise ValueError('Unsupported comparison version. Reopen a supported saved comparison.')
    ...
    def identity_list(before, ours, theirs, path):
        ...
        separate_order = (version == 2 and path == '/blocks'
                          and base_set == set(our_ids) == set(their_ids)
                          and not (base_ids == our_ids == their_ids))
        structural = False
        ...
        if structural and not separate_order:
        ...
        if separate_order:
            order = difference(base_ids, our_ids, their_ids, path, kind='order',
                               force=base_ids != their_ids)
            if (not isinstance(order, list) or any(not isinstance(i, str) for i in order)
                    or len(order) != len(base_ids) or set(order) != base_set):
                raise ValueError('Keep every content block exactly once when arranging its order. Order cannot add, remove or replace content.')
