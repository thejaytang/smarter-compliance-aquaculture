...
export function checkingLogic(fields,exceptions=[]){
 const values=Object.fromEntries(interpretationKeys.map(k=>[k,(fields[k]?.value||'').trim()]));
 return {version:1,executable:false,steps:[
  ...
  gaps:[...interpretationKeys.filter(k=>!values[k]||fields[k]?.basis==='unresolved').map(k=>interpretationLabels[interpretationKeys.indexOf(k)]),...interpretationKeys.flatMap(k=>fields[k]?.gaps||[])],
...
export class InterpretationEditor{
 ...
 async refreshProvider(){
  try{const p=await this.m.api('/api/settings/ai');for(const [key,d] of this.drafts)if(key.startsWith(this.owner()+':'))d.provider=p;this.render();}catch(e){this.m.message(e.message,'warning');}
 }
 ...
 canLeave(){if(this.hasUnsaved()){this.m.message('Save all interpretation drafts before leaving. Select an unsaved requirement in the fourth pane to continue.','warning');return false;}return true;}
 journal(){try{const data=[...this.drafts.entries()].filter(([,d])=>d.dirty||d.retry).map(([k,d])=>[k,d]);sessionStorage.setItem('interpretation-drafts:'+this.m.tabIdentity,JSON.stringify(data));}catch{this.notice='This browser could not retain the draft locally. Keep this page open and save your interpretation.';}}
 restoreJournal(){if(this.restored)return;this.restored=true;try{for(const [k,d] of JSON.parse(sessionStorage.getItem('interpretation-drafts:'+this.m.tabIdentity)||'[]'))this.drafts.set(k,d);}catch{this.notice='The browser draft could not be recovered. Saved server versions remain available.';}}
 async open(uid){
  ...
  try{const doc=await this.m.api('/api/interpretations?'+new URLSearchParams({unit_id:uid}));if(this.owner()!==owner||this.active!==key)return;
   const old=this.drafts.get(key);if(old?.dirty||old?.retry){old.latest=doc;old.provider=doc.provider;old.sourceChanged=old.context?.fingerprint!==doc.context?.fingerprint||old.revision!==doc.revision;}
   ...
   this.notice=old?.dirty?'Unsaved draft restored.':doc.stale?'Source dependencies changed. Review and refresh the context before saving.':'Saved and reviewed are separate states.';
 ...
 render(){
  ...
  if(!valid){if(this.loading){host.innerHTML='<p role="status">Loading saved source context for this interpretation…</p>';return;}host.innerHTML=`${this.notice?'<p class="ip-warning" role="alert">'+esc(this.notice)+'</p>':''}`+'<div class="ip-empty"><h4>Explain how this requirement can be checked</h4><p>Choose <strong>Interpret requirement</strong> in the third pane. Define Scope, Condition and Demand, then the information and evidence each needs.</p><p>The checking chain is generated from your six fields.</p></div>';return;}
  ...
  host.innerHTML=`<div class="ip-heading"><p class="ip-caption">${esc(d.unit_id)} · revision ${d.revision||0}</p><p class="ip-state">${d.dirty?'Unsaved draft':d.reviewed?'Interpretation reviewed':d.revision?'Saved / not reviewed':'Not saved'}${d.stale||d.sourceChanged?' · needs source review':''}</p></div><p class="ip-notice" role="status">${esc(this.notice)}</p>${other.length?`<details><summary>${other.length} unsaved interpretation(s)</summary>${other.map(([k,x])=>`<button data-ip="switch" data-unit="${esc(x.unit_id)}">${esc(x.context?.requirement?.text?.slice(0,80)||x.unit_id)}</button>`).join('')}</details>`:''}
   <blockquote class="ip-original">${esc(d.context?.requirement?.text||d.latest?.context?.requirement?.text||'Current source unavailable')}</blockquote><button data-ip="source">Locate source</button>
   ...
   <div class="ip-save"><button data-ip="save" class="primary" ${disabled||d.context_error?'disabled':''}>Save interpretation</button><button data-ip="review" ${disabled||d.dirty||!d.revision||d.stale||d.sourceChanged||interpretationKeys.some(k=>!d.fields[k].value.trim()||d.fields[k].basis==='unresolved'||d.fields[k].gaps.length)?'disabled':''}>Mark interpretation reviewed</button><button data-ip="reload" ${disabled}>Reload saved version</button></div>
   <details><summary>Interpretation history</summary><p>Restore creates a new revision. Splitting and interpretations are not included in collaboration ZIPs.</p>${(d.history||[]).map(h=>`<p>Revision ${h.revision} · ${esc(h.action)} · ${esc(h.at)} <button data-ip="restore" data-revision="${h.revision}" ${disabled}>Restore</button></p>`).join('')}</details>`;
  ...
  host.querySelectorAll('[data-ip-field]').forEach(el=>{const k=el.dataset.ipField;el.querySelectorAll('[data-ip-value],[data-ip-basis],[data-ip-gaps]').forEach(input=>input.oninput=()=>{if(input.matches('[data-ip-value]')&&d.fields[k].basis==='source')el.querySelector('[data-ip-basis]').value='interpretation';d.fields[k].value=el.querySelector('[data-ip-value]').value;d.fields[k].basis=el.querySelector('[data-ip-basis]').value;d.fields[k].gaps=el.querySelector('[data-ip-gaps]').value.split('\n').filter(x=>x.trim());this.changed();host.querySelector('.ip-logic').innerHTML=logicMarkup(checkingLogic(d.fields,d.context?.exceptions));host.querySelector('.ip-state').textContent='Unsaved draft';host.querySelector('[data-ip="review"]').disabled=true;});});
 }
 changed(){this.draft.dirty=true;this.draft.reviewed=false;this.journal();}
 async action(action,node){
  ...
  if(action==='reload'){
   if(d.dirty||d.retry){this.m.dialog('<h2>Reload saved interpretation?</h2><p>Your current unsaved draft will be replaced by the server version.</p><button data-ip-confirm>Reload saved version</button>');this.m.q('[data-ip-confirm]').onclick=()=>{this.m.q('#mw-dialog').close();this.drafts.delete(this.active);this.journal();void this.open(d.unit_id);};return;}
 ...
 async save(action,node){
  ...
  const request=action==='retry'?d.retry:{request_id:crypto.randomUUID(),unit_id:d.unit_id,expected_revision:d.revision,context_fingerprint:d.context?.fingerprint,linked_material_ids:d.linked_material_ids,fields:copy(d.fields),action:action==='save'?'save':action,history_revision:Number(node.dataset.revision)};
  if(!request)return;this.pending=true;d.retry=request;this.journal();this.notice='Saving interpretation…';this.render();
  try{await this.m.api('/api/interpretations/save',request);d.retry=null;d.dirty=false;this.journal();const saved=await this.m.api('/api/interpretations?'+new URLSearchParams({unit_id:d.unit_id}));this.drafts.set(key,{...saved,dirty:false,candidate:d.candidate});this.notice='Interpretation saved. Material review and archive status are unchanged.';}
