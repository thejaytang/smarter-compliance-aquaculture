...
ROOT=Path(__file__).resolve().parents[2];OUT=Path(__file__).resolve().parent / 'final-activation';OUT.mkdir(exist_ok=True)
...
assets={}
for name in ['check-design.js','interpretations.js','four-pane.css','materials.js','requirements.js','global-settings.js']:
