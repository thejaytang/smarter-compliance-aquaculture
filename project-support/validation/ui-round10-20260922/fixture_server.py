...
class Fixture(Handler):
 def do_GET(self):
  ...
  if path=='/region.svg':return self.send(200,b'<svg xmlns="http://www.w3.org/2000/svg" width="600" height="400"><rect width="600" height="400" fill="#e9f2f9"/><rect x="80" y="70" width="400" height="200" fill="none" stroke="#265e8a"/><text x="100" y="170" font-size="24">Synthetic original PDF region</text></svg>','image/svg+xml')
 ...
 def do_POST(self):
  if self.path not in {'/api/interpretations/save','/api/requirements/step'}:return self.send(403,{'error':'Only explicit isolated Interpretation save is enabled'})
  return super().do_POST()

def main():
 parser=argparse.ArgumentParser();parser.add_argument('--port',type=int,default=0);args=parser.parse_args()
 with TemporaryDirectory(prefix='ui-r10-fixture-') as temp:
  folder=Path(temp);path=folder/'synthetic.sqlite';mid='a'*32
  material=dict(id=mid,title='Synthetic ABC source',revision=1,source={'source_id':'FX010','snapshot_id':'FX010-001','content_hash':'synthetic-r10'},blocks=[dict(id='main',type='text',text=TEXT,source_refs=[{'scope_id':'fixture:main'}]),dict(id='records',type='text',text='Keep records of integrity. Facility context.',source_refs=[{'scope_id':'fixture:records'}]),dict(id='other',type='text',text='Another facility requires a separate interpretation.',source_refs=[{'scope_id':'fixture:other'}])])
  material.update(content_revision=1,issues=[],scope=[dict(id='fixture:main',label='Synthetic first page',location={'page':1})],checked_scope=[],association_review_required=True,content_status='draft',candidates=[])
  material['blocks'][0]['source_refs']=[{'page':1,'scope_id':'fixture:main'}]
  for i in range(1,46):
   wording=f'Passage {i}: keep this exact reading position. '+('Current revised wording. '*8)
   material['blocks'].append(dict(id=f'passage-{i}',type='text',text=wording,source_refs=[{'page':1,'scope_id':'fixture:main'}],markdown={'source':wording,'baseline':('Earlier deleted wording. '*30 if i<15 else wording),'kind':'paragraph'}))
  material['blocks'].append(dict(id='sheet-link',type='text',text='This source association points to C23.',source_refs=[{'sheet':'Water','cell_range':'C23'}]))
  ...
  server=ThreadingHTTPServer(('127.0.0.1',args.port),Fixture);server.app=SimpleNamespace(reviewer=False,csrf='r10-fixture',store=SimpleNamespace(session=lambda token:{'id':'fixture','name':ACTOR,'token':'fixture'}),collaboration=c)
  server.info={'material':material,'units':units,'sessions':sessions,'csrf':'r10-fixture'};server.material=material;server.original=deepcopy(material)
  print(f'R10 isolated keyboard and history: http://127.0.0.1:{server.server_port}/',flush=True)
