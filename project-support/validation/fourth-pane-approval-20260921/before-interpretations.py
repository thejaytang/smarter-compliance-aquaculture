...
from backend.system3.requirements import Requirements, FIELDS, RELATIONS, leaves, encode
...
class Interpretations:
    def __init__(self, c):
        ...
        with c.db() as db:
            ...
            # One-time legacy backfill, then indexed per-unit reads. This table is
            # a projection; the persisted run body remains authoritative.
            db.execute('CREATE TABLE IF NOT EXISTS interpretation_migrations(name TEXT PRIMARY KEY)')
            if not db.execute("SELECT 1 FROM interpretation_migrations WHERE name='run-index-1'").fetchone():
                db.execute('''INSERT OR IGNORE INTO interpretation_run_index
    ...
    def trace(self, actor, uid, revision=None):
        ...
        with self.c.db() as db:
            ...
            revision=latest if revision is None else int(revision)
            lineage.backfill(db,actor,uid)
            result=lineage.read(db,actor,uid,revision)
            if result['status']!='saved':raise ValueError('The requested saved source trail is unavailable.')
    ...
    def context(self, actor, uid, linked=(), *, _materials=None):
        ...
        # Adding this index must not make unchanged legacy drafts appear stale.
        payload['fingerprint']=fingerprint({k:v for k,v in payload.items() if k!='origin'})
        return payload
    ...
    def generate(self,actor,req):
        ...
        with self.c.lock,self.c.db() as db:
            ...
            if not isinstance(keys,list) or not 1<=len(keys)<=6 or any(k not in KEYS for k in keys) or len(set(keys))!=len(keys):raise ValueError('Choose valid fields.')
    ...
    def _generate(self,actor,run,cfg):
        try:
            ...
            from backend.shared.ai_settings import provider_opener
            ...
            try:
                ...
                with RUN_LOCK:RUNNING.discard((str(self.c.app.runtime),actor,run['id']))
