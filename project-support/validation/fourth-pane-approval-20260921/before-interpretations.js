...
export class InterpretationEditor{
 ...
 editorMarkup(d){
  ...
  return `<div class="ip-heading"><p class="ip-state">${esc(r.label(d.unit_id))} · ${d.dirty?'Unsaved changes':d.revision?'Saved':'Not saved'} · ${d.reviewed?'Confirmed':'Review pending'}</p><button data-ip="edit" aria-pressed="${editing}" ${editDisabled}>${editing?'Done editing':'Edit'}</button></div><p class="ip-notice" role="status">${esc(notice)}</p>
   ...
   <details class="ip-evidence-panel" data-disclosure="sources"><summary>Evidence</summary><button data-ip="source">Locate source</button>${['scope','condition','demand'].map((key,i)=>`<section><h5>${['Scope','Condition','Demand'][i]}</h5><p class="ip-caption">From the third pane</p><blockquote class="ip-source-wording" tabindex="0" aria-label="${['Scope','Conditions','Demands'][i]} mapped wording">${esc(derived[key].value||'No wording assigned in the third pane.')}</blockquote>${editing&&!design.groups[key]?`<button data-ip="use-mapped" data-field="${key}" ${disabled||!derived[key].value?'disabled':''}>Use as starting text</button>`:''}${design.groups[key]||!editing?this.fieldMarkup(d,key,'',true):''}${this.fieldMarkup(d,logicKeys[i],interpretationLabels[interpretationKeys.indexOf(logicKeys[i])])}</section>`).join('')}</details>
   <details class="ip-more" data-disclosure="more"><summary>More</summary>
    <div class="ip-generate"><button data-ip="generate" ${disabled||!d.provider?.available||d.generation?'disabled':''}>${d.generation?'Generating interpretation…':'Generate interpretation draft'}</button>${!d.provider?.available?'<span class="ip-caption">AI: Not connected</span>':''}</div>
 ...
 changed(){const d=this.draft;d.dirty=true;d.reviewed=false;d.workingStored=false;d.check_design=setDesign(d.check_design||emptyDesign());d.check_design.based_on=null;d.changeNumber=(d.changeNumber||0)+1;const state=this.host?.querySelector?.('.ip-state');if(state)state.textContent='Unsaved changes';this.refreshOutput();const save=this.host?.querySelector?.('[data-ip="save"]');if(save)save.disabled=!!(this.pending||this.loading||d.retry||this.m.collaboration?.readonly);}
