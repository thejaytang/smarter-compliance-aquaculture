...
import {emptyDesign,setDesign,setHandoff,filterPreview,designMarkup,ruleMarkup,conceptIssues,designNode,ruleValue,bindDesign,ruleSummaryMarkup} from '../frontend/components/check-design.js';
...
test('reading preserves nested OR and group negation, shared concepts and escaped source text',()=>{
 ...
 const before=structuredClone(d),html=ruleSummaryMarkup(d,'scope');
 assert.match(html,/All of/);assert.match(html,/NOT · Any of/);assert.match(html,/Storm/);assert.match(html,/Earthquake/);assert.match(html,/Facility &lt;one&gt;/);assert.match(html,/unconfirmed/);
 assert.equal((html.match(/class="ip-concept-term"/g)||[]).length,1);assert.doesNotMatch(html,/<textarea|<input|<select|<button/);assert.deepEqual(d,before);
});
