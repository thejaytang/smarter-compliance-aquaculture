...
// Read the same tree without turning every rule into an always-visible form.
export function ruleSummaryMarkup(design,key){
 const node=n=>{
  if(!n)return '<p class="ip-caption">No logic defined.</p>';
  if(!n.rules)return `<p>${esc(logicText(n))}</p>`;
  if(n.rules.length===1&&!n.not)return node(n.rules[0]);
  return `<div class="ip-read-group"><p class="ip-junction">${n.not?'NOT · ':''}${n.condition==='AND'?'All of':'Any of'}</p>${n.rules.length?`<ul>${n.rules.map(r=>`<li>${node(r)}</li>`).join('')}</ul>`:'<p class="ip-caption">No rules defined.</p>'}</div>`;
 };
 const used=new Set(),collect=n=>{if(n?.rules)n.rules.forEach(collect);else n?.concept_ids?.forEach(id=>used.add(id));};collect(design.groups[key]);
 const concepts=(design.concepts||[]).filter(c=>used.has(c.id));
 return `${node(design.groups[key])}${concepts.length?`<div class="ip-concept-summary" aria-label="Concepts for ${key}"><span>Concepts: </span>${concepts.map(c=>`<span class="ip-concept-term" title="${esc(kinds[c.kind])} · ${c.status==='confirmed'?'Confirmed':'Unconfirmed'}" aria-label="${esc(c.label)} · ${c.status==='confirmed'?'confirmed':'unconfirmed'}">${esc(c.label)}</span>`).join(' · ')}</div>`:''}`;
}
