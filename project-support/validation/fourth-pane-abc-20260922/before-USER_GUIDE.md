...
The fourth pane follows the selected Requirement in the third pane. It opens in a reading view with **Scope · A**, **Condition · B**, **Demand · C** and their linked concepts. Choose **Edit** to change rules, concepts or supporting information; nested AND/OR/NOT and quantitative or temporal wording remain intact. **Done editing** returns to reading without saving. Each card shows **From third-column annotations · program-combined**: a deterministic projection of recorded wording, grouping and quantities, with no model call or semantic inference. **Copy as starting text** is explicit and does not approve anything. The editable interpretation and its supporting notes remain human work. **Evidence** holds earlier explanations and exact quotations. Earlier prose-only records remain editable until you explicitly choose **Define logic**. The grammatical Subject is evidence, not an automatic Scope: a facility can be the scoped object while seawater temperature, depth and weekly frequency constrain Demand.

While editing, link local **Concepts**, **Relations**, **Properties**, **Events** and **Actions** to each rule. Names have stable IDs and a proposed/confirmed meaning status. Expand a term to edit its name/type, attach quotations, or confirm its meaning in context. Shared terms keep the same ID across cards. This does not bind them to an external ontology. Unconfirmed meanings remain in **Review questions** and block interpretation confirmation. One **Generate AI candidates** button generates Scope, Condition and Demand together, including their supporting notes and proposed concepts, after provider configuration and context confirmation. Nothing is generated on opening a Requirement. Each result remains separate until **Approve scope candidate**, **Approve condition candidate** or **Approve demand candidate** replaces that card in the editable draft. Other cards remain unchanged. **Dismiss candidate** retains existing text. Manual editing works without an API. **More** contains source-context management and history.

The summary shows **B ⊆ C**: every applicable object selected from A must meet Demand in the same assessment context. **Check context** holds the shared object and assessment period; edit it in editing mode. **More → Data integration** holds Site Model identity/field mappings, QueryBuilder filters and the full set handoff. Mapping readiness is separate from semantic review and stays outside the everyday reading view. Unsupported predicates and NOT block their whole filter. **Confirm data mappings** is separate from **Confirm interpretation**. For manually written or earlier saved drafts, use the individual **Approve scope**, **Approve condition** and **Approve demand** actions after reading their supporting notes. There is no approve-all action. **Save draft** records content and those explicit approvals with reviewer/time; saving alone grants none. Changes to a card or its supporting notes invalidate its approval; changes to shared object/period or source context require renewed approvals. **Confirm interpretation** requires all three saved approvals, resolved interpretation questions and confirmed concept meanings. Missing evidence is unresolved, and no compliance result is executed. Existing history and exact Requirement bindings are retained.
