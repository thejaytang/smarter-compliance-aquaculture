...
def detect_table_boxes(image_path: Path, settings: TableSettings) -> list[PixelBox]:
    ...
    for contour in contours:
        ...
        # A page frame has only outer edges. A large table can occupy most of
        # the page; retain it when internal rules independently prove a grid.
        if width * height > page_area * 0.45 and (
            len(horizontal_rows) < 3 or len(vertical_cols) < 3
        ):
            continue
