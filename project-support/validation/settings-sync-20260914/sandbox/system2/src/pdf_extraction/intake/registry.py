...
def read_registry(path: Path) -> tuple[list[dict], str]:
    # Legacy workbook intake only. Database-backed lists must not import Excel.
    ...
    try:
        ...
        next(rows)
        headers = next(rows)
        required = set(Snapshot.model_fields) - {'relative_path', 'registry_sha256', 'registry_kind', 'selection_origin', 'assessment_id', 'parsing_copy_of'}
...
def build_manifest(records: list[dict], registry_hash: str, root: Path,
    ...
    for source_id in sorted(source_ids):
        ...
        if row.get('selection_origin') == 'machine' and row.get('assessment_id') and row.get('operator_selection_decision') == 'PENDING':
            gates.pop('operator_selection_decision')
        ...
        try:
            ...
            item = Snapshot(**{k:row[k] for k in Snapshot.model_fields if k not in {'relative_path','registry_sha256'} and k in row},
