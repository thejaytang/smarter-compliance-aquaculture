...
def _inspect_pdf_table_grid(page, box, xs, ys, specs, outer):
    """Check proposed cells against unchanged native lines, respecting merges.

    This DEV-calibrated consistency guard does not establish table or heading
    truth. Ambiguous ownership retains the grid as evidence and falls back to
    source lines; a bad region must not fail the rest of the document.
    """
    cells = [{'row':r, 'col':c, 'rowspan':rs, 'colspan':cs,
              'bbox':[(box.x0+xs[c])/1.5, (box.y0+ys[r])/1.5,
                      (box.x0+xs[c+cs])/1.5, (box.y0+ys[r+rs])/1.5]}
             for r,c,rs,cs in specs]
    captured, uncertain, fallback = [], [], []
    tolerance = POLICY['table_grid_tolerance_points']
    for line in page.text_lines or page.words:
        if not line.text.strip():
            continue
        left, top, right, bottom = line.bbox_points
        cx, cy = (left+right)/2, (top+bottom)/2
        if outer[0]<=left and outer[1]<=top and right<=outer[2] and bottom<=outer[3]:
            fallback.append(asdict(line))
        if not (outer[0]<=cx<outer[2] and outer[1]<=cy<outer[3]):
            continue
        owners = [cell for cell in cells if cell['bbox'][0]<=cx<cell['bbox'][2]
                  and cell['bbox'][1]<=cy<cell['bbox'][3]]
        if len(owners) != 1:
            uncertain.append({'line':asdict(line), 'owners':owners,
                              'reason':'missing_cell_owner' if not owners else 'multiple_cell_owners'})
            continue
        cell = owners[0]
        x0,y0,x1,y1 = cell['bbox']
        cuts = [max(0.,x0-left) if x0>outer[0]+1e-6 else 0.,
                max(0.,right-x1) if x1<outer[2]-1e-6 else 0.,
                max(0.,y0-top) if y0>outer[1]+1e-6 else 0.,
                max(0.,bottom-y1) if y1<outer[3]-1e-6 else 0.]
        captured.append({'line':asdict(line), 'owner':cell,
                         'internal_overflow_points':cuts,
                         'crosses_internal_boundary':max(cuts)>tolerance,
                         'external_overflow_points':[max(0.,outer[0]-left),max(0.,outer[1]-top),
                                                     max(0.,right-outer[2]),max(0.,bottom-outer[3])]})
    crossing = sum(item['crosses_internal_boundary'] for item in captured)
    ratio = crossing/len(captured) if captured else None
    reason = ('cell_ownership_uncertain' if uncertain else
              'native_line_evidence_missing' if ratio is None else
              'excessive_line_crossing' if ratio>=POLICY['table_grid_reject_ratio'] else None)
    return {'method':POLICY['table_grid_guard'], 'tolerance_points':tolerance,
            'reject_ratio':POLICY['table_grid_reject_ratio'],
            'captured_line_denominator':len(captured), 'crossing_line_numerator':crossing,
            'crossing_ratio':ratio, 'reject':reason is not None, 'reason':reason,
            'cells':cells, 'captured_lines':captured, 'uncertain_lines':uncertain,
            'original_positioned_line_fallback':fallback}
...
def _pdf_tables(page, original_page, source, reference, output):
    ...
    for number, box in enumerate(detect_table_boxes(image_path, TableSettings(backend='native', recognition_pipeline_enabled=False, img2table_enabled=False, gmft_enabled=False))):
        ...
        consistency = _inspect_pdf_table_grid(page, box, xs, ys, specs, bbox)
        inspected.append({'candidate_id':block['id'], **{key:consistency[key] for key in
                          ('captured_line_denominator','crossing_line_numerator','crossing_ratio','reject','reason')}})
        if consistency['reject']:
            rejected.append({'candidate':block, 'source_refs':block['source_refs'], **consistency})
            continue
        candidates.append((bbox,block))
    review = {'method':POLICY['table_grid_guard'], 'source_sha256':source.content_hash,
              'page_reference':reference, 'tolerance_points':POLICY['table_grid_tolerance_points'],
              'reject_ratio':POLICY['table_grid_reject_ratio'], 'rejected':rejected, 'inspected':inspected}
    return candidates, image_path.name, review


def _pdf_blocks(raw, source, output, page_indices):
    from ..assemble.material_rows import join_source_rows
        ...
        for offset in range(0, len(selected), 3):
            ...
            for local, page in enumerate(native.pages):
                ...
                with closing(original[index]) as original_page:
                    tables, page_image, grid_review = _pdf_tables(page, original_page, source, reference, output)
                    artifacts.append(page_image)
                    grid_evidence = f'pdf-table-grid-review-{index+1:04d}.json'
                    _write(output / grid_evidence, grid_review)
                    artifacts.append(grid_evidence)
                    for rejected in grid_review['rejected']:
                        explanation = (f"proposed table cells cross {rejected['crossing_line_numerator']} of {rejected['captured_line_denominator']} source text lines"
                                       if rejected['reason']=='excessive_line_crossing' else
                                       'table cells could not be associated reliably with source text')
                        message = f'Page {index+1}: {explanation}. Original positioned text is retained; compare the marked range with the source and correct the table if needed.'
                        unresolved.append({'scope_id':scope_id, 'code':'table_grid_inconsistent',
                                           'reason':rejected['reason'], 'message':message,
                                           'source_refs':rejected['source_refs'],
                                           'rejected_candidate_id':rejected['candidate']['id'],
                                           'evidence_ref':grid_evidence})
                    ...
                    # Prove each same-row join against source text before sorting
                    # fragments by their slightly different glyph top positions.
                    row_evidence = {'page_reference':reference, 'native_text':None, 'groups':[]}
                    try:
                        with closing(original_page.get_textpage()) as source_text_page:
                            row_evidence['native_text'] = source_text_page.get_text_range()
                        if not row_evidence['native_text'].strip() and any(line.text.strip() for line in lines):
                            row_evidence['native_text'] = None
                            raise ValueError('complete_source_text_empty')
                    except Exception as exc:
                        row_evidence['error_type'] = type(exc).__name__
                        message = f'Page {index+1}: original text order could not be checked. Positioned fragments are retained; compare their order with the original before confirming.'
                        unresolved.append({'scope_id':scope_id, 'code':'source_row_evidence_unavailable', 'message':message,
                                           'source_refs':[reference], 'evidence_ref':'pdf-source-row-groups.json'})
                        warnings.append(message)
                    if row_evidence['native_text'] is not None:
                        page_blocks, row_evidence['groups'] = join_source_rows(page_blocks, page.width_points, row_evidence['native_text'], lines, image_boxes)
                    source_rows.append(row_evidence)
    ...
    _write(output / 'pdf-source-row-groups.json', {'method': POLICY['pdf_source_rows'], 'source_sha256':source.content_hash,
           'adapter_config_hash':CONFIG_HASH, 'offset_basis':'whitespace-normalized native text, not glyph indices', 'pages':source_rows})
    artifacts.append('pdf-source-row-groups.json')
    if any(page['groups'] for page in source_rows):
        warnings.append('Same-row fragments were joined only where original text and geometry agree. Every original fragment is retained in evidence; verify row and paragraph boundaries before confirming.')
