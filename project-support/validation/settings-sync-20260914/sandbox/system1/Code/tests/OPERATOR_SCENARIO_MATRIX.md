...
The table names internal adapter actions used by isolated fixtures. It is not an Excel operator tutorial. Daily interaction is in the browser; see [USER_GUIDE.md](../../USER_GUIDE.md). Browser-specific coverage is in `test_workbench_bridge.py` and the sibling workbench tests.
