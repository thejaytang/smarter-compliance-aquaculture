...
class HumanOperationsTests(unittest.TestCase):
    ...
    def test_weekly_qa_five_items_catchup_and_year_boundary(self):
        ...
        wb = load_workbook(self.workbook_path)
        src = wb['Source Register']
        first = [cell.value for cell in src[3]]
        for i in range(2, 12):
            values = list(first); values[0] = f'PA{i:03}'
            src.append(values)
        wb.save(self.workbook_path); wb.close()
        first = operations.generate_random_qa(self.config_path, random.Random(1), today=date(2026, 12, 30))
        self.assertEqual(first['batch_week'], '2026-12-28')
        self.assertEqual(first['generated_count'], 5)
        duplicate = operations.generate_random_qa(self.config_path, today=date(2027, 1, 3))
        self.assertEqual(duplicate['status'], 'ALREADY_CREATED')
        second = operations.generate_random_qa(self.config_path, random.Random(2), today=date(2027, 1, 4))
        self.assertEqual(second['generated_count'], 5)
        self.assertFalse({s.rsplit('-',1)[-1] for s in first['operation_ids']} & {s.rsplit('-',1)[-1] for s in second['operation_ids']})
        third = operations.generate_random_qa(self.config_path, today=date(2027, 1, 11))
        self.assertEqual(third['generated_count'], 1)
        empty = operations.generate_random_qa(self.config_path, today=date(2027, 1, 18))
        self.assertEqual(empty['generated_count'], 0)
        self.assertEqual(operations.generate_random_qa(self.config_path, today=date(2027, 1, 19))['status'],'ALREADY_CREATED')
    ...
    def test_returned_include_is_not_restored_by_automatic_download_repair(self):
        item = {
        ...
        self.assertEqual(ops["K3"].value, "PENDING")
        self.assertIsNone(ops["L3"].value)
        self.assertFalse(ops.row_dimensions[3].hidden)
        after.close()
...
if __name__ == "__main__":
    ...
    def test_file_only_candidate_allows_blank_url_without_duplicate_blank_match(self):
        from unittest.mock import patch
        import source_updater as u
        cfg=u.read_config(self.config_path);wb=u.open_registry(cfg);ws=wb[cfg['sheet_name']];headers=u.workbook_headers(ws,2)
        ws.cell(3,headers['official_url'],'')
        payload={**self._candidate(),'official_url':'','retrieval_url':'','intake_file':'original.html'}
        with patch.object(operations,'promote_manual_candidate',return_value={'snapshot_status':'STORED','content_hash':'test'}),patch.object(operations,'archive_intake_file'):
            sid=operations.add_candidate_to_source_register(ws,headers,payload,cfg,u.local_now(cfg),'Weijie Tang','Reviewed original')
        self.assertEqual(sid,'PA002');self.assertEqual(ws.cell(4,headers['official_url']).value,'')
        with patch.object(operations,'promote_manual_candidate',return_value=None):
            with self.assertRaisesRegex(operations.HumanOperationError,'validated original'):
                operations.add_candidate_to_source_register(ws,headers,payload,cfg,u.local_now(cfg),'Weijie Tang','Missing original')
        wb.close()
