...
class SourceWorkflow:
 ...
 def intake_capabilities(self,actor):
  ...
  return {'update_check':checks[0] if checks else None,'can_register':self.c.mode=='coordinator' and actor==COORDINATOR,
 ...
 def intake(self,actor,request):
  ...
  with self.c.lock:
   ...
   if not payload:
    ...
    payload['actor']=actor;payload['basic_inspection']=inspected['inspection']
