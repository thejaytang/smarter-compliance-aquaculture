...
ROOT=Path(__file__).resolve().parents[2];OUT=Path(__file__).resolve().parent / 'activation-source-negation';OUT.mkdir(exist_ok=True)
...
def logical(path):
 ...
 return out
info=layout(ROOT);backup=OUT/'activation-backup';records=json.loads((backup/'manifest.json').read_text())
after=call('/health');assert not after['runtime_evidence']['parent_source_changes']
...
assets={}
for name in ['quantity.js','requirement-structure.js','requirement-source.js','interpretations.js','requirements.js','requirements.css']:
