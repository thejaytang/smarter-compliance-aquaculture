...
ROOT=Path(__file__).resolve().parents[2];OUT=Path(__file__).resolve().parent / 'activation-register-cleanup';OUT.mkdir(exist_ok=True)
...
try:
 ...
 # User requested removing PA001/PA004 testing work from the active view.
 # Retain the complete workspace descriptor and its database for recovery.
 with closing(sqlite3.connect(ROOT/'workbench/runtime/workbench.sqlite')) as db:
  db.execute('BEGIN IMMEDIATE')
  reset=[]
  for key,raw in db.execute("SELECT key,data FROM collaboration_objects WHERE kind='workspace'").fetchall():
   w=json.loads(raw)
   if w.get('actor')!='Weijie Tang' or w.get('source_id') not in ('PA001','PA004'):continue
   assert w['base_material']['revision']==0 and not w['base_material'].get('blocks')
   db.execute('INSERT INTO collaboration_objects VALUES(?,?,?)',('retired_test_workspace',key+':'+w['id'],raw))
   db.execute("DELETE FROM collaboration_objects WHERE kind='workspace' AND key=?",(key,))
   reset.append({'source_id':w['source_id'],'material_id':w['material_id'],'retained_workspace':w['id']})
  assert len(reset)==2
  db.commit()
 (OUT/'test-work-reset.json').write_text(json.dumps(reset,indent=2))
finally:
...
assets={}
for name in ['source-workspace.js','markdown-content.js','markdown-content.css','quantity.js','requirement-structure.js','requirement-source.js','interpretations.js','requirements.js','requirements.css']:
