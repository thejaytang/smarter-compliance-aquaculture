import {installGlobalSettings} from './global-settings.js';
'use strict';
...
import {workspaceModules, modulePages, moduleFor, workspaceFor, createNavigation, guardedNavigation} from './shell-navigation.js';
...
const modulePaths={addSources:'M12 5v14M5 12h14',system1:'m8 12 3 3 5-6M5 3h14v18H5z',sourceRecords:'M4 4h16v16H4zM4 9h16M9 9v11',history:'M3 11a9 9 0 1 1 2 7M3 4v7h7M12 7v5l3 2',system2:'M5 3h10l4 4v14H5zM14 3v5h5M8 12h8M8 16h6',system2History:'M3 4h18v5H3zM5 9v12h14V9M9 13h6',sourceWebsite:'M3 5h18v14H3zM3 9h18M7 7h.01M10 7h.01',sourceUpload:'M5 3h10l4 4v14H5zM12 17V10m-3 3 3-3 3 3',sourceDiscovery:'M10 3a7 7 0 1 0 0 14 7 7 0 0 0 0-14m5 12 6 6'};
function moduleIcon(id){return '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="'+modulePaths[id]+'"/></svg>';}
let railCollapsed=false;
try{railCollapsed=navigationStorage?.getItem('workbench-sidebar-collapsed')==='true';}catch{}
function renderRail(){
 document.body.classList.toggle('sidebar-collapsed',railCollapsed);
 const toggle=$('#toggle-navigation');
 toggle.setAttribute('aria-expanded',String(!railCollapsed));
 toggle.setAttribute('aria-label',railCollapsed?'Expand navigation':'Collapse navigation');
 toggle.title=railCollapsed?'Expand navigation':'Collapse navigation';
}
$('#toggle-navigation').onclick=()=>{railCollapsed=!railCollapsed;renderRail();try{navigationStorage?.setItem('workbench-sidebar-collapsed',String(railCollapsed));}catch{}};
renderRail();
...
async function refresh(render=true){
  ...
  $('#operator-name').textContent=state.actor.name||'Settings';
  $('#operator-menu-label').title=state.actor.name ? state.actor.name+' · Reviewer and settings' : 'Settings';
...
function renderNavigation(){
 ...
 const activeModule=moduleFor(view),signature=workspace+':'+activeModule;
 if(host.dataset.navigation!==signature){host.innerHTML=workspaceModules[workspace].map(([id,label])=>{
  const children=modulePages[id],expanded=id===activeModule;
  return '<div class="navigation-group"><button type="button" data-view="'+id+'" '+(children?'data-module="'+id+'" aria-expanded="'+expanded+'" aria-controls="subnav-'+id+'" ':'')+'title="'+label+'" aria-label="'+label+'">'+moduleIcon(id)+'<span class="navigation-label">'+label+'</span>'+(children?'<span class="navigation-chevron navigation-label" aria-hidden="true">'+(expanded?'⌄':'›')+'</span>':'')+'</button>'+(children?'<div id="subnav-'+id+'" class="navigation-children" role="group" aria-label="'+label+' pages" '+(expanded?'':'hidden')+'>'+children.map(([page,title])=>'<button type="button" data-view="'+page+'" data-subpage title="'+title+'" aria-label="'+title+'">'+moduleIcon(page==='addSources'?'sourceWebsite':page)+'<span class="navigation-label">'+title+'</span></button>').join('')+'</div>':'')+'</div>';
 }).join('');host.dataset.navigation=signature;}
 document.querySelectorAll('[data-view]').forEach(button=>{button.disabled=!state||navigating||switchingActor;const parent=button.dataset.module,current=parent?parent===activeModule:button.dataset.view===view;button.classList.toggle(parent?'active-section':'active',current);if(current)button.setAttribute('aria-current',parent?'location':'page');else button.removeAttribute('aria-current');});
 const title=Object.values(modulePages).flat().find(([id])=>id===view)?.[1]||Object.values(workspaceModules).flat().find(([id])=>id===view)?.[1]||{settings:'Confidence settings',system2QA:'Weekly checks',assessments:'Source machine assessments',legacyReview:'Legacy extraction review',legacyHistory:'Legacy review history'}[view]||'Workbench';
...
const globalSettings=installGlobalSettings({api,getState:()=>state,canLeave:()=>sources.canLeave()&&materials.canLeave(),onSynced:()=>refresh(true),notice});
