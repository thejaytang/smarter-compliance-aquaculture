...
def collaboration_apply(config_path: Path, request: dict) -> dict:
    ...
    """
    if request.get('actor') != COORDINATOR or request.get('contributor') not in CONTRIBUTORS:
    ...
    with exclusive_process_lock(cfg['log_root'] / '.system1-run.lock'), u.registry_lock(cfg, 12):
        ...
        try:
            ...
            wb._governance_actor = COORDINATOR
            ...
            if base != review_document(source,cfg):
            ...
            operation = {'source_id': source_id, 'operator': COORDINATOR,
            ...
            for issue_row, record, payload in issue_ops:
                ...
                for report in reports:
                    ...
                    if review['issue_checks'].get(key)==issue_digest:
                        ...
                        changed=True;report['resolved']=True;report['resolution']={'actor':COORDINATOR,'contributor':request['contributor'],'submission_id':submission,'note':review['note'],'at':current_time.isoformat()}
            ...
            receipt = {'status': 'saved_partial' if reasons else 'applied', 'request_id': rid,
                       ...
                       'submission_id': submission, 'contributor': request['contributor'], 'actor': COORDINATOR}
            ...
            h.append_operation(ops, oh, {'operation_id': opid, 'operation_type': 'SOURCE_REVIEW',
                ...
                'decision': 'PENDING' if reasons else 'ACCEPT', 'operator': COORDINATOR,
