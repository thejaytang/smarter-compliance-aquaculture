...
def intake(config_path, request):
    ...
    import shutil
    from .workbench_bridge import digest
    if request.get('actor') != 'Weijie Tang': raise ValueError('Only the coordinator may stage source intake.')
    allowed={'request_id','actor','source_title','official_url','retrieval_url','issuer','version','folder_code','file_format','note','upload_path','upload_hash','upload_root'}
    if set(request)-allowed: raise ValueError('Unsupported intake fields.')
    rid=str(uuid.UUID(request['request_id'])); opid='INTAKE-'+rid
    fields={k:str(request.get(k) or '').strip() for k in ('source_title','official_url','retrieval_url','issuer','version','folder_code','file_format','note')}
    ...
    with exclusive_process_lock(cfg['log_root']/'.system1-run.lock'),u.registry_lock(cfg,12):
        ...
        try:
            ...
            for existing_id, row in index.items():
                ...
                if same_file or same_link:return {**old['intake_receipt'],'existing_candidate':True}
            ...
            for row in range(int(cfg['data_start_row']),ws.max_row+1):
                ...
                if fields['official_url'] and fields['official_url'].rstrip('/').lower()==str(record.get('official_url') or '').rstrip('/').lower():
                    ...
                    break
            payload={**fields,'candidate_origin':'HUMAN_WEB','operator_selection_decision':'PENDING','request_digest':digest(request),'requested_by':request['actor'],'duplicate_source_id':duplicate,'version_of_source_id':version_of}
