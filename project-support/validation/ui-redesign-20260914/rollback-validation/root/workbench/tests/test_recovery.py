...
import tempfile
import unittest
from local_workbench.recovery import backup, restore, verify, digest, extract_code, SOURCE_REQUIRED
...
class RecoveryTests(unittest.TestCase):
    def setUp(self):
        ...
        with sqlite3.connect(self.root/self.stores[2]) as db:
    ...
    def test_leader_store_is_a_locked_snapshot_and_jobs_store_is_optional(self):
        ...
        with sqlite3.connect(leader) as db:
            db.execute('BEGIN IMMEDIATE')
            with self.assertRaisesRegex(ValueError, 'database is busy'): backup(self.root, self.package)
        self.assertFalse(self.package.exists())
    ...
    def test_wal_store_verification_never_creates_sidecars_in_package(self):
        with sqlite3.connect(self.root/'system2/runtime/workflow/workflow.sqlite') as db:
        ...
        self.addCleanup(db.close)  # Keep committed WAL-backed source live during backup.
        backup(self.root, self.package)
        ...
        with closing(sqlite3.connect((restored/'system2/runtime/workflow/workflow.sqlite').as_uri()+'?mode=ro&immutable=1', uri=True)) as db:
    ...
    def test_personal_collaboration_store_and_assets_are_consistent_snapshots(self):
        ...
        with closing(sqlite3.connect((self.package/'files'/relative).as_uri()+'?immutable=1', uri=True)) as snapshot:
    ...
    def test_restored_personal_runtime_rebases_without_rewriting_historical_paths(self):
        ...
        personal.mkdir(parents=True)
        with closing(sqlite3.connect(personal/'workflow.sqlite')) as db:
        ...
        with closing(sqlite3.connect((actual/'workflow.sqlite').as_uri()+'?mode=ro',uri=True)) as db:
            self.assertEqual(db.execute('SELECT text FROM history').fetchone()[0],'preserved reviewer text')

    def test_truncated_mapping_cannot_become_empty_fingerprint(self):
        from unittest.mock import patch
        from contextlib import nullcontext
        path = self.root/'resource.bin'; path.write_bytes(b'protected content')
        with patch('local_workbench.recovery.mmap.mmap',return_value=nullcontext(b'')):
    ...
    def test_empty_resource_uses_valid_empty_hash_without_mapping(self):
        from unittest.mock import patch
        import hashlib
        path=self.root/'empty.bin'; path.write_bytes(b'')
        with patch('local_workbench.recovery.mmap.mmap',side_effect=AssertionError('Empty mapping forbidden')):
