...
## UI redesign candidate (2026-09-14)

The current redesign is being verified in an isolated candidate. Activation status and the exact opening/recovery location are recorded in [the execution checklist](../project-support/ui-redesign-20260914/execution.md). The older local-delivery-rc1 examples below are retained historical evidence, not the redesigned runtime status.

Use the compact top row to choose **Sources** or **Materials**. Sources contains **Add sources**, **Source review**, **Source register**, and **Review history**. Materials contains **Material review** and **Archive**. Collaboration and the named operator remain in the same row; runtime, settings and exit are in the operator menu.

- Add sources accepts a source URL or an original PDF, HTML or Excel file and editable metadata. Adding creates a pending candidate. Automatic search explicitly remains unavailable without a connected discovery provider.
...
- Save (also Cmd/Ctrl+S in material review) is separate from completion. Archive explicitly previews/adopts personal changes when required and then separately confirms content. Content-only archives display **Content finalized · Requirements unfinished**. Requirements remains **Not connected**.
