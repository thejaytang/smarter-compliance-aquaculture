...
ranked before PDF. Missing fields remain pending at the named-human gate. The
hidden Human Operation Desktop stores adapter tasks; new candidate acceptance
currently uses the maintainer-assisted compatibility path.
"""
...
def existing_hashes(config: dict[str, Any], candidates: list[dict[str, Any]]) -> set[str]:
    ...
    if config.get('governance_db'):
        ...
        hashes.update(str(entry['record'].get('content_hash') or '').strip().lower()
                      for entry in GovernanceStore(config['governance_db']).snapshot()['sources'])
        hashes.discard('')
        return hashes
    # Normal mode is required because some valid workbooks expose no max_row
    # through openpyxl's streaming reader.
    workbook = updater.open_registry(config)
