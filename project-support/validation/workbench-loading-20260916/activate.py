...
info=layout(ROOT);backup=OUT/'activation-backup';backup.mkdir(exist_ok=True);assert not any(backup.iterdir())
call('/api/stop',{},csrf)
for _ in range(300):
...
assets={}
for name in ['site-catalog.js','app.js','check-design.js','interpretations.js','four-pane.css','materials.js','requirements.js','global-settings.js']:
