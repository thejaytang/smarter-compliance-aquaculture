...
ROOT=Path(__file__).resolve().parents[2];OUT=Path(__file__).resolve().parent / 'activation';OUT.mkdir(exist_ok=True)
...
def logical(path):
 ...
 return out
records=json.loads((OUT/'activation-backup/manifest.json').read_text())
health={'instance':'before-controlled-restart'}
...
for entry in records:
 ...
 with closing(sqlite3.connect(ROOT/entry['path'])) as db:entry['foreign_key_violations']=db.execute('PRAGMA foreign_key_check').fetchall();entry['integrity']=db.execute('PRAGMA integrity_check').fetchone()[0]
assert all(r['business_unchanged'] and not r['foreign_key_violations'] and r['integrity']=='ok' for r in records)
assets={}
for name in ['materials.js','markdown-content.js','markdown-content.css','quantity.js','requirement-structure.js','requirement-source.js','interpretations.js','requirements.js','requirements.css']:
