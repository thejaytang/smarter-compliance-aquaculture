...
def read(config_path):
    ...
    wb.close()
    assessment_path = cfg['log_root'] / 'source-assessments.sqlite'
...
def main():
    ...
    try:
        if message["command"] == "read":
            ...
            if message['request'].get('actor') != 'Weijie Tang': raise ValueError('Only the coordinator may apply source operations.')
            ...
            cfg = u.read_config(config)
            result = Assessments(cfg['log_root'] / 'source-assessments.sqlite').policy(message['revision'], message['threshold'])
        elif message['command'] == 'assessment_hold':
            cfg=u.read_config(config)
            result=Assessments(cfg['log_root']/'source-assessments.sqlite').hold(message['source_id'],message['actor'])
            ...
            owner = Assessments(cfg['log_root'] / 'source-assessments.sqlite')
