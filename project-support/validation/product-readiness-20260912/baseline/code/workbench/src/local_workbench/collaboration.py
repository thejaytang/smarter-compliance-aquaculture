...
from .collaboration_exchange import pack, unpack, segment
from .identities import canonical_name, REVIEWERS
...
class PackageSourceAdapter:
    ...
    def call(self, command, **kwargs):
        if command == 'read':
            ...
            return {'sources': sources, 'tasks': [], 'history': [], 'revision': fingerprint(sources),
                'authority': {'kind': 'offline_review_package', 'revision': 0},
                'counts': {'sources': len(sources), 'tasks': 0,
...
class Collaboration:
    def __init__(self, app, reviewer=False):
        ...
        self.root = app.runtime / 'collaboration'
        self.root.mkdir(exist_ok=True)
        self.database = app.runtime / 'workbench.sqlite'
    ...
    def get(self, kind, key, default=None):
        with self.db() as db:
            row = db.execute('SELECT data FROM collaboration_objects WHERE kind=? AND key=?', (kind, key)).fetchone()
        return json.loads(row[0]) if row else default

    def put(self, kind, key, value):
        with self.db() as db:
            db.execute('INSERT INTO collaboration_objects VALUES(?,?,?) ON CONFLICT(kind,key) DO UPDATE SET data=excluded.data',
    ...
    def all(self, kind):
        with self.db() as db:
            return [json.loads(row[0]) for row in db.execute('SELECT data FROM collaboration_objects WHERE kind=? ORDER BY rowid', (kind,))]

    def coordinator(self, actor):
        if self.mode != 'coordinator' or named(actor) != COORDINATOR:
            raise ValueError('Only the main workspace coordinator can adopt or confirm main versions.')

    def state(self, actor=None):
        return {'mode': self.mode, 'coordinator': COORDINATOR,
            ...
            'can_adopt': self.mode == 'coordinator' and actor == COORDINATOR}

    def source_records(self):
        if self.mode == 'coordinator':
            return self.app.adapter.call('read')['sources']
        return [item['source'] for item in self.all('source_catalog')]
    ...
    def source_issues(self, source_id):
        ...
        result = []
        for task in self.app.adapter.call('read').get('tasks', []):
    ...
    def _archive(self, identity, blob):
        ...
        folder = self.root / 'packages'; folder.mkdir(exist_ok=True)
    ...
    def prepare_own(self, actor, request):
        ...
        with self.lock:
            metadata = self._submission(actor, request)
            self._import_submission(metadata)
            return self.preview(actor, {'submission_id': metadata['id']})
    ...
    def merge_view(self, merge):
        ...
        result = merge_documents(base, current, proposed, merge['decisions'])
        if merge.get('kind') == 'machine' and current['blocks']:
            ...
            if required:
                result = merge_documents(base, current, proposed, {**merge['decisions'], **{key: {'action': 'current'} for key in required}})
                for difference in result['differences']:
                    if difference['id'] in required:
                        difference.update(conflict=True, resolution=None, kind='machine_difference')
    ...
    def adopt(self, actor, request):
        ...
        with self.lock:
            ...
            if merge.get('source_request') and not merge.get('source_receipt'):
                result = self.app.adapter.call('collaboration_apply', request=merge['source_request'])
            ...
            if incoming['actor'] == actor:
                for kind, key in [('workspace', actor + ':' + str(mid)), ('source_draft', actor + ':' + sid)]:
                    ...
                    if old:
                        ...
                        with self.db() as db:
                            db.execute('DELETE FROM collaboration_objects WHERE kind=? AND key=?', (kind, key))
            return receipt
    ...
    def machine_preview(self, actor, request):
        ...
        merge = {'id': identity, 'actor': actor, 'kind': 'machine', 'submission_id': identity,
            ...
            'current_source': source, 'master_digest': fingerprint(material), 'source_digest': fingerprint(source),
            'decisions': {}, 'status': 'preview', 'at': now(), 'candidate_id': candidate['id'],
            'candidate_digest': fingerprint(candidate), 'input_revision': candidate['input_revision'],
