...
Current browser collaboration: save source ratings, field corrections and issue findings as personal proposals in the material workbench. Export or inspect the personal submission; Weijie Tang explicitly adopts it into the governed source store. A saved proposal is not an effective source decision. See [offline workflow](../docs/design/offline-human-review.md). Earlier direct-operation descriptions below remain relevant to the owning compatibility/audit mechanism, not a bypass of this personal-adoption workflow.
