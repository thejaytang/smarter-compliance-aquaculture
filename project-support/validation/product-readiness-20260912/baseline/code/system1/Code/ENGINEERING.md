...
## Database authority and one-way output

Normal activation status belongs to [System1 state](../PROJECT_STATE.md). The explicit local `governance_db` key is resolved relative to the configuration file, like other owned paths. Its absence retains the legacy workbook authority. Setting it never imports Excel implicitly: a verified, explicit `GovernanceStore.migrate` rehearsal and controlled transition must precede configuration changes.

`governance_store.py` owns source and operation rows, revision-guarded commits, append-only history and immutable artifact/version identities. Its matching `*-migration-input.xlsx` companion is immutable layout/import evidence, required for restoration. Normal derived Excel is never an input in this mode; `sync_run_state` refuses workbook readback. Backups publish an independent database plus the verified companion. Preserve originals, assessment history and workbench receipts with the database recovery package.

Existing domain operations use a lean in-memory business view over a fresh database snapshot. Only immutable template metadata is cached; the full workbook presentation is reconstructed separately for export. Historical record positions cannot be reordered through this view. Data files retain old version paths after replacement; source identity and file hashes cannot be rewritten. These implementation constraints do not alter human ownership, source selection rules or the provisional B policy.

`governance_export.py` owns a coherent, protected, cached Excel snapshot. Separate `.excel.json` and `.excel-worker.json` markers distinguish saved revision, exported revision and failure. The workbench worker coalesces changes and retries locks/failures; saving a database decision does not wait for this writer. System1 exports include logical assessment-store versions. System2 schema-2 handoffs explicitly identify a database state digest with `registry_kind=sqlite_snapshot`; old workbook contracts remain compatible. See [the normal transition and recovery evidence](reports/2026-09-10-governance-database-transition.md) and [the earlier isolated rehearsal](reports/2026-09-10-governance-database-rehearsal.md).
...
- Database writes use an owning lock and revision guard. Generated workbook replacement checks the Excel temporary lock independently.
- Controlled database writes retain verified backups and transactional history. Generated files use temporary-file replacement.
- Excel file occupation defers output safely; it does not block database decisions.
...
`python -m system1 doctor` checks Python/dependency versions, configuration and relative paths, owning database integrity/schema/immutable companion, derived workbook and `Data`, Excel lock, timezone, runtime writability, Windows-compatible paths, schedule configuration and scheduler registration. Missing or altered database recovery components fail. An open or missing derived Excel is a synchronization warning in database mode; diagnosis never imports Excel or creates a missing business database.
...
- `runtime/backups`: verified database backups with immutable companions, plus retained historical recovery workbooks
...
Every `save_workbook_atomic` now calculates the workbook's supported formula vocabulary before saving, writes cell and chart result caches into the temporary file, validates the ZIP, rechecks workbook modification time and Excel-open state, and only then promotes the file. Formula expressions, audit rows, locks and backups remain intact. A successful background publication therefore includes immediately readable workbook results; users do not have to open Excel to populate caches. Database receipt success does not wait for this publication.
