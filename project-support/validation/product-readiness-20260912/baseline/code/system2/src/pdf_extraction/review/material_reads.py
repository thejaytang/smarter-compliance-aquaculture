...
def candidate_summary_sql(expression):
    ...
    return f"json_set({removed},'$.block_count',json_array_length({expression},'$.blocks'),'$.warning_count',json_array_length({expression},'$.warnings'),'$.warnings',json({warnings}))"
...
class MaterialReads:
    def ensure_read_indexes(self, db):
        ...
        ''')
        def ready():
            return db.execute('SELECT 1 FROM material_read_schema WHERE version=4').fetchone() and all(
        ...
        db.execute('INSERT OR IGNORE INTO material_read_schema VALUES(4)')
