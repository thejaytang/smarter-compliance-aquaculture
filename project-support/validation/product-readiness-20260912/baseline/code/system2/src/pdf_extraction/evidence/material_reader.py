...
def _html(raw):
    ...
    return {'html': output, 'anchors': anchors, 'warnings': warnings, 'label': 'Complete bound HTML snapshot · offline reading view'}
...
def _pdf(raw, page):
    ...
    with pdfium.PdfDocument(raw) as doc:
        ...
        with closing(doc[page - 1]) as current:
            ...
            bitmap = current.render(scale=min(1.5, 1800/max(width, height)))
            try:
                buf = BytesIO()
...
def read_material(path, expected_hash, *, sheet=None, row=1, column=1, row_count=80, column_count=24, page=1):
    options = dict(sheet=sheet, row=row, column=column, row_count=row_count, column_count=column_count, page=page)
    def produce(raw):
        ...
        elif result['kind'] == 'pdf': result.update(_pdf(raw, page))
        return result
    return _cached(path, expected_hash, options, produce)
