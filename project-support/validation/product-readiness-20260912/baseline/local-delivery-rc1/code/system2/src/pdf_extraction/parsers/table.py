...
def _merged_cell_specs(
    ...
    gray = cv2.cvtColor(image, cv2.COLOR_RGB2GRAY)
    # The grid detector already accepts faint/color rules. Absolute blackness
    # would then erase those same boundaries and merge the entire table.
...
def _compact_grid_specs(specs, xs, ys):
    """Remove latent grid divisions crossed by every cell, retaining rectangles.

    Shaded-header threshold halos can propose a line that no inferred cell
    actually ends at. Its row/column index has no source boundary to represent.
    Genuine partial merges retain the divisions used by other cells.
    """
    rows = sorted({0, len(ys)-1, *[i for r, c, rs, cs in specs for i in (r, r+rs)]})
    columns = sorted({0, len(xs)-1, *[i for r, c, rs, cs in specs for i in (c, c+cs)]})
    ri = {old: new for new, old in enumerate(rows)}
    ci = {old: new for new, old in enumerate(columns)}
    return ([(ri[r], ci[c], ri[r+rs]-ri[r], ci[c+cs]-ci[c]) for r,c,rs,cs in specs],
            [xs[i] for i in columns], [ys[i] for i in rows])
...
def _uniform_cell_interior(image: np.ndarray) -> bool:
    """A narrowly excluded ruling edge and uniform pixels prove a blank crop.

    Only used when page/native word evidence is also absent. Any interior ink,
    including a lone scanned digit or negation mark, retains the OCR path.
    This evidence selection is not a calibrated confidence or release score.
    """
    height, width = image.shape[:2]
    pad = min(4, height // 10, width // 10)
    if pad < 1:
        return False
    interior = image[pad:-pad, pad:-pad].astype(np.int16)
    return bool(interior.size and np.max(np.ptp(interior, axis=(0, 1))) <= 2)
...
class TableParser:
    ...
    def parse(
        ...
        cell_specs, xs, ys = _compact_grid_specs(cell_specs, xs, ys)
        for row, column, row_span, column_span in cell_specs:
                ...
                text, confidence = _text_for_words(words)
                cell_pixels = image[local.y0:local.y1, local.x0:local.x1]
                visually_blank = not text and _uniform_cell_interior(cell_pixels)
                if not text and not visually_blank:
                    cell_crop = Image.fromarray(cell_pixels)
                ...
                content = TextContent(
                    ...
                    resolution=Resolution(
                        ...
                        reason=("table_cell_ocr" if text else
                                "blank_raster_and_no_word_evidence" if visually_blank else "empty_cell"),
                        confidence=confidence,
                    ),
                )
                cells.append(
