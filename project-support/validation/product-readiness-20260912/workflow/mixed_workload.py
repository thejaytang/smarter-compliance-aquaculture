...
class Workload:
    ...
    def faults(self):
        ...
        assert current['blocks'] == m['blocks'], 'Worker recovery replaced human blocks'
        if candidate['status'] == 'kept':
            full = adapter.call('material_candidate', material_id=mid, candidate_id=candidate['id'])
            reuse = full.get('prior_resolution', {})
            prior = adapter.call('material_candidate', material_id=mid, candidate_id=reuse['prior_candidate_id'])
            assert reuse['candidate_id'] == candidate['id'] and prior['id'] != candidate['id']
            assert prior['status'] in ('kept', 'adopted', 'merged')
            assert prior['adoption']['reviewed_against_source'] is True and prior['adoption']['actor']
            assert reuse['adoption'] == prior['adoption']
            assert prior['adoption']['result_content_revision'] == current['content_revision'] == m['content_revision']
            assert full['base_blocks'] == current['blocks'] and full['source'] == current['source']
            assert all(full.get(k) == prior.get(k) for k in ('source', 'scope', 'blocks', 'complete', 'warnings', 'error'))
            self.record('verify_prior_human_resolution_reuse', candidate_id=candidate['id'], prior_candidate_id=prior['id'])
        else:
            assert candidate['status'] in ('ready', 'partial'), candidate['status']
...
def main():
    parser=argparse.ArgumentParser();parser.add_argument('--run-name',required=True);parser.add_argument('--label',default='development mixed workload');parser.add_argument('--require-code-stable',action='store_true');parser.add_argument('--only-group',choices=['machine_order_content_resume','worker_and_save_faults']);args=parser.parse_args()
    ...
    if not run.failures:
        ...
        if 'TS001' in run.materials:
            ...
            if not args.only_group or args.only_group == 'worker_and_save_faults':
