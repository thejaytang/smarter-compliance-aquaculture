...
## Active goal extension: product readiness, 2026-09-12

The user explicitly activated the outcome-driven product-readiness goal in task `01a09761-a69b-7640-b7a2-8e399ac01cc3`. **Scope revised by explicit user instruction on 2026-09-13: stop further recognition/extraction accuracy optimization and prioritize accurate, safe human review.** Extraction and recognition are aids; future machine-learning improvement will use human annotations. This revision supersedes the earlier automatic-quality targets, including their wording in the already-created goal record. Historical measurements and failures remain evidence, not retroactive passes. Current integration status remains in [PROJECT_STATE.md](../../PROJECT_STATE.md).

Deliver three independently evidenced outcomes: (1) safe, complete human material processing and recovery, (2) a consistent, restrained translucent workbench using UI UX Pro Max and Impeccable, and (3) trustworthy review evidence: clear original/version binding, explicit human decisions, remaining uncertainty and durable revision history. Priority: originals and human work, accurate human review and workflow completion, repairability, efficiency, then appearance. A colleague must be able to find a material, read, explicitly extract if useful, compare, correct, save/resume, explicitly confirm and exchange work without a developer. The system supports human accuracy; engineering does not certify the correctness of real business judgments.

### Required acceptance

- Establish actual action-to-program wiring, current loaded state, ranked failures and a verified pre-change preservation baseline. Preserve pre-existing uncommitted/untracked work. Freeze samples, windows, task scripts, metric definitions, versions and development/holdout roles before modifying the relevant behavior.
- Exercise all ten actual-UI scenarios: eligible unstarted material; truthful extraction state and original location; text/numbering/hierarchy/order/table correction and reopen; refresh/close/restart resume; re-extraction with prior edits and explicit comparison; personal save versus master adoption versus content and Requirement confirmation; archive re-review with history; work-package exchange/conflicts/receipts; repeated requests and stale tabs; extraction/partial/empty/missing-original/lock/save failures. Record preconditions, expected outcomes, observations and evidence. API success alone is not UI acceptance.
- Keep one material workspace with Pending materials and Archive, three adjustable panes, full-screen expansion, clear material/version/selection, remaining review scope and recoverable errors. Reads, navigation and saving never start downstream work. No silent draft overwrite, wrong-version application, lost work, false success or ambiguous review status is acceptable.
- Glass belongs to navigation, toolbars and floating controls. Original text, tables and editors need opaque readable surfaces. Use local assets, consistent semantic tokens, restrained motion and solid fallbacks. Verify 1280/1440/1920 widths, recorded browser zoom, keyboard/focus, normal text contrast >=4.5:1, non-color status, reduced motion/transparency and any existing dark mode. Preserve matched before/after captures. One concentrated visual inspection, one correction batch and one confirmation precede behavior/quality validation; avoid decorative iteration.
- Distinguish source extraction, Requirement identification and full semantic processing. Keep assistance explicit and separate from human confirmation. Do not further tune accuracy, add a classifier adapter, expand automatic semantic processing or invent business definitions under this goal. The third pane remains Not connected until a separately approved schema and capability exist. Site Model, automatic compliance judgment and model training remain outside current scope.
- Retain existing frozen samples, annotations, denominators and exposure history as engineering evidence. Use bounded representative sources for manual-workflow and regression checks. Do not expand annotation/holdout programs or run additional accuracy experiments in this phase.
- The former >=95% source recall, >=90% structural fidelity and >=90% Requirement precision/recall targets are deferred, not passed or lowered by measurement changes. Automatic misses must not be mistaken for completed human review: show the original independently of candidates, keep unchecked source ranges visible, and support manual correction. Preserve source/version/location, reviewer identity, revisions, decisions and uncertainty using existing review records. Unreviewed or omitted content is not a negative training label; accepted machine text is not automatically expert Gold. Future training-data schema, adjudication and export are separate work, with no model training or external transmission now.
- Distinguish success, partial, empty, failed and unsupported extraction; show completed/unprocessed scope. Retain original/Canonical bytes, immutable attempts, overlays and decisions across retries. Manual correction is recovery, not automatic-quality acceptance.
- Freeze a versioned candidate, run required regression and risk-based isolated mixed-operation stability/recovery tests. Predefine materials, operation cycles, concurrency, interruptions, invariants and resource observations. Cover edit/save/reopen, extraction/comparison, import/export/replay, stale tabs/locks, restart and retry. Record operation counts and elapsed time as evidence, never as the pass criterion. Idle running is not validation; bounded local evidence does not establish multi-day or Windows stability.

### Execution, boundaries and stopping

Use rolling stages: preservation/current behavior; one real-material manual end-to-end path; reader/correction/review-state/recovery fixes; actual-browser verification and required regression; candidate freeze and handoff. No fixed work duration or deadline. Finish necessary regression and documentation for changes already made, then stop new accuracy work. After three attempts without new evidence or measurable improvement, re-diagnose or retain a precise gap and continue independent work. Stop adding changes when all applicable gates pass; preserve resumable state and accurately report remaining dependencies.

Local reversible code/UI work and proportionate isolated tests are authorized. All write/recovery/conflict/collaboration tests use isolated business data. Do not perform real colleague reviews, change formal business master versions because tests pass, upload/send/publish/push, add schedules/synchronization, or activate paid/external services. Existing original, Canonical, Gold, history and human-authority protections remain binding.

Report each goal and required subgate as PASS, FAIL, UNMEASURED or BLOCKED with evidence. Distinguish implementation, isolated verification, actual loaded environment/version and local-trial readiness. Real colleague usability and actual Windows round trips remain separate acceptance gates. Handoff includes runnable location/entry/version, changes per goal, matched UI evidence and one task example, sample comparisons and unmeasured scope, functional/recovery/regression/stability evidence, limitations and top three colleague checks, restore instructions and continuation entry. Update existing owning state files, with bounded supporting evidence under `project-support/product-readiness-20260912/`.

The final question is whether a colleague unfamiliar with development can understand the interface, complete real material work, find and repair errors, preserve their effort, and know what is usable versus still awaiting review.
