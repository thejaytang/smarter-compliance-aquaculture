...
for sample in freeze['samples']:
 annotation=read(ROOT/sample['annotation']['path'])
 if 'segments' not in annotation:continue
 byseg={s['segment_id']:s for s in annotation['segments']};frozen=next(r for r in freeze['reference_denominators'] if r['sample']==sample['id'])
 for backend in ['baseline','guard']:
  directory=(Q/'after-v4-conservative-pdf'/sample['id']) if backend=='baseline' else (OUT/sample['id'])
  if not (directory/'candidate.json').exists():continue
  c=read(directory/'candidate.json');blocks=c['blocks']
  if backend=='guard':
   audit=read(directory/'assembly-audit.json');rawblocks=[b for page in audit for b in page['before']]
   provenance=read(directory/'experiment-provenance.json')
  else:
   lookup={g['group_block_id']:g['fragments'] for g in read(directory/'pdf-paragraph-groups.json')['groups']}
   rawblocks=[f for b in blocks if b['type']!='image' for f in lookup.get(b['id'],[b])]
   provenance={'parser_version':c['parser_version'],'actual_native_backends':['pypdfium2'],'baseline':'Frozen /4; pre-grouping payload reconstructed from retained group members; image objects excluded as in original grouping audits.'}
