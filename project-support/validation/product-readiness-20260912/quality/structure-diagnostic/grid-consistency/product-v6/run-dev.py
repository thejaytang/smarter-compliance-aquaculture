...
if len(sys.argv)>1:
 ...
 assert module.VERSION=='material-structural-parser/6'
 ...
 save(target/'experiment-provenance.json',{'actual_native_backends':['pypdfium2'],'module_policy':module.POLICY,'config_hash':module.CONFIG_HASH,'parser_version':module.VERSION,'production_parser_hash':freeze['code']['material_parser.py'],'loaded_source_hash':hashf(ROOT/'system2/src/pdf_extraction/orchestration/material_parser.py'),'grid_guard':module.POLICY['table_grid_guard'],'all_group_invariants':all(all(a[k] for k in ['fragment_payload_order_exact','input_unmodified','group_text_exact','group_refs_exact']) for a in audits),'business_adoption':False,'docling_imported':False})
...
freeze={'samples':samples,'code':{**original_native_freeze['code'],'material_parser.py':hashf(ROOT/'system2/src/pdf_extraction/orchestration/material_parser.py')},'reference_denominators':reference_denominators,'relation_subset_hash':original_native_freeze['relation_subset_hash'],'base_parser':'Current source parser /6, compared with preserved /4 and isolated guard outputs; /5 HTML fix retained','predeclared_guard':{'tolerance_points':1,'reject_crossing_ratio_at_least':.25},'matching_policy':'Same whitespace-only/page-bound and marker rules; existing real engineering and synthetic annotations, no changed denominator/Gold; DEV calibration, no independent acceptance.'}
save(OUT/'freeze.json',freeze)
assert b"VERSION = 'material-structural-parser/6'" in (ROOT/'system2/src/pdf_extraction/orchestration/material_parser.py').read_bytes()
...
tmp=Path(tempfile.mkdtemp(prefix='smarter-grid-product-v6-',dir='/private/tmp'))
...
save(OUT/'experiment-freeze.json',{'created_utc':datetime.now(timezone.utc).isoformat(),'prechange_v5_parser_sha256':hashf(HERE/'prechange-material_parser.py'),'current_product_parser_sha256_at_start':hashf(ROOT/'system2/src/pdf_extraction/orchestration/material_parser.py'),'scripts':{p.name:hashf(p) for p in [Path(__file__),ROOT/'system2/src/pdf_extraction/orchestration/material_parser.py']},'baseline_candidates':{s['id']:hashf(Q/'after-v4-conservative-pdf'/s['id']/'candidate.json') for s in samples},'predeclared_grid_diagnosis_sha256':hashf(HERE.parent/'summary.json'),'original_manifest_sha256':hashf(Q/'manifest.json'),'native_config_sha256':hashf(ROOT/'system2/config/pdf-intake-positioned.yaml'),'temp':str(tmp),'readiness_guard_seconds':300,'parse_guard_seconds':60,'purpose':'Current /6 >=25% ratio/1pt guard; validation on isolated snapshots, no business data writes. Operational timeouts are not quality targets.'})
