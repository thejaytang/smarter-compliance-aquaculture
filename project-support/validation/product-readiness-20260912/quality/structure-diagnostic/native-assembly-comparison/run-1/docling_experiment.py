...
VERSION = 'material-structural-parser/4+dev-native-docling'
POLICY = {'adapter': VERSION, 'semantic_processing': False, 'external_models': False, 'ocr': False, 'pdf_window_pages': 3, 'pdf_backend': 'docling-parse', 'pdf_marker_order': 'same-row-before-body/1', 'pdf_paragraph_grouping': 'same-page-source-preserving/1', 'human_adoption_required': True}
...
def _pdf_blocks(raw, source, output, page_indices):
    ...
    warnings.append('EXPERIMENT: PDF candidate uses the installed native docling-parse parser in mapped three-page windows. Ruled-table grids use existing local geometry assembly. Reading order, headings and unruled tables may need manual correction; source page images and complete native evidence are retained. No Requirement classifier, semantic processor, OCR model or external service runs.')
