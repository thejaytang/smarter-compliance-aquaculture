...
def main():
 ...
 with TemporaryDirectory(prefix='ui-r7-fixture-') as temp:
  ...
  server=ThreadingHTTPServer(('127.0.0.1',args.port),Fixture);server.app=SimpleNamespace(reviewer=False,csrf='r7-fixture',store=SimpleNamespace(session=lambda token:{'id':'fixture','name':ACTOR,'token':'fixture'}),collaboration=c)
  server.info={'material':material,'units':units,'sessions':sessions,'csrf':'r7-fixture'};server.material=material;server.original=deepcopy(material)
  print(f'R7 isolated recovery: http://127.0.0.1:{server.server_port}/',flush=True)
