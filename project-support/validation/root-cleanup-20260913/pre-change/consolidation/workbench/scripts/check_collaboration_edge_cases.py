...
Run: PYTHONPATH=workbench/src workbench/.venv/bin/python workbench/scripts/check_collaboration_edge_cases.py
...
'''
    subprocess.run([str(ROOT/'system1/Code/.venv/bin/python'),'-c',program,str(CONFIG)],
        input=json.dumps({'source_id':source_id,'updates':updates}), text=True, check=True,
        cwd=ROOT,env=dict(os.environ,PYTHONPATH=str(ROOT/'system1/Code/src')),capture_output=True)
