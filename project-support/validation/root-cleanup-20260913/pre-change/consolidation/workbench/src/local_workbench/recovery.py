...
def digest(path):
    # Read-only mappings expose the complete descriptor length, avoiding an
    # anomalous premature EOF being accepted as the hash of an empty file.
    with path.open('rb') as stream:
        before = os.fstat(stream.fileno())
        if before.st_size:
            with mmap.mmap(stream.fileno(), 0, access=mmap.ACCESS_READ) as content:
                if len(content) != before.st_size:
                    raise OSError('Incomplete recovery resource mapping: ' + str(path))
        ...
        # Content identity is checked before/copy/after by backup. Timestamp-only
        # metadata changes must not be misreported as changed content.
        return result

def digests(paths):
    paths = list(paths)
    with ThreadPoolExecutor(max_workers=8, thread_name_prefix='recovery-read') as pool:
        return dict(zip(paths, pool.map(digest, paths)))
...
def quiescent(root, info):
    with ExitStack() as stack:
        ...
        if list((root/'system2/runtime/workflow').glob('~$*.xlsx')):
            raise ValueError('Close the System2 Excel register before backup.')
        marker = root / 'workbench/runtime/server.json'
        if marker.exists():
            state = json.loads(marker.read_text())
            if process_alive(state['pid']): raise ValueError('Stop the workbench and wait for all processing to finish before backing up.')
        locks = [root/'workbench/runtime/service.lock', info['logs']/'.system1-run.lock',
            info['workbook'].parent/'.system1-source-management.lock',
            root/'system2/runtime/workflow/.worker.lock', root/'system2/runtime/workflow/.material-worker.lock']
        locks.extend(path.parent/'.material-worker.lock' for path in info['stores']
            if path.is_relative_to(root/'workbench/runtime/collaboration/personal'))
        for path in locks:
...
SOURCE_DECLARATIONS = (
    'ENVIRONMENT.md', 'README.md', 'AGENTS.md', 'Open Workbench.command', 'Open Workbench.cmd', 'Open Reviewer Workbench.cmd',
...
def source_inventory(root):
    ...
    for relative in ('system1/Code/src', 'system2/src', 'workbench/src', 'workbench/ui', 'system2/config/schemas'):
        for code in (root/relative).rglob('*'):
            if code.is_file() and code.suffix in SOURCE_SUFFIXES and not any(part.startswith('.') or part in {'__pycache__', 'node_modules'} for part in code.relative_to(root).parts):
                files.add(code)
    return sorted(files)
...
def backup(root, destination):
    ...
    with quiescent(root, info):
        ...
        manifest = {'format': 'workbench-recovery-v2', 'status': 'incomplete', 'created_at': datetime.now(timezone.utc).isoformat(),
        ...
        # Every source remains protected by the same writer reservations. Files
        # have disjoint destinations; all worker errors fail this package closed.
        with ThreadPoolExecutor(max_workers=8, thread_name_prefix='recovery-copy') as pool:
            manifest['files'] = dict(pool.map(copy_resource, files))
...
def verify(package):
    ...
    if manifest.get('format') not in {'workbench-recovery-v1', 'workbench-recovery-v2'} or manifest.get('status') != 'complete': raise ValueError('Recovery package is incomplete or unsupported.')
...
def serve_restored(destination, code_root):
    ...
    for component in ('system1/Code', 'system2'):
    ...
    from .adapter import System2
    from .runtime_status import ObservedAdapter
    app = Application(destination/'workbench', system_root=code_root/'system1',
    ...
    marker = destination/'workbench/runtime/server.json'
