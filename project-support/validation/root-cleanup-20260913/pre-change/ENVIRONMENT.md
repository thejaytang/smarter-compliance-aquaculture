...
The optional standalone [System2 API module](system2/src/pdf_extraction/api/app.py) creates its default `runtime/jobs.sqlite3` at import time, relative to the current working directory. Do not import that module from the workstream root merely to check installation; use the package-only verification below. Run API/worker operations from `system2/` with the intended database path. This root has no owning job store; `system1/Code/runtime/`, `system2/runtime/` (including `workflow/`) and `workbench/runtime/` retain their component ownership. An explicit later database argument does not undo the module's earlier default-app import. This guidance avoids accidental root imports; it is not a code-level prevention of recurrence. Never treat all runtime files as disposable caches. The [retained maintenance evidence](docs/reports/stage-goal-v2-maintenance-evidence.json) records the narrowly authorized empty-root-store cleanup.
