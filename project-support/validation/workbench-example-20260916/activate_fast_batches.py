...
ROOT=Path(__file__).resolve().parents[2];OUT=Path(__file__).resolve().parent / 'activation-fast-batches';OUT.mkdir(exist_ok=True)
...
assets={}
for name in ['markdown-content.js','markdown-content.css','pane-layout.js','requirement-source.js','version-comparison.js','interpretations.js','materials.js','requirements.js']:
