...
## Interaction language (current, 2026-09-17)

The editor has one interaction model for saved and unfinished Requirements. Expand an Rx and edit directly; only stale/read-only sources and requests in progress lock editing. The top source text is the annotation surface. Nested cards show the resulting structure, rather than repeating an editable-looking copy of the source at every level.

| Action | Entry point | Result |
|---|---|---|
| Expand / Collapse | Rx header or Group header; keyboard on its disclosure | Changes the view only. Source selection and action buttons never toggle their parent. |
| Mark a field / Group / Relationship | Select source wording; choose the toolbar action | Creates an unsaved source-bound mark. **Add to** names the enclosing Requirement or Group and lets the operator choose an outer scope. Same-field subgroups offer only that field. |
| Split | Field card | Replaces that fragment with an empty same-field Group over the same original span. Fill it by selecting the top source again. |
| Group selected | Above a Group's direct items | Enabled after checking two or more siblings. Wraps those items; select the new quantity explicitly. |
| Ungroup | Eligible nested Group | Keeps its contents. Offered only when both All quantities can be preserved and neither grouping has NOT or Relationship metadata. |
| Link | Exception / Subrequirement selector | Enabled after choosing another Rx. Root selectors stay visible; **Link requirement…** reveals the same selectors within a nested Group. |
| Remove (×) | Top-right of the owning card | Removes that field, link, connector or Group. Group removal includes descendants; link removal preserves the target Requirement. Whole-entry **Remove** asks for confirmation. |
| Save | Single bottom action bar | Saves one revision and keeps the editor open. Incomplete structure may be saved. |
| Save & close | Same action bar | Validates all units and commits one revision atomically, then closes. Failure keeps the draft and editor; the error appears beside the save controls. Closing already-saved complete work creates no new history. |
| Discard | Same action bar, when dirty | Confirms before replacing only this Requirement's unsaved work with its saved version. |

No Resume editing, per-unit Finish, or separate quantity-disclosure action is needed. Quantity is shown above its direct children. Manual range input and disclosure stay in page memory until an explicit save; no autosave history is introduced. The floating toolbar has no removal or ungroup action. Help and saved-history recovery remain under **?**.

## Group relationship (current)

An explicit source Group can retain one optional `relationship`, containing its exact original text and source span. Create a Group covering both parts, select the connector (for example **including**) within it, then choose **Relationship** beside **Group** in the floating toolbar. It is unavailable at the Requirement root. The relationship row appears between the marked parts in source order; the collapsed original retains its colour. Children before and after the connector retain their IDs and nesting. A child crossing the connector must be regrouped first. Unmarked content on either side leaves the Group unfinished.

Relationship is metadata, not a counted child. AND / OR remain in the original text and use the existing quantity controls: All N, Any [1,N], or Only 1 when the source supports exclusivity. Exact AND / OR (including Norwegian og / eller) cannot be assigned as Relationship. Do not infer exclusivity from bare OR. This source annotation does not introduce a Site Model operator or compliance judgment.

Use the relationship row's × to remove the connector annotation while retaining both parts. Removing its Group removes the entire subtree. Ungroup must not silently discard an attached relationship; remove that annotation explicitly first. Single-child implicit All wrappers are omitted from display, with no change to saved IDs or quantities.

Relationship-bearing sessions use `requirement-structure/2`; existing `/1` sessions remain readable. Exact connector spans and the before/after child IDs project into `requirement_group_relationships`, linked to the existing session/node/source lineage. Immutable saved steps remain authoritative. Full reviewer bundles containing this schema use `requirement-delivery/3`; history, import/export, interpretation fingerprints and source projections retain the new field. Old bundles still import. Previews remain page-only until manual Save.
...
The entry header shows Locate, Auto-extract, then Remove; Auto-extract remains disabled with Not connected until processing is available; Remove asks for explicit confirmation and keeps recoverable history. No completion badge is shown there. The whole top row beside Rx expands or collapses the entry, except its three action buttons. The Rx button also works with the keyboard. Clicking coloured original wording never toggles it. One entry is open at a time. Expanded original text is selectable but read-only; selection opens a floating toolbar beside the pointer (or keyboard selection) with two sections: **Group / Relationship** and **Subject / Modal Verb / Main Verb / Object / conditions**. Collapsed source text has no annotation toolbar. Escape or clicking elsewhere dismisses the toolbar. The complete source uses a full-width row in narrow panes.

Group retains existing whole source marks inside its selected range. A boundary through an existing mark must be adjusted or cleared first. Remove a field from the × at the top-right of its card. Removing a Group deletes that Group and all its descendants, while preserving original text and siblings. Ungroup remains on the lower Group module, where it only unwraps the grouping and retains its fields; it is absent from the floating selection toolbar. These edits stay in page memory until explicit Save; saved history remains recoverable. Complete entries reopen as drafts when a card is removed; stale/read-only entries remain protected. Ungroup preserves fields and refuses to discard existing alternative quantities or NOT implicitly. Plain single fields have no QC; explicit combinations retain exact/range QC. Explicit NOT controls and operations are unavailable, including Conditions. Select the complete negative wording (for example, “not installed in the North of Norway”) as an ordinary Condition; no extra operator is inferred. Historical stored NOT remains visibly read-only and retains its recorded projection; no migration rewrites saved history.

**Exception remains an independent relationship.** New Exception and Subrequirement relationships must select another complete Requirement entry (Rx). Its own Scope/Conditions/Demand belong to that referenced entry. It is not automatically negated or merged into the main Scope. Use the two visible Exception and Subrequirement link selectors. Linked Requirements form a vertical list, with Rx and source wording on each row. The top-right × removes only that link, never the referenced entry; clicking its Rx/source opens the target. Each relationship group has quantity controls above its direct children. Select sibling checkboxes and use **Group selected** to create a nested group with its own quantity; the parent counts that group as one child. Nested groups appear inline, and **Ungroup** retains their links only when flattening preserves the quantities. Link, quantity, grouping and removal actions on a completed entry reopen an unsaved draft; explicit Save remains necessary. Local fragments and internal child IDs are not selectable link targets; prior inline content and its exact quantity stay in the Requirements **?** history. Mixed legacy groups keep their quantity there instead of applying it to a partial visible list. Each active relationship has one heading and one dropdown per group. Legacy fragments are not repeated in the daily editor. No compatibility notice appears in the daily editor. The contextual field toolbar excludes both relationship types. A source-defined exception may be incomplete; do not invent a missing demand or deadline. Prior saved records remain readable and unmodified.
...
Adding/removing/wrapping members leaves multi-item QC unresolved until the operator chooses its meaning. Empty groups and unresolved QC prevent Save & close. Legacy exceptions appear as independent exception clause/reference groups without automatic negation; opening a record does not rewrite its history.
