...
class OperatorJourneyTests(unittest.TestCase):
    ...
    def test_human_reported_issue_survives_repeated_queue_refresh(self):
        wb = load_workbook(self.workbook)
        ops = wb[human_operations.HUMAN_SHEET]
        headers = human_operations.operation_headers(ops)
        source = self._source_record()
        issue = {"reason": "Reviewer: wrong document", "action": "Verify replacement",
                 "evidence_id": "fixture-review-1"}
        item = {"review_id": "human-fixture", "source_id": "PA001",
                "source_title": source["source_title"], "trigger": "HUMAN_REPORTED_ISSUE",
                "issue_codes": ["HUMAN_REPORTED_ISSUE"], "human_reported_issue": issue}
        values = human_operations.review_operation_values(item, source, source_updater.local_now({ "timezone": "Europe/Oslo"}))
        values.update(operator="Reviewer", operator_note="Wrong document", program_status="NEEDS_REPLAN")
        human_operations.append_operation(ops, headers, values)
        wb.save(self.workbook)
        wb.close()
        with patch("source_updater.run_updates", side_effect=AssertionError("No download")), redirect_stdout(io.StringIO()):
            for _ in range(2):
                self.assertEqual(run_cycle(self.config_path, full=False)[0], 0)
                pending = human_operations.pending_human_reported_issues(self.config_path)
                self.assertEqual(len(pending), 1)
                self.assertEqual(pending[0]["human_reported_issue"], issue)
        wb = load_workbook(self.workbook)
        ops = wb[human_operations.HUMAN_SHEET]
        headers = human_operations.operation_headers(ops)
        rows = [r for r in range(3, human_operations.last_operation_row(ops, headers)+1)
                if human_operations.operation_record(ops, r, headers)["program_status"] in human_operations.OPEN_STATUSES]
        self.assertEqual(len(rows), 1)
        human_operations.set_operation_fields(ops, rows[0], headers,
            {"decision": "APPLY", "operator": "Verifier", "operator_note": "Replacement independently verified"})
        wb.save(self.workbook)
        wb.close()
        human_operations.process_decisions(self.config_path)
        self.assertEqual(human_operations.pending_human_reported_issues(self.config_path), [])
