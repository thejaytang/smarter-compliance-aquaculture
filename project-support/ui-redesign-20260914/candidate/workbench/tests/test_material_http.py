...
class MaterialHTTPTests(unittest.TestCase):
    ...
    def test_reader_forwards_exact_bounded_render_options_without_extract(self):
        for suffix, expected in [('', {'page':2}), ('&render_width=2304', {'page':2,'render_width':2304})]:
            client = HTTPConnection('127.0.0.1', self.server.server_port, timeout=5)
            client.request('GET', '/api/material/reader?id='+'a'*32+'&page=2'+suffix)
            response = client.getresponse()
            self.assertEqual(response.status, 200, response.read())
            client.close()
            self.assertEqual(self.calls[-1], ('material_reader', {'material_id':'a'*32, 'options':expected}))
        self.assertEqual(len(self.calls), 2)

    def test_only_pdf_reads_advertise_local_interactive_reader_without_processing(self):
        for kind in ('pdf', 'html', 'spreadsheet'):
            self.result={'kind':kind,'source_sha256':'b'*64}
            client=HTTPConnection('127.0.0.1',self.server.server_port,timeout=5)
            try:
                client.request('GET','/api/material/reader?id='+'a'*32)
                response=client.getresponse();result=json.loads(response.read())
                self.assertEqual(response.status,200)
                self.assertEqual(result.get('interactive_pdf_reader'),'pdfjs-6.3.289' if kind=='pdf' else None)
                self.assertEqual(result['source_sha256'],'b'*64)
                self.assertEqual(self.calls[-1],('material_reader',{'material_id':'a'*32,'options':{}}))
            finally:client.close()
        self.assertEqual(len(self.calls),3)

    def test_reader_rejects_invalid_or_ambiguous_render_width_before_adapter_call(self):
        for value in ('', '0', '-1', '32769', '1.5', 'true', '100000000000', '1&render_width=2'):
            client = HTTPConnection('127.0.0.1', self.server.server_port, timeout=5)
            client.request('GET', '/api/material/reader?id='+'a'*32+'&render_width='+value)
            response = client.getresponse()
