...
def test_candidate_extraction_summary_survives_projection_rebuild_without_rewriting_evidence(tmp_path):
    from pdf_extraction.review.material_reads import compact_candidate
    store=MaterialStore(tmp_path);material=opened(store)
    started=store.start_candidate(request(material));cid=started['candidate']['id']
    metadata={'parser_status':'empty','parser_version':'material-structural-parser/2','processed_scope':['page:1'],
        'usable_scope':[],'unprocessed_scope':['page:2'],'unresolved':[{'scope_id':'page:1','message':'No native text'}]}
    store.finish_candidate(material['id'],cid,[],complete=False,metadata=metadata)
    full=store.candidate_detail(material['id'],cid);summary=store.read_compact(material['id'])['candidates'][0]
    assert summary['extraction']==compact_candidate(full)['extraction']
    assert summary['extraction']['parser_status']=='empty'
    assert summary['extraction']['processed_scope_count']==1 and summary['extraction']['usable_scope_count']==0
    assert summary['extraction']['unprocessed_scope_count']==1 and summary['extraction']['unresolved_count']==1
    with store.connect() as db:
        preserved=[tuple(row) for row in db.execute('SELECT * FROM material_candidates')]
        history=[tuple(row) for row in db.execute('SELECT * FROM material_revisions')]
        db.execute('DELETE FROM material_read_schema WHERE version=5')
    rebuilt=MaterialStore(tmp_path)
    assert rebuilt.read_compact(material['id'])['candidates'][0]['extraction']==summary['extraction']
    with rebuilt.connect() as db:
        assert [tuple(row) for row in db.execute('SELECT * FROM material_candidates')]==preserved
        assert [tuple(row) for row in db.execute('SELECT * FROM material_revisions')]==history
    assert rebuilt.candidate_detail(material['id'],cid)['metadata']==metadata


def test_old_candidate_does_not_invent_processed_or_usable_scope(tmp_path):
    store=MaterialStore(tmp_path);material=opened(store)
    started=store.start_candidate(request(material));cid=started['candidate']['id']
    store.finish_candidate(material['id'],cid,[],complete=False,metadata={'covered_scope':['page:1']})
    extraction=store.read_compact(material['id'])['candidates'][0]['extraction']
    assert extraction['parser_status'] is None
    assert all(extraction[key] is None for key in ('processed_scope_count','usable_scope_count','unprocessed_scope_count','unresolved_count'))
