...
The sidebar has Source Management System (Pending review, Source records, Review history) and Requirement Extraction System (Pending materials, Archive). Source review opens its own source/task details. Material entries open the three-pane workspace, including eligible originals not yet extracted. The reserved Requirement pane has no separate System3 navigation.

Overview presents source governance and material-level progress. A saved original, a personal draft, an accepted main version and a complete content review remain distinct. Counts do not establish extraction quality. Sources may have multiple unresolved reasons; reason counts cannot be summed into a count of sources. Historical A/B review remains available only through the explicit compatibility links.
