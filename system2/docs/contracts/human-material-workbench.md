...
PDF reader `/9` accepts optional `render_width`, an integer desired physical pixel width from 1 through 32768. The server rejects blank, duplicate, malformed and out-of-range requests; the reader also rejects non-integers and booleans before file/cache access. Width is rounded upward in 64-pixel steps for cache reuse. A request does not change the selected page, source hash or PDF-point geometry. With no request, the prior default remains `scale = min(1.5, 1800 / max(page_width, page_height))`.

Rendering preserves page aspect ratio and caps the longest bitmap edge at 4096 pixels and total area at 12,000,000 pixels, including upward bitmap dimension rounding. Only the requested page is rendered. `image_width` and `image_height` report the actual PNG dimensions; existing `width` and `height` remain PDF points. `render_width_requested` is the original request or null, `render_width_normalized` is the cache width or null, and `render_limited` is true when the actual image width cannot meet the request. `render_limits` reports `max_dimension`, `max_pixels`, `max_requested_width` and `width_step`. The UI must use actual dimensions to identify capped or old-server responses; enlargement alone is not a new high-resolution render. Metadata is additive. Request-specific fields are applied after the normalized cache lookup so adjacent requests cannot inherit one another's values. Reader version `/9` invalidates older derived cache entries; each cache hit still checks original hash and cache checksum. Rendering does not run parsing, OCR or downstream processing.

HTML reader `/9` adds `display_fidelity` with the explicit `reflowed_reading_view` mode, `original_layout_preserved: false`, resource counts and anchored visual omissions. Unsupported SVG, MathML, canvas and embedded/media regions receive a visible placeholder at the original outer region anchor before sanitization; nested implementation nodes do not inflate region counts. External/missing image references retain existing placeholders. Scripts, original styles and external resources remain disabled. These diagnostics describe display limitations, not automatic extraction quality or permission to fetch missing assets. Original bytes and existing source anchors remain separate from the derived view.
...
The [offline human review contract](../../../project-support/design/offline-human-review.md) adds named personal drafts and explicit main adoption. Workbench mutation requests now require `X-Material-Workbench: 3`; older UI writes are refused. This HTTP compatibility marker does not change the material document schema or manufacture historical acceptance.
...
New Workbench comparisons persist and return `merge_version: 2`. Historical comparisons without this field use version 1, including their existing whole-list decisions; opening, replaying or applying them never upgrades their meaning. In version 2, exact equal block-ID sets across all three inputs allow order to be compared separately from per-block fields. An order difference has `path: /blocks`, `kind: order`, and per-side arrays of IDs; choosing its value changes only the sequence, not block text, tables or relationships. An edited sequence must contain every existing block exactly once. Explicit order choices also validate heading positions and levels before recording a decision. Per-side `reference_context` provides readable labels and original references from each actual input. The same path hash in another saved comparison is not authority to reuse a decision.

Unknown split/replacement correspondence, changed identity sets and ambiguous insertions retain conservative structural handling. Table-shape changes remain whole-table choices; identical block IDs do not establish cell correspondence after a row/merge change. Machine comparison retains current personal work until required choices are explicit, including order choices. `input_revision` identifies the extraction's content revision; it is distinct from the saved record revision advanced by extraction lifecycle events. No unrecorded input saved-revision value is inferred.
...
## Product-readiness diagnostic additions

Parser `/3` reports `processed_scope`, `usable_scope`, `unprocessed_scope` and location-bound `unresolved` entries. Historical `covered_scope` means processed range, not usable text or accuracy. Image-only and blank native pages cannot claim usable extraction; table-only content remains usable. The service retains the raw parser status in candidate metadata. For workflow compatibility an `empty` parser result is a repairable `partial` candidate, never a complete directly adoptable result. Compact reads expose additive counts, with null for historical unknowns; full evidence remains available through candidate Processing details. Marker-order repairs preserve each block and its original references and emit a separate repair artifact. They do not infer headings or legal meaning.

Reader `/7` adds `navigation_anchors` and `default_anchor` for a content-oriented original outline. The full `anchors` collection and complete sanitized original remain accessible, including source-linked nodes outside the outline. Navigation does not delete source content.

Repeated personal preparation reuses an identical saved-input submission; repeated preview reuses its persisted comparison choices only while master/source inputs remain identical. A changed input creates a new comparison without inheriting old choices. Historical submissions and comparisons are retained. Machine preview additionally binds actor, material, candidate and exact material/candidate/source digests. With identical inputs it resumes recorded choices, preferring an existing decided preview over a later empty duplicate retained from older behavior; it never combines, deletes or migrates historical decisions. Same-instance duplicate preparation is serialized. Changed inputs create a fresh preview. This identity behavior does not grant automatic adoption or confirmation.
